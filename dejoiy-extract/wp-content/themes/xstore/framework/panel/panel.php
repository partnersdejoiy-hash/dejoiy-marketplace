...
class EthemeAdmin{
    ...
    public $filter_name = 'upload_mimes';

    // ! Main construct/ add actions
    public function main_construct(){
        add_action( 'admin_menu', array( $this, 'et_add_menu_page' ), 20 );
//        add_action( 'admin_head', array( $this, 'et_add_menu_page_target') );
        add_action('admin_enqueue_scripts', array( $this, 'et_add_menu_page_target') );
        add_action( 'wp_ajax_et_ajax_panel_popup', array($this, 'et_ajax_panel_popup') );

        // Enable svg support
        add_filter( $this->filter_name, [ $this, 'add_svg_support' ] );
        ...
        // Stas
        //$this->init_vars(); - _load_textdomain_just_in_time notice
        // Andrii
                ...
                case 'demos.min':
                    if (etheme_is_activated()){
	                    wp_localize_script( 'etheme_panel_'.$this->page['script'], 'XStorePanelPatcherConfig', array(
		                    ...
		                    'test_mode' => isset($_GET['xstore-patches-test-mode']),
		                    'nonce'      => wp_create_nonce( 'xstore_patches_apply_nonce' ),
		                    'theme_version' => ETHEME_THEME_VERSION,
		                    'refresh_patches_nonce' => wp_create_nonce('refresh-patches'),
	                    ) );
                    }
                case 'plugins.min':
...
//				$this->page['class'] = 'ai';
            ...
            case 'et-panel-theme-builders':
                ...
                $this->page['class'] = 'plugins';
                break;
            case 'et-panel-language':
                $this->page['script'] = 'theme-language.min';
                $this->page['template'] = 'language';
                $this->page['class'] = 'language';
                break;
...
//                do_action('etheme_panel_page_has_options');
    ...
    public function et_panel_ajax(){
        ...
        if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'nonce_etheme_panel_actions' ) ) {
        ...
        if ( !empty($action_type) ){
            ...
            $allowed_actions = array(
                'et_instagram_user_add',
                'et_instagram_user_remove',
                'et_instagram_save_settings',
                'et_email_builder_switch_default',
                'et_built_in_language',
                'et_built_in_language_select',
                'et_documentation_beacon',
                'et_email_builder_switch_dev_mode_default',
                'et_maintenance_mode_switch_default',
                'et_panel_dark_light_switch_default',
                'et_built_in_language_sync',
            );
            ...
            switch ( $action_type ) {
                ...
                case 'et_email_builder_switch_default':
                    ...
                    break;
                 case 'et_built_in_language':
                    $this->require_class('language');
                    $class = new ETC_Language();
                    $class->et_switch_language();
                    break;
                case 'et_built_in_language_select':
                    $this->require_class('language');
                    $class = new ETC_Language();
                    $class->et_built_in_language_select();
                    break;
                case 'et_built_in_language_sync':
                    $this->require_class('language');
                    $class = new ETC_Language();
                    $class->et_built_in_language_sync();
                    break;
    ...
    public function et_add_menu_page(){
        ...
        $system->system_test();
        $system_log = count($system->system_logs());
        $result = $system->result();

        $domains_limit_reached = false;
        ...
        $priorities = array(
            ...
            'widgets' => 52.35,
            'email-builder' => 52.4,
            'theme-options' => 52.1, // used in another file
            'etheme_slides' => 53.1,
        ...
        if (
            ...
            if (apply_filters('etheme_hide_updates', false)){
                ...
                $is_update_support = $force_hide_main_badges ? 'active' : $check_update->get_support_status(); // force activate to reset counter in category

                $domains_count = $check_update->domains_count_limit();
...
//            $server_label = esc_html__( 'Server Reqs.', 'xstore' );
        ...
        if ( !$force_hide_main_badges ) {
            if (in_array('welcome', $show_pages)) {
                if (!$is_activated || $is_update_support != 'active') {
                    $notices['welcome'] = 1;
                }
            }

            if ($domains_limit_reached) {
                if (isset($notices['welcome'])) {
                    $notices['welcome']++;
                } else {
                    $notices['welcome'] = 1;
                }
        ...
        if ( in_array('system_requirements', $show_pages) ) {
            if ($system_log) {
                $notices['system_requirements'] = $system_log;
            }
        }

//        if ( $domains_limit_reached )
//            $notices['licence_limit'] = 1;
        ...
        if ( $is_activated || $allow_full_access ) {
            if ( class_exists('Etheme_Patcher') ) {
                ...
                if ( $available_patches ) {
                    $notices['patches'] = $available_patches;
                }
            }
        }
        ...
        if (count($notices)) {
            ...
            if ( !$force_hide_main_badges )
                $main_item_postfix = ' <span class="awaiting-mod">'.$count.'</span> ';
        }

        add_menu_page(
            $label . ' ' . $main_item_postfix . ( ( (!$is_activated && !$allow_full_access) || !$result || $is_update_available || $is_update_support !='active' ) ? $info : '' ),
            $label . ' ' . $main_item_postfix . ( ( (!$is_activated && !$allow_full_access) || !$result || $is_update_available || $is_update_support !='active' ) ? $info : '' ),
            'manage_options',
            'et-panel-welcome',
        ...
        if ( in_array('welcome', $show_pages) ) {
            $postfix = isset($notices['welcome']) ? '<span class="et-title-label et-title-label-error">'.$notices['welcome'].'</span> ' : '';
            add_submenu_page(
                'et-panel-welcome',
                esc_html__( 'Dashboard', 'xstore' )  .' '. $postfix . ( (!$is_activated && !$allow_full_access) || ($is_update_support !='active' && $result) ? $info : ''),
                esc_html__( 'Dashboard', 'xstore' ) .' '. $postfix . ( (!$is_activated && !$allow_full_access) || ($is_update_support !='active' && $result) ? $info : ''),
                'manage_options',
                'et-panel-welcome',
                array( $this, 'etheme_panel_page' )
...
//            if ( in_array('system_requirements', $show_pages) ) {
//                $server_req_item_shown = true;
//                $postfix = isset($notices['system_requirements']) ? '<span class="et-title-label et-title-label-error">'.$notices['system_requirements'].'</span> ' : '';
//                add_submenu_page(
//                    'et-panel-welcome',
//                    $server_label . $postfix,
//                    $server_label . $postfix,
//                    'manage_options',
...
//        }
        ...
        if ( $is_activated || $allow_full_access ) {
            ...
            if ( in_array('demos', $show_pages) ) {
                add_submenu_page(
                    'et-panel-welcome',
                    esc_html__( 'Import Demos 150+', 'xstore' ),
                    esc_html__( 'Import Demos 150+', 'xstore' ),
                    'manage_options',
                    'et-panel-demos',
                    array( $this, 'etheme_panel_page' )
            ...
            if ( in_array('welcome', $show_pages) ) {
                add_submenu_page(
                    'et-panel-welcome',
                    esc_html__( 'Theme License', 'xstore' ),
                    esc_html__( 'Theme License', 'xstore' ),
                    'manage_options',
                    'et-panel-welcome',
                    array( $this, 'etheme_panel_page' )
            ...
            $postfix = isset($notices['patches']) ? ' <span class="et-title-label patches-count count-'.$notices['patches'].'">'.$notices['patches'].'</span> ' : '';
            add_submenu_page(
                'et-panel-welcome',
                (in_array('patcher', $show_pages) ? esc_html__( 'Patcher', 'xstore' ) : ''),
                (in_array('patcher', $show_pages) ? esc_html__( 'Patcher', 'xstore' ) : '') . $postfix,
                'manage_options',
                'et-panel-patcher',
                array( $this, 'etheme_panel_page' )
            );
        ...
        add_submenu_page(
            'et-panel-welcome',
            esc_html__( 'Built-In Translations', 'xstore' ),
            esc_html__( 'Built-In Translations', 'xstore' ),
            'manage_options',
            'et-panel-language',
            array( $this, 'etheme_panel_page' )
...
//            if (!$server_req_item_shown && in_array('system_requirements', $show_pages)) {
            if (in_array('system_requirements', $show_pages)) {
                $postfix = isset($notices['system_requirements']) ? '<span class="et-title-label et-title-label-error">'.$notices['system_requirements'].'</span> ' : '';
                add_submenu_page(
                    'et-panel-welcome',
            ...
            if ( in_array('support', $show_pages) ) {
                add_submenu_page(
                    'et-panel-welcome',
                    esc_html__( 'Get Help Now', 'xstore' ),
                    esc_html__( 'Get Help Now', 'xstore' ),
                    'manage_options',
                    etheme_support_forum_url(),
                    ''
                );
            }
        ...
        if ( ($is_activated || $allow_full_access) && ($allow_force_access_woocommerce_features || $is_wc) && in_array( 'sales_booster', $show_pages ) ) {
            ...
            do_action('admin_menu_pages_after_sales_booster', 'et-panel-sales-booster');
        }
    ...
    public function et_add_menu_page_target() {
        wp_add_inline_script( 'etheme_admin_js', '
            jQuery(document).ready( function($) {
                let setup_wizard = $(\'#adminmenu [href="admin.php?page=xstore-setup"]\').parent();'.
                (!get_option('etheme_current_version') ?
                    'if(setup_wizard.html()){ $(\'#adminmenu [href="admin.php?page=et-panel-welcome"]:not(.wp-has-submenu)\').parents(\'.wp-first-item\').after(\'<li>\'+setup_wizard.html()+\'</li>\'); setup_wizard.remove();}'
                    :
                    'setup_wizard.remove();').
...
//        echo ob_get_clean();
    ...
    public function et_ajax_panel_popup(){
        ...
        if ( isset( $_POST['type'] ) ) {
            if ( in_array($_POST['type'], array('registration', 'deregister', 'remove_content', 'install_theme_builder')) ) {
                ob_start();
                get_template_part( 'framework/panel/templates/popup-theme', sanitize_file_name($_POST['type']));
                $response['content'] = ob_get_clean();
            }
            else {
                ob_start();
                get_template_part( 'framework/panel/templates/popup-'.sanitize_file_name($_POST['type']), 'content' );
                $response['content'] = ob_get_clean();
            }
        } else {
    ...
    public function admin_redirects() {
        ...
        if(!get_option('etheme_current_version')){
            wp_safe_redirect( admin_url( 'admin.php?page=xstore-setup&step=welcome' ) );
        } else {
...
//			$local_settings_parsed[ $local_settings_key ][ $setting['name'] ] = $setting['value'];
        ...
        <ul class="et-filters et-filters-type-1">
            ...
            <ul>
                ...
                foreach ($filters_list as $filter_key => $filter_details) {
	                ...
	                $filter_classes = $filter_classes ? ' class="'.$filter_classes.'"' : '';
                    ?>
                    <li class="et-filter <?php echo esc_attr($settings['type']); if ( $active_key && $filter_key == $active_key ) echo ' active'; ?>" data-filter="<?php echo esc_attr($filter_key); ?>">
                        <?php if ( $filter_link ) { ?>
                        <a href="<?php echo esc_url($filter_link); ?>" target="_blank" rel="nofollow" <?php echo esc_html( $filter_classes);?>>
                            <?php }
    ...
    public function get_loader($percentage = false, $echo = true) {
        $loader_html = '<svg class="loader-circular" viewBox="25 25 50 50">';
            $loader_html .= '<circle class="loader-path" cx="50" cy="50" r="12" fill="none" stroke-width="2" stroke-miterlimit="10"></circle>';
        $loader_html .= '</svg>';
        if ( $percentage )
            $loader_html .= '<span class="loader-percent" data-percent="1">1%</span>';
        if ( $echo )
            echo '<span class="et-loader' . (($percentage) ? ' et-loader-percent' : '') . '">' . $loader_html . '</span>';
        else
            return '<span class="et-loader' . (($percentage) ? ' et-loader-percent' : '') . '">' . $loader_html . '</span>';
    }
    public function get_search_form($type = 'demos', $placeholder_text = '', $args = []) {
        ...
        <div class="etheme-search">
            <?php if ( $args['icon_position'] == 'after' ) { ?>
                <input type="text" class="etheme-<?php echo esc_attr($type); ?>-search form-control" placeholder="<?php echo esc_attr($placeholder_text); ?>"<?php if (isset($_GET['s']) && !empty($_GET['s'])) echo ' value="'.esc_attr($_GET['s']).'"'; ?>>
            <?php } ?>
            <?php if ( $args['icon'] ) { ?>
            ...
            <?php if ( $args['icon_position'] == 'before' ) { ?>
                <input type="text" class="etheme-<?php echo esc_attr($type); ?>-search form-control" placeholder="<?php echo esc_attr($placeholder_text); ?>"<?php if (isset($_GET['s']) && !empty($_GET['s'])) echo ' value="'.esc_attr($_GET['s']).'"'; ?>>
            <?php } ?>
        </div>
    <?php }
    ...
    public function get_additional_panel_blocks($args = array()) {
        $args = wp_parse_args( $args, array(
            'type' => '',
            'vertical_columns' => false,
            'documentation_block' => false,
        ...
        $vertical_columns = ( $args['vertical_columns'] ) ? ' one-col': '';
        ?>
        <div class="xstore-panel-info-blocks<?php echo esc_attr($vertical_columns); ?>">
            <?php if ( $args['documentation_block'] ) : ?>
                <div class="xstore-panel-info-block">
                    ...
                    <div>
                        ...
                        <p><a href="<?php etheme_documentation_url() ?>" target="_blank"><?php if ( $args['link_icon'] ) { ?><span class="dashicons dashicons-external"></span> <?php } ?><?php echo esc_html__('Go to documentation', 'xstore'); ?></a></p>
                    </div>
                </div>
            <?php endif; ?>
            <?php if ( $args['support_block'] ) : ?>
                <div class="xstore-panel-info-block">
                    ...
                    <div>
                        ...
                        <p><a href="<?php etheme_support_forum_url(true); ?>" target="_blank" rel="nofollow"><?php if ( $args['link_icon'] ) { ?><span class="dashicons dashicons-external"></span> <?php } ?><?php echo esc_html__('Get assistance now', 'xstore'); ?></a></p>
                    </div>
                </div>
            <?php endif; ?>
            <?php if ( $args['suggestion_block'] ) : ?>
                <div class="xstore-panel-info-block">
                    ...
                    <div>
                        ...
                        <p><a href="<?php etheme_contact_us_url(true); ?>" target="_blank" rel="nofollow"><?php if ( $args['link_icon'] ) { ?><span class="dashicons dashicons-external"></span> <?php } ?><?php echo esc_html__('Send a message', 'xstore'); ?></a></p>
                    </div>
                </div>
            <?php endif; ?>
        </div>
