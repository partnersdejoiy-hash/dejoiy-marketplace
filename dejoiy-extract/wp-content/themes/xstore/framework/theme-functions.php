...
//		$id = $post_id['id'];
        ...
        $is_mobile_device = ( function_exists('wp_is_mobile') && wp_is_mobile() ) ? true : false;
...
//				set_query_var( 'et_product-archive-advanced-stock', etheme_get_option( 'advanced_stock_status_archive', false ) );
            ...
            if  (is_shop() || $is_product_cat || is_product_tag() || is_product_taxonomy() || is_tax('brand') || is_post_type_archive( 'product' ) ||
                ...
                if ($filters_area_widgets_open_close_type == 'closed' ||
                    (($filters_area_widgets_open_close_type == 'closed_mobile') && $is_mobile_device) ||
                    (($filters_area_widgets_open_close_type == 'closed_desktop') && !$is_mobile_device)) {
                ...
                if ($sidebar_widgets_open_close_type == 'closed' ||
                    (($sidebar_widgets_open_close_type == 'closed_mobile') && $is_mobile_device) ||
                    (($sidebar_widgets_open_close_type == 'closed_desktop') && !$is_mobile_device)) {
...
if(!function_exists('etheme_add_body_classes')) {
    function etheme_add_body_classes($classes) {
        ...
        if ( !apply_filters('etheme_is_fse_theme', false) ) {
            $classes[] = 'et_is-classic-theme';
        }
...
if ( ! function_exists( 'etheme_search_post_excerpt' ) ) :
    ...
    function etheme_search_post_excerpt( $where = '' ) {
        ...
        $prefix = $wpdb->prefix ? $wpdb->prefix : 'wp_';


        $is_wc_query = ! empty( $wp_the_query->query_vars['wc_query'] );
        $post_type   = isset( $wp_the_query->query_vars['post_type'] ) ? $wp_the_query->query_vars['post_type'] : '';

        $is_product_query = $is_wc_query;
        ...
        if ( $is_product_query && isset( $_GET['filter_brand'] ) && ! empty( $_GET['filter_brand'] ) ) {
            ...
            if ( ! empty( $ids ) ) {
                ...
                $where .= " AND {$wpdb->posts}.ID IN (
					...
					WHERE tr.term_taxonomy_id IN ({$ids})
				)";
            }
        }

        if ( $is_product_query && isset( $_GET['filter_cat'] ) && ! empty( $_GET['filter_cat'] ) ) {
        ...
        if ( is_search() ) {
            ...
            if ( etheme_get_option( 'search_by_sku_et-desktop', 1 ) ) {
                ...
                if ( defined( 'ICL_LANGUAGE_CODE' ) && ! defined( 'LOCO_LANG_DIR' ) && ! defined( 'POLYLANG_PRO' ) ) {
                    ...
                    $sku_search_where = $wpdb->prepare(
                        " OR ( {$wpdb->posts}.ID IN (
                            ...
                            AND {$wpdb->posts}.ID IN (
                                SELECT ID
                                FROM {$prefix}posts
                                LEFT JOIN {$prefix}icl_translations
                                    ON {$prefix}icl_translations.element_id = {$prefix}posts.ID
                                WHERE post_type = 'product'
                                AND post_status = 'publish'
                                AND {$prefix}icl_translations.language_code = %s
                            )",
                        $like,
                        ICL_LANGUAGE_CODE
                    );
                    ...
                    if ( $hide_out_of_stock ) {
                        $sku_search_where .= " AND {$prefix}posts.ID IN (
								SELECT {$prefix}postmeta.post_id
								FROM {$prefix}postmeta
								WHERE {$prefix}postmeta.meta_key = '_stock_status'
								AND {$prefix}postmeta.meta_value = 'instock'
							)";
                    }

                    $sku_search_where .= ') )';
                    $where            .= $sku_search_where;
                    ...
                    $exclude_terms = array_merge(
                        (array) $visibility['exclude-from-catalog'],
                        (array) $visibility['exclude-from-search']
                    );
                    ...
                    if ( ! empty( $exclude_terms ) ) {
                        $where .= " AND {$wpdb->posts}.ID NOT IN (
								...
								WHERE term_taxonomy_id IN (" . implode( ',', array_map( 'intval', $exclude_terms ) ) . ")
							)";
                    }
            ...
            if ( etheme_get_option( 'search_product_variation_et-desktop', 0 ) || $variable_products_detach ) {
                $variation_search_where = $wpdb->prepare(
...
//			$type = get_theme_mod( 'images_loading_type_et-desktop', 'lazy' );
        ...
        if ( $type && $type != 'default' ) {
            if ( $type == 'lqip') {
                ...
                $new_attr = 'src="' . $placeholder_image . '"';

                // Add custom css to image because wpb stop resizing images
                if (
                    function_exists('wpb_getImageBySize')
                    && is_array($size)
                    && count($size) == 2
                ){
                    $new_attr .= ' style="width: '.$size[0].'px; height: '.$size[1].'px;"';
                }
                $new_attr .= ' data-src';
...
add_action('after_switch_theme', 'etheme_clear_required_core_notice_cache');
add_filter('pre_set_site_transient_update_themes', 'etheme_clear_required_core_notice_cache');
add_action('admin_init', 'etheme_sync_gutenberg_html_templates_on_admin_init');

function etheme_clear_required_core_notice_cache() {
    $transients = [
        'etheme_xstore_core_min_version'
    ];
...
function etheme_sync_gutenberg_html_templates_on_admin_init() {
    ...
    if ( empty( $imported['gutenberg_html_templates'] ) || ! is_array( $imported['gutenberg_html_templates'] ) ) {
        return;
    }

    $templates = array_values( array_filter( $imported['gutenberg_html_templates'], 'is_string' ) );
    if ( empty( $templates ) ) {
        return;
    }
    ...
    foreach ( $templates as $filename ) {
        ...
        if ( $filename === '' || substr( $filename, -5 ) !== '.html' ) {
            continue;
        }
...
function etheme_required_core_notice(){
    ...
    $transient_key = 'etheme_xstore_core_min_version';
    $cached_core_version = get_transient($transient_key);
    if ( $cached_core_version !== false ) {
        $should_display_notice = defined('ETHEME_CORE_MIN_VERSION') ? version_compare( ETHEME_CORE_MIN_VERSION, $cached_core_version, '>' ) : false;
    }
    else {
        $file = ABSPATH . 'wp-content/plugins/et-core-plugin/et-core-plugin.php';
        if (!file_exists($file)) return;
        $plugin = get_plugin_data($file, false, false);
        $should_display_notice = version_compare( ETHEME_CORE_MIN_VERSION, $plugin['Version'], '>' );
        set_transient($transient_key, ETHEME_CORE_MIN_VERSION, DAY_IN_SECONDS );
    }
    ...
    if ( $should_display_notice ) {
        ...
        <div class="et-message et-info">
        	...
        	<ul>
				...
				<li>2. <strong>FTP:</strong> Download the updated XStore Core plugin from your <a href="https://themeforest.net/" target="_blank">Downloads</a> section and upload it via FTP.</li>
...
function etheme_get_demo_versions(){
    ...
    $url = 'https://www.8theme.com/import/xstore-demos/1/versions/?json=1';
...
function etheme_get_demo_import_data( $version = '', $engine = 'wpb' ) {
    ...
    $allowed_engines = array( 'wpb', 'elementor', 'gutenberg', 'elementor_builders' );
    if ( empty( $engine ) || ! in_array( $engine, $allowed_engines, true ) ) {
        $engine = 'wpb';
    }

    if ($engine == 'elementor' && ! in_array($engine, $versions[$version]['engine'])){
        $engine = 'elementor_builders';
    }

    $transient_key = 'etheme_demo_import_data_' . $version . '_' . $engine;

    $data = get_transient( $transient_key );

    if ( ! is_array( $data ) || empty( $data ) ) {
        $base_url = apply_filters( 'etheme_protocol_url', ETHEME_BASE_URL . 'import/xstore-demos/' );
        $version_slug = rawurlencode( $version );
        $engine_slug  = rawurlencode( $engine );

        $url = $base_url . $version_slug . '/' . $engine_slug . '/import-data.json';
        $data = array();
        $api_response = wp_remote_get( $url );
    ...
    // Fallback for old payload format from versions list.
    if ( empty( $data ) ) {
        $versions = etheme_get_demo_versions();
        if ( isset( $versions[ $version ] ) && is_array( $versions[ $version ] ) ) {
            $version_data = $versions[ $version ];
            $data = array(
                'plugins'            => isset( $version_data['plugins'] ) && is_array( $version_data['plugins'] ) ? $version_data['plugins'] : array(),
                'to_import'          => isset( $version_data['to_import'] ) && is_array( $version_data['to_import'] ) ? $version_data['to_import'] : array(),
                'elementor_builders' => ! empty( $version_data['elementor_builders'] ),
            );
        }
    }
    ...
    if ( ! isset( $data['to_import'] ) || ! is_array( $data['to_import'] ) ) {
        $data['to_import'] = array();
    }
    if ( ! array_key_exists( 'elementor_builders', $data ) ) {
        $data['elementor_builders'] = false;
    }
...
add_filter('paginate_links_output', function ($r) {
    if (!empty($r) && apply_filters('etheme_should_enqueue_style', true) ) {
...
function et_wc_get_product_ids_on_sale(){
    $args = array(
        'post_type'      => 'product',
        'posts_per_page' => 100, // Do not use -1
        'fields'         => 'ids',
        'meta_query'     => array(
            'relation' => 'AND',
            array(
...
function et_get_domain_tokens($code){
    if(!$code) return false;
    $response = wp_remote_get( apply_filters('etheme_protocol_url', ETHEME_API) . 'domains/' . $code . '?type=tokens');

    //write_log( apply_filters('etheme_protocol_url', ETHEME_API) . 'domains/' . $code . '?type=tokens');
