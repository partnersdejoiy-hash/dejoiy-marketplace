...
<div class="categories_list">
	<ul>
	  <?php if( $selected_term ) { ?>
	    <li class="parent_cat"><a class="" href="<?php echo esc_url($store_user->get_shop_url()); ?>"><?php _e( 'Show All', 'wc-multivendor-marketplace' ); ?></a></li>
	  <?php } ?>
			...
			if( $vendor_category_id ) {
				...
				if( is_array( $vendor_category ) && !empty( $vendor_category ) ) {
					...
					if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
						?>
						<li class="parent_cat"><a data-term_id="<?php echo esc_attr($vendor_term->term_id); ?>" class="<?php if( $has_toggle ) echo 'wcfm_tax_filter_toggle_parent '; ?> <?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/#tab_links_area"><?php echo esc_html($vendor_term->name); ?></a></li>
						<?php
					}
					foreach( $vendor_category as $vendor_category_child_id => $vendor_category_child ) {
						...
						if( !is_array( $vendor_category_child ) ) {
							if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
								...
								?>
								<li class="child_cat <?php echo $tax_toggle_class; ?>"><a class="<?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/#tab_links_area"><?php echo esc_html($vendor_term->name); ?></a></li>
								<?php
							}
						} else {
							?>
							...
							foreach( $vendor_category_child as $vendor_category_child2_id => $vendor_category_child2 ) {
								...
								if( !is_array( $vendor_category_child2 ) ) {
									if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
										...
										$display_vendor_term[$vendor_term->term_id] = $vendor_term->term_id;
										?>
										<li class="child2_cat <?php echo $tax_toggle_class; ?>"><a class="<?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/#tab_links_area"><?php echo esc_html($vendor_term->name); ?></a></li>
										<?php
									}
								} else {
									?>
									...
									foreach( $vendor_category_child2 as $vendor_category_child3_id => $vendor_category_child3 ) {
										...
										if( !is_array( $vendor_category_child3 ) ) {
											if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
												...
												$display_vendor_term[$vendor_term->term_id] = $vendor_term->term_id;
												?>
												<li class="child3_cat <?php echo $tax_toggle_class; ?>"><a class="<?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/#tab_links_area"><?php echo esc_html($vendor_term->name); ?></a></li>
												<?php
											}
										} else {
											?>
											...
											foreach( $vendor_category_child3 as $vendor_category_child4_id => $vendor_category_child4 ) {
												...
												if( !is_array( $vendor_category_child4 ) ) {
													if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
														...
														$display_vendor_term[$vendor_term->term_id] = $vendor_term->term_id;
														?>
														<li class="child4_cat <?php echo $tax_toggle_class; ?>"><a class="<?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/#tab_links_area"><?php echo esc_html($vendor_term->name); ?></a></li>
														<?php
													}
												} else {
													?>
													...
													foreach( $vendor_category_child4 as $vendor_category_child5_id => $vendor_category_child5 ) {
														...
														if( !is_array( $vendor_category_child5 ) ) {
															if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
																...
																$display_vendor_term[$vendor_term->term_id] = $vendor_term->term_id;
																?>
																<li class="child5_cat <?php echo $tax_toggle_class; ?>"><a class="<?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/#tab_links_area"><?php echo esc_html($vendor_term->name); ?></a></li>
																<?php
															}
														} else {
															?>
															...
															foreach( $vendor_category_child5 as $vendor_category_child6_id => $vendor_category_child6 ) {
																...
																if( !is_array( $vendor_category_child6 ) ) {
																	if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
																		...
																		$display_vendor_term[$vendor_term->term_id] = $vendor_term->term_id;
																		?>
																		<li class="child6_cat <?php echo $tax_toggle_class; ?>"><a class="<?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/#tab_links_area"><?php echo esc_html($vendor_term->name); ?></a></li>
																		<?php
																	}
																} else {
																	?>
																	...
																	foreach( $vendor_category_child6 as $vendor_category_child7_id => $vendor_category_child7 ) {
																		...
																		if( !is_array( $vendor_category_child7 ) ) {
																			if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
																				...
																				$display_vendor_term[$vendor_term->term_id] = $vendor_term->term_id;
																				?>
																				<li class="child7_cat <?php echo $tax_toggle_class; ?>"><a class="<?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/#tab_links_area"><?php echo esc_html($vendor_term->name); ?></a></li>
																				<?php
																			}
																		} else {
																			?>
					...
					if( $vendor_term && $vendor_term->term_id && $vendor_term->name ) {
						...
						$display_vendor_term[$vendor_term->term_id] = $vendor_term->term_id;
						?>
						<li class="parent_cat"><a class="<?php if( $vendor_term->slug == $selected_term ) echo 'active'; ?>" href="<?php echo esc_url($store_user->get_shop_url() . $url_base . '/' . $vendor_term->slug); ?>/"><?php echo esc_html($vendor_term->name); ?></a></li>
						<?php 
					}
				}
			} 
