...
//        add_action( 'wp_ajax_nopriv_xstore_wishlist_page_action', array( $this, 'global_page_actions' ) );
        ...
        add_action( 'wp_ajax_xstore_get_origin_wishlist_product_variation_id', array( $this, 'get_origin_wishlist_product_variation_id' ) );
        add_action( 'wp_ajax_nopriv_xstore_get_origin_wishlist_product_variation_id', array( $this, 'get_origin_wishlist_product_variation_id' ) );
...
//		$this->define_settings();
    ...
    public function define_settings() {
        self::$settings = array(
            ...
            'add_text' => esc_html(get_theme_mod('xstore_wishlist_label_add_to_wishlist', esc_html__('Add to Wishlist', 'xstore-core'))),
            'remove_text' => esc_html(get_theme_mod('xstore_wishlist_label_browse_wishlist', esc_html__('Remove from Wishlist', 'xstore-core'))),
        ...
        switch (get_theme_mod('xstore_wishlist_icon', 'type1')) {
            ...
            case 'custom':
                ...
                if ( $icon_custom != '' ) {
                    ...
                    add_filter('etheme_mobile_panel_element_wishlist_icon', function ($icon_svg, $icon_key) {
    ...
    public function enqueue_scripts(){
        ...
        wp_localize_script(
            ...
            [
                ...
                'ajax_fragments' => apply_filters('xstore_wishlist_mini_content_ajax', (self::get_products_count()>=1)),
        ...
        if ( apply_filters('etheme_should_enqueue_local_script', true, self::$key ) )
     ...
     */
    function header_wishlist_quantity($updated_count = false, $only_count = false) {
     ...
     */
    public function header_mini_wishlist($args = array()) {
        ...
        if ( apply_filters('xstore_wishlist_mini_content_ajax', true) ) {
            $wishlist_class  = 'et_b_wishlist-dropdown product_list_widget cart_list';
            ?>
            <div class="<?php esc_attr_e( $wishlist_class ); ?>"></div>
            <?php
        }
        else
            $this->header_mini_wishlist_products();

        $this->header_mini_wishlist_footer($args);
    }

    public function header_mini_wishlist_footer($args = array()) {
    ...
    public function header_mini_wishlist_products($updated_products = false) {
        ...
        $limit = apply_filters('etheme_mini_wishlist_items_count', $limit);
        ...
        <div class="<?php esc_attr_e( $wishlist_class ); ?>">
            <?php if ( ! empty( $products ) ) : ?>
                ...
                <ul class="cart-widget-products">
                    ...
                    $etheme_ltv_price = etheme_get_option( 'ltv_price', esc_html__( 'Login to view price', 'xstore-core' ) );
                    $just_catalog = defined( 'DOING_AJAX' ) && DOING_AJAX ? etheme_is_catalog() : get_query_var('et_is-catalog', false);
                    ...
                    foreach ( $products as $product_info ) {
                        ...
                        <li class="woocommerce-mini-wishlist-item">
                            ...
                            <div class="product-item-right" data-row-id="<?php esc_attr_e( $origin_product_id ); ?>">
                                ...
                                <<?php echo apply_filters('etheme_woocommerce_mini_wishlist_product_title_tag', 'h4'); ?> class="product-title">
                                    <a href="<?php echo esc_url( $_product->get_permalink() ); ?>"><?php echo wp_specialchars_decode( $product_name ); ?></a>
                                </<?php echo apply_filters('etheme_woocommerce_mini_wishlist_product_title_tag', 'h4'); ?>>

                                <?php if ( $add_remove_ajax ) : ?>
                                    <a href="<?php echo add_query_arg( 'remove_from_wishlist', $origin_product_id, esc_url( self::$wishlist_page ) ); ?>"
                                ...
                                <div class="descr-box">
                                    <?php
                                    if ( $just_catalog ) {
                                        if (etheme_get_option('just_catalog_price', 0) && $etheme_ltv_price != '') {
                                            echo $etheme_ltv_price;
                                        }
                                    }
                                    else {
                                    $quantity_args = array(
    ...
    public function fragments() {
        ...
        ob_start();
        $this->header_wishlist_quantity(isset($_POST['products_count']) ? $_POST['products_count'] : false, true);
        ...
        if ( isset($_POST['products_count'])) {
            ...
            $this->header_wishlist_quantity(false);
        }
        ...
        if ( isset($_POST['products'])) {
            ...
            $this->header_mini_wishlist_products(false);
        }
        $data['fragments']['.et_b_wishlist-dropdown'] = ob_get_clean();

        $data = array(
			'fragments' => apply_filters(
				'xstore_wishlist_refresh_fragments',
				$data['fragments'],
				$product_count,
				$data['fragments']['.et_b_wishlist-dropdown']
			),
		);
     ...
     * @return mixed
     */
    public function get_origin_wishlist_product_variation_id() {
...
//    }
    ...
    public function add_display_post_states($post_states, $post) {
        if ( $post && (int)get_theme_mod('xstore_wishlist_page', '') === $post->ID ) {
            $post_states['xstore_page_for_wishlist'] = __( 'Wishlist page', 'xstore-core' );
        }
...
//		}
    ...
    public function update_ids_after_login( $user_login, $user){
		...
		// Remove "time" from massive for cookie
		$ready_products = array_map(function($item) {
			return preg_replace('/,"time":\d+/', '', $item);
		}, $ready_products);

//        unset( $_COOKIE[$cookie_key] );
        \wc_setcookie($cookie_key, implode('|', $ready_products), time() + ($this->get_days_cache() * WEEK_IN_SECONDS));
//        $_COOKIE[ $cookie_key ] = implode('|', $ready_products);
     ...
     * @return mixed|string
     */
    public function get_user_key($custom_user_id = false) {

        $user_id = $custom_user_id ? $custom_user_id : get_current_user_id();
...
//    }
    ...
    public function reset_products() {
        $cookie_key = $this->get_cookie_key();
//        unset( $_COOKIE[$cookie_key] );
        \wc_setcookie($cookie_key, null, 0);
    }
    ...
    public function print_button_single($productId = null, $custom_settings = array()) {
        ...
        if ( !apply_filters('xstore_wishlist_print_single_product_button', true) ) return;
...
//		if ( count(self::$settings) < 1) {
        ...
        $href = apply_filters('xstore_wishlist_product_query_args', true) ? add_query_arg(($add_action?'add_to_wishlist':'remove_wishlist'), $productId, self::$wishlist_page) : self::$wishlist_page;

        $attributes_rendered[] = 'href="'.esc_url($href).'"';
        if ( apply_filters('xstore_wishlist_product_query_args', true) ) {
            $attributes_rendered[] = 'rel="nofollow noopener"';
        }
        ...
        $attributes_rendered = apply_filters('xstore_wishlist_product_query_atts', $attributes_rendered);
        if ( $settings['is_single']) echo '<div class="single-wishlist">';
        ?>
        <a <?php echo implode(' ', $attributes_rendered); ?>>
    ...
    public function check_wishlist_item_validity() {
        ...
        foreach ( self::$products as $product_info) {
            ...
            if ( ! $product || ! $product->exists() || 'trash' === $product->get_status() ) {
                $return['errors'][] = new \WP_Error( 'invalid', sprintf( __( 'Sorry, %s is no longer available and was removed from your wishlist.', 'xstore-core' ), is_object($product ? '"'.$product->get_name().'"' : esc_html__('One of your products', 'xstore-core')) ) );
        ...
        if ( count($products_ids_to_reset) > 0) {
            ...
            $cookie_key = $this->get_cookie_key();
//            unset( $_COOKIE[$cookie_key] );
            \wc_setcookie($cookie_key, implode('|', $products_to_leave), time() + ($this->get_days_cache() * WEEK_IN_SECONDS));
//            $_COOKIE[ $cookie_key ] = implode('|', $products_to_leave);
    ...
    public function ghost_wishlist_page() {
        ...
        // compatibility with Elementor Account page widget
        add_filter('etheme_elementor_account_page_shortcode', '__return_true');
...
//        $_COOKIE[ $cookie_key ] = implode('|', $ready_products);
    ...
    public function register_admin_page(){
        ...
        $hook = add_menu_page(
            esc_html__('Wishlist', 'xstore-core'),
            esc_html__('Wishlist', 'xstore-core'),
            'edit_posts',
            'et-wishlists',
...
//        add_action( 'current_screen',array($this, 'add_help_tab'));
    ...
    public function add_help_tab() {
        ...
        $screen->add_help_tab( array(
            'id'    => 'et_wishlist_help_tab',
            'title' => esc_html__('Information', 'xstore-core'),
            'content'   => '<p>' . __( 'Here you can find all list of wishlist request from customers.', 'xstore-core' ) . '</p>',
        ) );
    }
