...
//		$_chosen_attributes = \WC_Query::get_layered_nav_chosen_attributes();
		$check_empty_tax    = isset($_GET['filter_brand']) && apply_filters('et_swatch_filter_check_empty_tax', false);
		...
		if ( in_array( $display_type, array( 'dropdown', 'select2' ) ) ) {
			if ( $display_type == 'select2' ) {
                ...
                wp_add_inline_script(apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general'), "
		            jQuery(document).ready(function ($) { jQuery( '.etheme_widget_cats_filter .dropdown_product_cats' ).select2();
			...
			if ( ! etheme_get_option( 'ajax_product_filter', 0 ) ) {
                wp_add_inline_script(apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general'), "
                    jQuery(document).ready(function ($) { jQuery( '.dropdown_product_cats' ).change( function() {
		...
		$categories = get_terms(apply_filters( 'etheme_widget_product_categories_query', array(
			'taxonomy' => 'product_cat',
		...
		if ( count( $categories ) > 0 ) {
			...
			$ignore_wc_term_cache = apply_filters('et_widget_cat_filter_ignore_wc_term_cache', false);

			if (!$ignore_wc_term_cache) {
			...
			foreach ( $categories as $category ) {
				...
				if ( !$ignore_wc_term_cache && ! array_key_exists( $category->term_id, $term_counts ) ) {
					continue;
				}
...
//					}
				...
				if ( in_array( $display_type, array( 'dropdown', 'select2' ) ) ) {
					...
					if ($check_empty_tax){
						$stock = $this->check_woocommerce_url_for_products($link);
					}
...
//		$cache = false;
	...
	public function check_woocommerce_url_for_products( $url ) {
	    $query_args = wp_parse_args( parse_url( $url, PHP_URL_QUERY ) );
	    ...
	    foreach ( $query_args as $key => $raw_value ) {
	        if ( strpos( $key, 'filter_' ) !== 0 || $raw_value === '' ) {
	            continue;
	        }
	        ...
	        $terms = array_filter( array_map( 'sanitize_title', explode( ',', wc_clean( wp_unslash( $raw_value ) ) ) ) );
	        if ( empty( $terms ) ) {
	            continue;
	        }

	        $qtype_key = 'query_type_' . $taxonomy;
	        $qtype     = isset( $query_args[ $qtype_key ] ) ? strtolower( wc_clean( wp_unslash( $query_args[ $qtype_key ] ) ) ) : 'or';
	        $operator  = ( $qtype === 'and' ) ? 'AND' : 'IN';
	    ...
	    if ( ! empty( $query_args['s'] ) ) {
	        $args['s'] = sanitize_text_field( wp_unslash( $query_args['s'] ) );
	    }
