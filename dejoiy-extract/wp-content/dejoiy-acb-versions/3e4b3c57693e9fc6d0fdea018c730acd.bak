...
/** @var int Total characters (D + 10 alphanumeric). */
define( 'DEJOIY_DPIN_LENGTH', 11 );

/** @var string Fixed prefix letter. */
define( 'DEJOIY_DPIN_PREFIX', 'D' );

/** @var int Products migrated per HTTP request (batched). */
define( 'DEJOIY_DPIN_MIGRATE_BATCH', 50 );

/** @var int Batches per request during v5 migration. */
define( 'DEJOIY_DPIN_MIGRATE_LOOPS', 6 );
...
function dejoiy_dpin_is_valid_format( $dpin ) {
	...
	return (bool) preg_match( '/^' . preg_quote( DEJOIY_DPIN_PREFIX, '/' ) . '[A-Z0-9]{10}$/', $dpin );
}
...
function dejoiy_dpin_normalize( $raw ) {
	$dpin = strtoupper( preg_replace( '/[^A-Z0-9]/', '', (string) $raw ) );
	if ( '' === $dpin ) {
		return '';
	}
	if ( DEJOIY_DPIN_PREFIX !== $dpin[0] ) {
		$dpin = DEJOIY_DPIN_PREFIX . ltrim( $dpin, DEJOIY_DPIN_PREFIX );
	}
...
/**
 * Generate a unique DPIN (D + 10 alphanumeric).
 *
 * @return string
 */
...
function dejoiy_get_product_dpin( $product_id ) {
	...
	if ( dejoiy_dpin_is_valid_format( $stored ) && DEJOIY_DPIN_PREFIX === $stored[0] ) {
		return $stored;
	}
	return '';
}
...
function dejoiy_ensure_product_dpin( $product_id ) {
	...
	$raw = strtoupper( trim( (string) get_post_meta( $product_id, dejoiy_dpin_meta_key(), true ) ) );
	if ( '' !== $raw && ! dejoiy_dpin_is_valid_format( $raw ) ) {
...
/**
 * DPIN for UI (cards, home shelves) — valid D-prefix, migrates legacy meta when needed.
 *
 * @param int $product_id Product ID.
 * @return string
 */
function dejoiy_display_product_dpin( $product_id ) {
	$product_id = (int) $product_id;
	if ( $product_id < 1 ) {
		return '';
	}
	...
	if (
		function_exists( 'dejoiy_marketplace_ensure_valid_dpin' )
		&& function_exists( 'dejoiy_marketplace_product_uses_pretty_urls' )
		&& dejoiy_marketplace_product_uses_pretty_urls( $product_id )
	) {
		$dpin = dejoiy_marketplace_ensure_valid_dpin( $product_id );
		if ( $dpin && DEJOIY_DPIN_PREFIX === $dpin[0] ) {
			return $dpin;
		}
	}
 ...
 * @return bool
 */
function dejoiy_dpin_product_needs_migrate( $product_id ) {
	$product_id = (int) $product_id;
	if ( $product_id < 1 ) {
		return false;
	}
	...
	return DEJOIY_DPIN_PREFIX !== $stored[0];
}

/**
 * Marketplace catalog product (excludes Nexus / QuickMart-only ecosystems).
 *
 * @param int $product_id Product ID.
 * @return bool
 */
function dejoiy_dpin_is_marketplace_catalog_product( $product_id ) {
	if ( ! function_exists( 'dejoiy_get_product_ecosystem' ) ) {
		return true;
	}
	$eco = dejoiy_get_product_ecosystem( (int) $product_id );
	return in_array( $eco, array( 'marketplace', 'business' ), true );
}

/**
 * One batch: assign D-prefix DPINs where missing or legacy.
 *
 * @return string complete|continue
 */
function dejoiy_dpin_migrate_invalid_format_batch_once() {
	$offset = (int) get_option( 'dejoiy_dpin_format_v5_offset', 0 );
	$q      = new WP_Query(
		array(
			'post_type'              => 'product',
			'post_status'            => array( 'publish', 'private', 'draft', 'pending' ),
			'posts_per_page'         => DEJOIY_DPIN_MIGRATE_BATCH,
			'offset'                 => $offset,
			'fields'                 => 'ids',
			'orderby'                => 'ID',
			'order'                  => 'ASC',
	...
	if ( empty( $q->posts ) ) {
		update_option( 'dejoiy_dpin_format_v5_all_done', '1', false );
		delete_option( 'dejoiy_dpin_format_v5_offset' );
		return 'complete';
	}

	foreach ( $q->posts as $pid ) {
		$pid = (int) $pid;
		if ( dejoiy_dpin_product_needs_migrate( $pid ) ) {
			dejoiy_dpin_maybe_assign( $pid, true );
		}
	}

	update_option( 'dejoiy_dpin_format_v5_offset', $offset + DEJOIY_DPIN_MIGRATE_BATCH, false );
	return 'continue';
}

/**
 * Migrate all marketplace catalog products to D + 10 alphanumeric DPINs.
 */
function dejoiy_dpin_migrate_marketplace_catalog_batch() {
	if ( get_option( 'dejoiy_dpin_format_v5_all_done' ) ) {
		return;
	}

	for ( $i = 0; $i < DEJOIY_DPIN_MIGRATE_LOOPS; $i++ ) {
		if ( 'complete' === dejoiy_dpin_migrate_invalid_format_batch_once() ) {
			break;
		}
	}
}
add_action( 'init', 'dejoiy_dpin_migrate_marketplace_catalog_batch', 96 );
...
function dejoiy_dpin_maybe_assign( $product_id, $force = false ) {
	...
	if ( ! $force && dejoiy_dpin_is_valid_format( $current ) && DEJOIY_DPIN_PREFIX === $current[0] ) {
		return;
	}
...
/**
 * Migrate stored DPINs to D + 10 alphanumeric format.
 */
function dejoiy_dpin_migrate_d_prefix_batch() {
	if ( get_option( 'dejoiy_dpin_format_v3_done' ) ) {
		return;
	}
	$q = new WP_Query(
		array(
	...
	if ( empty( $q->posts ) ) {
		update_option( 'dejoiy_dpin_format_v3_done', '1' );
		return;
	}
	if ( 0 === $migrated ) {
		update_option( 'dejoiy_dpin_format_v3_done', '1' );
...
add_action( 'init', 'dejoiy_dpin_migrate_d_prefix_batch', 97 );
...
function dejoiy_dpin_resolve_product_id( $dpin, $exact_only = false ) {
	$raw = strtoupper( preg_replace( '/[^A-Z0-9]/', '', (string) $dpin ) );
	if ( strlen( $raw ) < DEJOIY_DPIN_LENGTH ) {
		return 0;
	}
	if ( strlen( $raw ) > DEJOIY_DPIN_LENGTH ) {
		$raw = substr( $raw, 0, DEJOIY_DPIN_LENGTH );
	}

	$pid = dejoiy_dpin_lookup_product_by_meta_value( $raw );
	if ( $pid > 0 ) {
		return $pid;
	}

	$normalized = dejoiy_dpin_normalize( $raw );
	if ( $normalized && $normalized !== $raw ) {
		$pid = dejoiy_dpin_lookup_product_by_meta_value( $normalized );
		if ( $pid > 0 ) {
			return $pid;
		}
	}

	if ( ! dejoiy_dpin_is_valid_format( $normalized ) ) {
		return 0;
	}

	return dejoiy_dpin_lookup_product_by_meta_value( $normalized );
}

/**
 * @param string $meta_value Stored DPIN meta (exact match).
 * @return int
 */
function dejoiy_dpin_lookup_product_by_meta_value( $meta_value ) {
	$meta_value = strtoupper( trim( (string) $meta_value ) );
	if ( '' === $meta_value ) {
		return 0;
	}
	$ids = get_posts(
...
function dejoiy_dpin_product_url( $product_id, $eco = '' ) {
	...
	if ( 'marketplace' === $eco || '' === $eco ) {
		if ( function_exists( 'dejoiy_marketplace_product_url' ) ) {
...
function dejoiy_dpin_single_product_display() {
	...
	$dpin = dejoiy_display_product_dpin( $product->get_id() );
	if ( '' === $dpin ) {
		return;
	}
