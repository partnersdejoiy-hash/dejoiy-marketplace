		...
		<?php if ( ! empty( $childs ) ) : ?>
			<?php foreach ( $childs as $term ) : ?>
				...
				if ( $show_count and $count == 0 and ! in_array( $term['slug'], $current_request ) ) :
					?>
					disabled=""<?php endif; ?> value="<?php echo esc_attr( $term['slug'] ); ?>" <?php selected( in_array( $term['slug'], $current_request ) ); ?> class="woof-padding-<?php echo esc_attr( $level ); ?>"><?php /* echo str_repeat('&nbsp;&nbsp;&nbsp;', esc_attr($level)) */ ?>
				<?php
...
?>
	<?php if ( ! empty( $terms ) ) : ?>
		<?php foreach ( $terms as $term ) : ?>
			...
			if ( $show_count and $count == 0 and ! in_array( $term['slug'], $current_request ) ) :
				?>
				disabled=""<?php endif; ?> value="<?php echo esc_attr( $term['slug'] ); ?>" <?php selected( in_array( $term['slug'], $current_request ) ); ?>>
			<?php
			if ( has_filter( 'woof_before_term_name' ) ) {
				echo wp_kses_post( wp_unslash( apply_filters( 'woof_before_term_name', $term, $taxonomy_info ) ) );
			} else {
