...
class WCFMmp_Frontend {
	...
	public function __construct() {
		...
		if( function_exists( 'rehub_option' ) ) {
			...
			} elseif( $vendor_sold_by_position == 'bellow_atc' ) {
				add_action( 'rhwoo_template_single_add_to_cart',array( &$this, 'wcfmmp_sold_by_single_product' ), 25 );
			} else {
	...
	public function wcfmmp_sold_by_cart( $cart_item_meta = array(), $cart_item = array() ) {
		global $WCFM, $WCFMmp;
	...
	function wcfmmp_checkout_user_location_save( $order_id ) {
		if( apply_filters( 'wcfmmp_is_allow_checkout_user_location', true ) ) {
			...
			if ( ! empty( $_POST['wcfmmp_user_location'] ) ) {
				$order->update_meta_data('_wcfmmp_user_location',sanitize_text_field( $_POST['wcfmmp_user_location'] ));
			}
			if ( ! empty( $_POST['wcfmmp_user_location_lat'] ) ) {
				$order->update_meta_data('_wcfmmp_user_location_lat',sanitize_text_field( $_POST['wcfmmp_user_location_lat'] ));
			}
			if ( ! empty( $_POST['wcfmmp_user_location_lng'] ) ) {
				$order->update_meta_data('_wcfmmp_user_location_lng',sanitize_text_field( $_POST['wcfmmp_user_location_lng'] ));
			}
	...
	function wcfmmp_order_details_user_location_show( $order ) {
		if( apply_filters( 'wcfmmp_is_allow_checkout_user_location', true ) ) {
			$address = $order->get_meta('_wcfmmp_user_location');
			$lat     = $order->get_meta('_wcfmmp_user_location_lat');
			$lng     = $order->get_meta('_wcfmmp_user_location_lng');
			if( $address ) {
	...
	function wcfmmp_product_list_geo_location_filter_post_clauses( $args, $wp_query ) {
		...
		if ( $wp_query->is_main_query() && isset( $_GET['filter_vendor'] ) && !empty( $_GET['filter_vendor'] ) ) {
			$args['where'] .= $wpdb->prepare(" AND $wpdb->posts.post_author = %d", absint($_GET['filter_vendor']));
		}
		...
		if( !empty( $radius_lat ) && !empty( $radius_lng ) && !empty( $radius_range ) ) {
			...
			$available_vendors_placeholders = implode(', ', array_fill(0, count($available_vendors), '%d'));

			$args['where'] .= $wpdb->prepare(" AND $wpdb->posts.post_author in ({$available_vendors_placeholders})", $available_vendors);
		}
