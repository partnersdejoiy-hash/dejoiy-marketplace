...
class Gallery extends \Elementor\Widget_Base {
	...
	public function get_script_depends() {
        return array_merge(['elementor-gallery'], apply_filters('etheme_elementor_widget_script_depends', [ 'etheme_gallery' ]));
	}
	...
	public function get_style_depends() {
		return array_merge(['elementor-gallery'], apply_filters('etheme_elementor_widget_style_depends', [ 'etheme-elementor-gallery' ]));
	}
...
//				],
	...
	protected function render() {
		...
		if ( (!$is_multiple && !count($settings['gallery'])) || ($is_multiple && (!count($settings['galleries']) || count($settings['galleries']) == 1 && !count($settings['galleries'][0]['multiple_gallery']) )) ) {
            echo Elementor::elementor_frontend_alert_message(esc_html__('Please, add images to create gallery', 'xstore-core'), 'elementor-panel-alert-warning');
            return;
        }
		?>
