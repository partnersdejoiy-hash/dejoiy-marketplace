        ...
        <div class="categories-mask <?php echo esc_attr($mask_classes); ?>">
			...
			if ( (isset($styles['hide_all']) && !$styles['hide_all']) || count($styles) < 1 ) :
				...
				<?php if ( empty($sorting[0]) || in_array('name', $sorting) ) {
                    ...
                    <<?php echo apply_filters('etheme_product_category_title_tag', 'h4'); ?>><?php echo esc_html($category->name); ?><?php if ( $styles_default['count_label'] && $category->count > 0 ) { echo ' <sup>('.$category->count.')</sup>'; } ?></<?php echo apply_filters('etheme_product_category_title_tag', 'h4'); ?>>
