	...
	$('#withdrawal_payment_methods').find('.payment_options').each(function() {
		$(this).change(function() {
			...
			if( $(this).is(':checked') ) {
				...
				// if the unchecked checkbox is stripe/stripe_split
				if( ['stripe', 'stripe_split'].includes( $payment_option ) ) {
					// if any of the stripe/stripe_split chekbox is active
					if( $( '.payment_options[value^="stripe"]:checked' ).length > 0 ) {
						if ('stripe_split' === $payment_option) {
							// hide all stripe_split inputs except stripe inputs
							$('.withdrawal_mode_stripe_split:not(.withdrawal_mode_stripe)').addClass('wcfm_ele_hide');
						} else if ('stripe' === $payment_option) {
							// hide all stripe inputs except stripe_split inputs
							$('.withdrawal_mode_stripe:not(.withdrawal_mode_stripe_split)').addClass('wcfm_ele_hide');
						} 
					} else {
			...
			// hides Enable test mode if no payment method selected
			if ($('#withdrawal_payment_methods .payment_options:checked').length > 0) {
				$('#withdrawal_test_mode, .withdrawal_test_mode').removeClass('wcfm_ele_hide');
			} else {
				$('#withdrawal_test_mode, .withdrawal_test_mode').addClass('wcfm_ele_hide');
			}
		}).change();
	});
