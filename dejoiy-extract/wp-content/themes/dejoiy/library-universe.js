...
(function () {
  ...
  function joiAjaxPost(action, fields) {
    var fd = new FormData();
    ...
    return fetch(CFG.ajax || '/wp-admin/admin-ajax.php', {
      method: 'POST',
  ...
  function renderJoiBooks(books, homeUrl, wrap) {
    if (!wrap) return;
    ...
    var html = '<div class="dlu-joi-books">';
    books.forEach(function (b) {
      html += '<a href="' + (b.url || '#') + '"><img src="' + (b.cover || '') + '" alt="" loading="lazy" /><span>' + escapeHtml(decodeEntities(b.title || ''));
    ...
    html += '</div>';
    if (homeUrl) {
      html += '<p class="dlu-joi-more"><a href="' + homeUrl + '" class="dlu-btn-secondary">View all on Nexus</a></p>';
    }
    wrap.innerHTML = html;
    wrap.hidden = false;
  }
  ...
  function linkifyBookTitles(text, books) {
    var html = escapeHtml(text);
    if (!books || !books.length) {
      return html;
    }
    books.slice(0, 8).forEach(function (book) {
      if (!book || !book.title || !book.url) return;
      var title = String(book.title).replace(/&#(\d+);/g, function (_, n) {
      ...
      html = html.replace(
        ...
        '<a class="dlu-joi-inline-book" href="' + escapeHtml(book.url) + '">$&</a>'
      );
    });
    return html;
  }

  function addJoiBubble(chat, role, text, books) {
    if (!chat) return null;
    if (!text && (!books || !books.length)) return null;
    if (!text && books && books.length) {
      text = 'Here are books from the Nexus catalog that match your search:';
    }
    var bubble = document.createElement('div');
    bubble.className = 'dlu-joi-bubble dlu-joi-bubble--' + role;
    var who = document.createElement('strong');
    who.textContent = role === 'user' ? 'You' : 'JOI';
    var body = document.createElement('div');
    body.className = 'dlu-joi-bubble__body';
    if (role === 'joi' && books && books.length) {
      body.innerHTML = linkifyBookTitles(text, books);
      var list = document.createElement('div');
      list.className = 'dlu-joi-bubble__books';
      var kicker = document.createElement('span');
      kicker.className = 'dlu-joi-bubble__books-kicker';
      kicker.textContent = 'Open in Nexus';
      list.appendChild(kicker);
      books.slice(0, 6).forEach(function (book) {
        if (!book || !book.url) return;
        var a = document.createElement('a');
        a.className = 'dlu-joi-bubble__book-link';
        a.href = book.url;
        var title = document.createElement('span');
        title.className = 'dlu-joi-bubble__book-title';
        title.textContent = decodeEntities(book.title) || 'Book';
        a.appendChild(title);
        if (book.author) {
  ...
  function collectJoiContext(chatEl, excludeText) {
    if (!chatEl) return '';
    var lines = [];
    chatEl.querySelectorAll('.dlu-joi-bubble--user .dlu-joi-bubble__body').forEach(function (node) {
      var text = (node.textContent || '').trim();
      if (text.length >= 2 && text !== excludeText) {
        lines.push(text);
      }
    });
  ...
  /* ── JOI Librarian — books fast + OpenAI chat (persistent history) ── */
  function initJoi() {
    ...
    var chat = document.getElementById('dlu-joi-chat');
    var booksWrap = document.getElementById('dlu-joi-books-wrap');
    var closeBtn = document.getElementById('dlu-joi-close');
    if (!input || !btn || !out) return;

    var busy = false;
    var runId = 0;
    var lastSubmitted = '';
    var CHAT_KEY = 'dlu_joi_chat_v1';

    function saveChat() {
      if (!chat) return;
      try {
        sessionStorage.setItem(CHAT_KEY, chat.innerHTML);
      } catch (err) {
        /* ignore */
      }
    }
    ...
    function restoreChat() {
      if (!chat) return;
      try {
        var html = sessionStorage.getItem(CHAT_KEY);
        if (html) {
          chat.innerHTML = html;
          out.hidden = false;
        }
    ...
    function showBooks(books, homeUrl) {
      if (!books || !books.length) {
        return;
      }
      renderJoiBooks(books, homeUrl || '', booksWrap);
    }

    function closeJoiChat() {
      out.hidden = true;
      if (booksWrap) {
        booksWrap.hidden = true;
        booksWrap.innerHTML = '';
      }
    }

    function previewBooks(q) {
      if (!CFG.ajax || q.length < 2) {
        if (booksWrap) {
          booksWrap.hidden = true;
          booksWrap.innerHTML = '';
        }
      ...
      var ctx = collectJoiContext(chat, q);
      joiAjaxPost('dejoiy_library_nexus_search', { q: q, books_only: '1', context: ctx })
        .then(function (res) {
          if (input.value.trim() !== q || busy) return;
          if (res && res.success && res.data) {
            showBooks(res.data.books || [], res.data.home || '');
          }
        })
        .catch(function () { /* optional preview */ });
    }
    ...
    function run() {
      ...
      btn.disabled = true;
      out.hidden = false;
      var context = collectJoiContext(chat, q);
      addJoiBubble(chat, 'user', q);
      clearInputAfterSend();
      lastSubmitted = '';
      saveChat();
      var thinking = addJoiBubble(chat, 'joi', 'JOI is thinking…');
      var foundBooks = [];
      var foundHome = '';

      function finishJoiReply(replyText, books, homeUrl) {
        if (id !== runId) return;
        btn.disabled = false;
        busy = false;
        ...
        var list = (books && books.length) ? books : foundBooks;
        var home = homeUrl || foundHome || '';
        if (!replyText && list.length) {
          replyText = 'I found ' + list.length + ' book' + (list.length === 1 ? '' : 's') + ' in Nexus — tap a title below to open it.';
        }
        addJoiBubble(chat, 'joi', replyText || 'JOI is unavailable right now. Try the results page.', list);
        showBooks(list, home);
        saveChat();
      }

      joiAjaxPost('dejoiy_library_nexus_search', { q: q, books_only: '1', context: context })
        .then(function (res) {
          if (id !== runId) return;
          if (res && res.success && res.data) {
            foundBooks = res.data.books || [];
            foundHome = res.data.home || '';
            showBooks(foundBooks, foundHome);
          }
        })
        .catch(function () { /* books optional */ });

      joiAjaxPost('dejoiy_joi_librarian_chat', { message: q, context: context, nonce: CFG.nonce || '' })
        .then(function (res) {
          if (id !== runId) return;
          if (!res || !res.success || !res.data) {
            if (foundBooks.length) {
              finishJoiReply('', foundBooks, foundHome);
              return;
            }
            finishJoiReply('JOI is unavailable right now. Try the results page.', [], '');
            return;
          }
          var books = (res.data.books && res.data.books.length) ? res.data.books : foundBooks;
          finishJoiReply(res.data.reply || res.data.message || '', books, res.data.home || foundHome);
        })
        .catch(function () {
          if (id !== runId) return;
          if (foundBooks.length) {
          ...
          finishJoiReply('Search failed. Try the results page or ask again.', [], '');
        });
    }
