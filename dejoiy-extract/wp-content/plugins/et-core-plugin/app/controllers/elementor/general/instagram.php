...
class Instagram extends \Elementor\Widget_Base {
	...
	protected function register_controls() {
        ...
        $this->add_control(
            'img_source',
            [
                'label' 		=>	__( 'Images Source', 'xstore-core' ),
                'type' 			=>	\Elementor\Controls_Manager::SELECT,
                'options' 		=>	[
                    'instagram'  	 =>	esc_html__( 'Instagram', 'xstore-core' ),
                    'gallery'	 =>	esc_html__( 'Images gallery', 'xstore-core' ),
                ],
                'default' => 'instagram'
            ]
        );
		...
		$this->add_control(
            'show_likes_count',
            [
                'label' 		=>	__( 'Show likes count', 'xstore-core' ),
        ...
        $this->add_control(
            'show_comments_count',
            [
                'label' 		=>	__( 'Show comments count', 'xstore-core' ),
                'type' 			=>	\Elementor\Controls_Manager::SWITCHER,
                'return_value'  =>	'yes',
                'default' 		=>	'yes',
            ]
		...
		$this->add_control(
			'hide_empty_meta',
			[
				'label' 		=>	__( 'Hide empty meta', 'xstore-core' ),
				'type' 			=>	\Elementor\Controls_Manager::SWITCHER,
				'return_value'  =>	'true',
				'default' 		=>	'',
				'description' => esc_html__('Hide count of likes and comments if they empty or equal 0. Work for each post separately', 'xstore-core'),
                'condition' => [
                    'img_source' => 'instagram',
                ],
			]
        ...
        $this->start_controls_section(
            'gallery_settings',
            [
                'label' => __( 'Images Gallery', 'xstore-core' ),
                'tab' 	=> \Elementor\Controls_Manager::TAB_CONTENT,
                'condition' => [
                    'img_source' => 'gallery',
                ],
        ...
        $this->add_control(
            'images_link',
            [
                'label' => __( 'Images link', 'xstore-core' ),
                'type' 	=> \Elementor\Controls_Manager::TEXT,
            ]
        );
        ...
        $this->add_control(
            'likes_count',
            [
                'label' => __( 'Likes count', 'xstore-core' ),
                'type' 	=> \Elementor\Controls_Manager::TEXT,
                'default' => '0-100',
                'placeholder' => '0-2000',
                'condition' => [
        ...
        $this->add_control(
            'comments_count',
            [
                'label' => __( 'Comments count', 'xstore-core' ),
                'type' 	=> \Elementor\Controls_Manager::TEXT,
                'default' => '1-20',
                'placeholder' => '0-1000',
                'condition' => [
	...
	protected function render() {
		...
		if( $settings['img_source'] == 'gallery' && is_array($settings['gallery'])){
			foreach ( $settings['gallery'] as $img){
				$gallery_isd[] = $img['id'];
			}
		}

		$gallery_isd = implode(',', $gallery_isd );
		...
		echo do_shortcode( '[instagram 
			...
			gallery="'. $gallery_isd .'"
			images_link="'. $settings['images_link'] .'"
			type="'. $settings['type'] .'"
			columns="'. $settings['columns'] .'"
			like_count="'. $settings['show_likes_count'] .'"
			likes_custom_count="'. $settings['likes_count'] .'"
			comments_count="'. $settings['show_comments_count'] .'"
			comments_custom_count="'. $settings['comments_count'] .'"
			hide_empty_meta="'.!!$settings['hide_empty_meta'].'"
			target="'. $settings['target'] .'"
			link="'. $settings['link'] .'"
			slider="'. $settings['slider'] .'"
			spacing="'. $settings['spacing'] .'"
			ajax="'. ((\Elementor\Plugin::$instance->editor->is_edit_mode() || \Elementor\Plugin::$instance->preview->is_preview_mode()) ? false : $settings['ajax']) .'"
