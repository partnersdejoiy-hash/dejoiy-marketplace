...
if ( ! function_exists( 'etheme_enqueue_scripts' ) ) {
	function etheme_enqueue_scripts() {
		if ( ! is_admin() && (apply_filters('etheme_enable_load_scripts_on_ajax', false) || !(isset( $_GET['et_ajax'] ) && $_GET['et_ajax'])) ) {
...
//			$is_masonry = etheme_masonry();
			...
			$load_wc_cart_fragments = etheme_get_option( 'load_wc_cart_fragments_upd', 0 );
...
//			$is_product = $is_woocommerce && is_product();
			...
			foreach (etheme_config_js_files() as $script){
                ...
                if ( isset($script['localize_package']) ) {
                    foreach ($script['localize_package'] as $localize_package) {
                        wp_localize_script( $script['name'], $localize_package['name'], $localize_package['params'] );
                    }
                }
			}
			...
			if ( etheme_get_option('flying_pages_upd', false) ) {
				...
				elseif ( function_exists( 'run_litespeed_cache' ) || defined('WP_ROCKET_VERSION') ) {
					...
					wp_enqueue_script( 'flying_pages_upd' );
				}
			}
			if (get_query_var('et_is-woocommerce', false)) {
				...
				if ( get_query_var('et_is-cart-checkout-advanced', false) && !(get_query_var('et_is-cart-page-elementor-shortcode', false) || get_query_var('et_is-checkout-page-elementor-shortcode', false))) {
//					if ( get_query_var('et_cart-checkout-layout', 'default') != 'default')
                    if (get_theme_mod('cart_checkout_advanced_sticky_sidebar', true))
                    ...
                    if (get_theme_mod('cart_checkout_order_product_quantity', false))
				...
				if ( get_query_var('et_widgets-show-more', false) ) {
 				...
 				if ( get_query_var('et_is-woocommerce-archive', false) ) {
				    ...
				    if ( get_query_var('et_etspt', false) ) {
					    wp_enqueue_script( 'et_etspt' );
				    }
			    }
			}
            ...
            wp_enqueue_script(apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general'));

            if (apply_filters('etheme_should_enqueue_global_script', false, 'etheme_elementor_all_widgets_js')) {
                wp_enqueue_script('etheme_elementor_all_widgets_js');
            }

            if (get_query_var('et_is-woocommerce', false) && apply_filters('etheme_should_enqueue_global_script', false, 'etheme_woocommerce_all_scripts')) {
                wp_enqueue_script('etheme_woocommerce_all_scripts');
            }
			...
			$etGlobalConf = array(
				...
				'woocommerceSettings'     => array(
					...
					'cart_url'                => false,
					'cart_redirect_after_add' => apply_filters('etheme_cart_redirect_after_add', get_option( 'woocommerce_cart_redirect_after_add' ) == 'yes'),
				),
				'notices'                 => array(
	...
	C100.2924805,97.9419327,100.2924805,96.0305862,99.1186676,94.8567734z"></path></svg>',
                ...
                'successfullyRemoved'       => esc_html__( 'Product removed.', 'xstore' ),
                'successfullyUpdated'       => esc_html__( 'Product quantity updated.', 'xstore' ),
                ...
                'viewCart' => esc_html__('View cart', 'xstore'),
				'cartPageUrl' => $cartUrl,
				...
				'speedOptimization' => array(
					'imageLoadingOffset' => get_query_var('et_img-loading-type', 'lazy') != 'default' ? get_theme_mod('images_loading_offset_et-desktop', 200) . 'px' : '200px'
				),
			...
			if ( get_query_var('et_is-woocommerce', false) && current_theme_supports( 'woocommerce' ) ) {
                ...
                $etGlobalConf['woocommerceSettings']['ajax_add_to_cart'] = apply_filters('etheme_ajax_add_to_cart', true) ? etheme_get_option( 'ajax_add_to_cart', 1 ) : false;
				if ( get_query_var('et_is-woocommerce-archive', false) ) {
					$etGlobalConf['woocommerceSettings']['ajax_filters']    = etheme_get_option( 'ajax_product_filter', 0 );
					$etGlobalConf['woocommerceSettings']['ajax_pagination'] = etheme_get_option( 'shop_page_pagination_type_et-desktop', '' );
					$etGlobalConf['woocommerceSettings']['ajax_pagination_mobile'] = etheme_get_option( 'shop_page_pagination_type_et-mobile', '' );
					$etGlobalConf['woocommerceSettings']['no_more_products'] = __('No more products available...', 'xstore');
					$etGlobalConf['woocommerceSettings']['load_more_products'] = __('Load More Products', 'xstore');
                    $etGlobalConf['woocommerceSettings']['sidebar_widgets_dropdown_limited'] = get_query_var('et_widgets-show-more', false);
					...
					$etGlobalConf['woocommerceSettings']['wishlist_for_variations'] = etheme_get_option('wishlist_for_variations_new',0);
				}

                if ( get_query_var('is_single_product', false) ) {
                    $etGlobalConf['woocommerceSettings']['single_product_autoscroll_tabs_mobile'] = apply_filters('etheme_autoscroll_single_tabs_on_mobile', true);
                    if ( $etConf['variationGallery'] ) {
                        $etGlobalConf['woocommerceSettings']['variationGalleryKeepVideo'] = apply_filters('etheme_single_product_variation_gallery_video', true);
                        $etGlobalConf['woocommerceSettings']['product_video_position'] = etheme_get_option('product_video_position', 'last');
                    }
                }
				...
				$etGlobalConf['woocommerceSettings']['is_smart_addtocart'] = get_query_var('et_product-with-quantity', 0);
				...
				if ( etheme_get_option('sidebar_for_mobile', 'off_canvas') == 'off_canvas' ) {
					...
					if ( function_exists('etheme_fgcontent') && $sidebar_off_canvas_icon != '' ) { // Legal request to self hosted file
						$type      = get_post_mime_type( $sidebar_off_canvas_icon );
						$mime_type = explode( '/', $type );
						if ( isset($mime_type['1']) && $mime_type['1'] == 'svg+xml' ) {
							...
							if ( ! empty( $svg ) ) {
								...
								if ( $attachment_file ) {
									...
									$svg = etheme_fgcontent( $attachment_file , false, null); // Legal request to self hosted file
			...
			if ( $etGlobalConf['woocommerceSettings']['ajax_filters'] || $etGlobalConf['woocommerceSettings']['ajax_pagination'] ) {
				$etAjaxFiltersConfig = array(
					...
					'builder' => '',
                    'offcanvas_autoclose' => apply_filters('etheme_off_canvas_autoclose_ajax_filters_action', true),
				);

				if ( is_tax('dc_vendor_shop') || (function_exists('wcfm_is_store_page') && wcfm_is_store_page()) || ( function_exists('wcfmmp_is_stores_list_page') && wcfmmp_is_stores_list_page()) ) {
                    $etAjaxFiltersConfig['ajax_categories'] = false;
                }
				...
				if ( get_query_var('view_mode_smart', false) || isset($_GET['view_mode_smart']) ) {
				...
				wp_enqueue_script( 'ajaxFilters', get_template_directory_uri() . '/js/ajax-filters.min.js',array(apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general')), $theme->version, true );
				wp_localize_script( 'ajaxFilters', 'etAjaxFiltersConfig', $etAjaxFiltersConfig );
				
			}

			wp_localize_script( apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general'), 'etConfig', apply_filters( 'etheme_et_js_config', $etConf) );
...
if ( !function_exists('etheme_config_js_files')) {
	function etheme_config_js_files() {
        global $etheme_js_config;
        if ( !!!$etheme_js_config )
            $etheme_js_config = include get_template_directory() . '/framework/config-js.php';
        return $etheme_js_config;
	}
}
