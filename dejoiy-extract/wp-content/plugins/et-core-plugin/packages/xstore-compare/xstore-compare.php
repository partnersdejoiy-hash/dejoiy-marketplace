...
//        add_action( 'wp_ajax_nopriv_xstore_compare_page_action', array( $this, 'global_page_actions' ) );
        ...
        add_action( 'wp_ajax_xstore_get_origin_compare_product_variation_id', array( $this, 'get_origin_compare_product_variation_id' ) );
        add_action( 'wp_ajax_nopriv_xstore_get_origin_compare_product_variation_id', array( $this, 'get_origin_compare_product_variation_id' ) );
...
//		$this->define_settings();
	...
	public function define_settings() {
	    self::$settings = array(
            ...
            'add_text' => esc_html(get_theme_mod('xstore_compare_label_add_to_compare', esc_html__('Add to Compare', 'xstore-core'))),
            'remove_text' => esc_html(get_theme_mod('xstore_compare_label_browse_compare', esc_html__('Remove', 'xstore-core'))),
...
//                break;
            ...
            case 'custom':
                ...
                if ( $icon_custom != '' ) {
                    ...
                    add_filter('etheme_mobile_panel_element_compare_icon', function ($icon_svg, $icon_key) {
                        return 'et_icon-compare' == $icon_key ? self::$settings['custom_icon'] : $icon_svg;
                    }, 10, 2);
                }
                break;
        }
...
//                'animated_hearts' => self::$settings['animated_hearts'],
                'ajax_fragments' => apply_filters('xstore_compare_mini_content_ajax', (self::get_products_count()>=1)),
                'is_multilingual' => self::$is_multilingual,
                'view_compare' => esc_html__('View compare list', 'xstore-core'),
     ...
     */
    function header_compare_quantity($updated_count = false, $only_count = false) {
     ...
     */
    public function header_mini_compare($args = array()) {
        $args = wp_parse_args($args, array(
            'display_footer_buttons' => true,
            'show_view_page' => true,
            'show_add_all_products' => true
        ));
        if ( apply_filters('xstore_compare_mini_content_ajax', true) ) {
            $compare_class  = 'et_b_compare-dropdown product_list_widget cart_list';
            ?>
            <div class="<?php esc_attr_e( $compare_class ); ?>"></div>
            <?php
        }
        else
            $this->header_mini_compare_products();

        $this->header_mini_compare_footer($args);
    }


    public function header_mini_compare_footer($args = array()) {
    ...
    public function header_mini_compare_products($updated_products = false) {
        ...
        $limit = apply_filters('etheme_mini_compare_items_count', $limit);
                    ...
                    foreach ( $products as $product_info ) {
                        ...
                        <li class="woocommerce-mini-compare-item">
                            ...
                            <div class="product-item-right" data-row-id="<?php esc_attr_e( $origin_product_id ); ?>">
                                ...
                                <<?php echo apply_filters('etheme_woocommerce_mini_compare_product_title_tag', 'h4'); ?> class="product-title">
                                    <a href="<?php echo esc_url( $_product->get_permalink() ); ?>"><?php echo wp_specialchars_decode( $product_name ); ?></a>
                                </<?php echo apply_filters('etheme_woocommerce_mini_compare_product_title_tag', 'h4'); ?>>

                                <?php if ( $add_remove_ajax ) : ?>
                                    <a href="<?php echo add_query_arg( 'remove_from_compare', $origin_product_id, esc_url( self::$compare_page ) ); ?>"
                                       data-id="<?php echo esc_attr($origin_product_id); ?>"
                                       class="remove xstore-minicompare-remove remove_from_compare"
                                       title="<?php echo esc_attr( sprintf( __( 'Remove %s from compare', 'xstore-core' ), $product_name ) ); ?>"><i
                                                class="et-icon et-delete et-remove-type1"></i><i
                                                class="et-trash-wrap et-remove-type2"><img
                                                    src="<?php echo $trash_bin; ?>"
                                                    alt="<?php echo esc_attr( sprintf( __( 'Remove %s from compare', 'xstore-core' ), $product_name ) ); ?>"></i></a>
                                <?php endif; ?>
    ...
    public function fragments() {
        ...
        ob_start();
        $this->header_compare_quantity(isset($_POST['products_count']) ? $_POST['products_count'] : false, true);
        ...
        if ( isset($_POST['products_count'])) {
            ...
            $this->header_compare_quantity(false);
        }
        ...
        ob_start();
            if ( isset($_POST['products'])) {
                ...
                $this->header_mini_compare_products(false);
            }
        $data['fragments']['.et_b_compare-dropdown'] = ob_get_clean();

        $data = array(
			'fragments' => apply_filters(
				'xstore_compare_refresh_fragments',
				$data['fragments'],
				$product_count,
				$data['fragments']['.et_b_compare-dropdown']
			),
		);
     ...
     * @return mixed
     */
    public function get_origin_compare_product_variation_id() {
...
//    }
    ...
    public function add_display_post_states($post_states, $post) {
        if ( $post && (int)get_theme_mod('xstore_compare_page', '') === $post->ID ) {
            $post_states['xstore_page_for_compare'] = __( 'Compare page', 'xstore-core' );
        }
...
//        return ob_get_clean();
    ...
    public function print_button_single($productId = null, $custom_settings = array()) {
        ...
        if ( !apply_filters('xstore_compare_print_single_product_button', true) ) return;
...
//        }
        ...
        $href = apply_filters('xstore_compare_product_query_args', true) ? add_query_arg(($add_action?'add_to_compare':'remove_compare'), $productId, self::$compare_page) : self::$compare_page;

        $attributes_rendered[] = 'href="'.esc_url($href).'"';
        if ( apply_filters('xstore_compare_product_query_args', true) ) {
        ...
        $attributes_rendered = apply_filters('xstore_compare_product_query_atts', $attributes_rendered);
    ...
    public function check_compare_item_validity() {
        ...
        foreach ( self::$products as $product_info) {
            ...
            if ( ! $product || ! $product->exists() || 'trash' === $product->get_status() ) {
                $return['errors'][] = new \WP_Error( 'invalid', sprintf( __( 'Sorry, %s is no longer available and was removed from your compare.', 'xstore-core' ), is_object($product ? '"'.$product->get_name().'"' : esc_html__('One of your products', 'xstore-core')) ) );
...
//        $this->check_compare_items();
		...
		if ( count($products) < 1) {
            ...
            $compare_page_args['global_actions'] = apply_filters('xstore_compare_page_actions', $compare_page_args['global_actions']);
