...
class Billing_Details extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'Checkout Billing Details', 'xstore-core' );
	}
	...
	public function get_style_depends() {
        return apply_filters('etheme_elementor_widget_style_depends', [], true);
	}
    ...
    public function get_script_depends() {
        return array_merge([ 'wc-checkout' ], apply_filters('etheme_elementor_widget_script_depends', [], true));
    }
	...
	protected function register_controls() {
        ...
        $this->add_control(
            'billing_details_section_heading',
            [
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .woocommerce-billing-fields' => '--widget-title-border-width: {{SIZE}}{{UNIT}}',
                ],
                'condition' => [
        ...
        $this->add_control(
            ...
            [
                ...
                'type'      => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .woocommerce-billing-fields' => '--widget-title-border-color: {{VALUE}};',
                ],
                'condition' => [
                    'heading_type!' => ['classic']
                ]
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'size_units' => [ 'px', 'rem' ],
                'selectors' => [
                    '{{WRAPPER}} .woocommerce-billing-fields' => '--widget-title-inner-space-bottom: {{SIZE}}{{UNIT}}',
                ],
                'separator' => 'before',
                'condition' => [
                    'heading_type!' => ['classic']
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'size_units' => [ 'px', 'rem' ],
                'selectors' => [
                    '{{WRAPPER}} .woocommerce-billing-fields' => '--widget-title-space-bottom: {{SIZE}}{{UNIT}}',
                ],
            ]
        );
...
//                ],
//                'selectors' => [
//                    '{{WRAPPER}} .woocommerce-billing-fields' => '--widget-title-element-width: {{SIZE}}{{UNIT}}',
//                ],
//                'condition' => [
...
//        );
        ...
        $this->add_control(
            ...
            [
                ...
                'type'      => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .woocommerce-billing-fields' => '--widget-title-element-color: {{VALUE}};',
                ],
                'condition' => [
                    'heading_type' => ['line-aside', 'square-aside', 'circle-aside', 'colored-underline']
                ]
	...
	protected function render() {
        ...
        if ( $multistep_design ) {
            add_filter('etheme_checkout_form_billing_wrapper', '__return_true');
            add_action('etheme_after_checkout_billing_form_fields_wrapper', array($this, 'multistep_footer_steps'));
        }

        add_filter('etheme_checkout_form_account_registration', '__return_false');

        add_filter('etheme_woocommerce_checkout_billing_title', array($this, 'modify_section_title'));

        if ( !!$settings['show_heading'] ) {
            add_filter('etheme_checkout_form_billing_title', '__return_true');
            ...
            add_filter('etheme_checkout_form_billing_title', '__return_false');
        }
        ...
        if ( !!$settings['show_heading'] ) {
            remove_filter('etheme_checkout_form_billing_title', '__return_true');
            ...
            remove_filter('etheme_checkout_form_billing_title', '__return_false');
        }

        remove_filter('etheme_woocommerce_checkout_billing_title', array($this, 'modify_section_title'));

        remove_filter('etheme_checkout_form_account_registration', '__return_false');

        if ( $multistep_design ) {
            remove_filter('etheme_checkout_form_billing_wrapper', '__return_true');
            remove_action('etheme_after_checkout_billing_form_fields_wrapper', array($this, 'multistep_footer_steps'));
        }
                    ...
                    $( document )
                        .on( 'country_to_state_changing', function( event, country, wrapper ) {
                            ...
                            $.each( locale_fields, function( key, value ) {
                                ...
                                if ( $(field).attr( 'data-o_class' ) ) {
                                    let classes = $(field).attr( 'data-o_class' ).split(' ');
                                    let needed_classes = ['form-row-first', 'form-row-last', 'form-row-wide'];
                                    $.each(needed_classes, function (key, _needed_class) {
                                        let found_class = classes.includes(_needed_class);
                                        if ( found_class )
                                            $(field).removeClass(needed_classes.join(' ')).addClass(_needed_class);
                                    });
                                    if ( $(field).is(':hidden') ) {
                                        let siblingField = null;
                                        if ( $(field).hasClass('form-row-first') ) {
                                            siblingField = $(field).next();
                                        }
                                        else if ( $(field).hasClass('form-row-last') ) {
                                            siblingField = $(field).prev();
                                        }
                                        if ( siblingField ) {
                                            $(field).addClass('et-classes-modifications');
                                            if ($(siblingField).hasClass('form-row-first') || $(siblingField).hasClass('form-row-last')) {
                                                if (!$(siblingField).attr('data-et_o_class')) {
                                                    $(siblingField).attr('data-et_o_class', $(siblingField).attr('class'));
                                                }
                                                $(siblingField).removeClass(needed_classes.join(' ')).addClass('form-row-wide');
                                            }
                                        }
                                    }
                                    else {
                                        if ( $(field).hasClass('et-classes-modifications') ) {
                                            $(field).removeClass('et-classes-modifications');
                                            let nextField = $(field).next();
                                            let prevField = $(field).prev();
                                            if ( nextField ) {
                                                $(nextField).removeClass(needed_classes.join(' ')).addClass($(nextField).attr('data-et_o_class'));
                                            }
                                            if ( prevField ) {
                                                $(prevField).removeClass(needed_classes.join(' ')).addClass($(prevField).attr('data-et_o_class'));
                                            }
                                        }
                                    }
                                }
