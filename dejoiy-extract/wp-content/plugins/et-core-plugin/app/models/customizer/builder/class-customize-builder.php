 ...
 * Added multiple date
 * 1.4.18
 * Fixed custom loaded fonts for multiple header templates
 * Fixed custom loaded fonts for multiple single products templates
 */
...
//            $wp_customize->remove_panel( 'woocommerce');
	...
	public function et_ajax_get_remote_preset() {
		$url = 'http://8theme.com/import/xstore-headers/' . sanitize_text_field( $this->post['preset'] ) . '.json';
	...
	public function et_ajax_get_presets() {
		check_ajax_referer( 'etheme_header-presets', 'security' );

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => 'Unauthorized access' ), 403 );
		}

		$preset = isset( $this->post['preset'] ) ? sanitize_file_name( $this->post['preset'] ) : '';
		if ( $preset === '' ) {
			wp_send_json_error( array( 'message' => 'Missing preset' ), 400 );
		}

		if ( isset( $this->post['header-for-version'] ) ) {
			$file_name = $preset . '.options.json';
			$fs_path   = ET_CORE_DIR . 'app/models/customizer/builder/headers/' . $file_name;
		} else {
			$file_name = $preset . '.json';
			$fs_path   = ET_CORE_DIR . 'app/models/customizer/builder/presets/' . $file_name;
		}

		$fs_path = wp_normalize_path( $fs_path );

		$raw = false;

		if ( file_exists( $fs_path ) && is_readable( $fs_path ) ) {
			$raw = file_get_contents( $fs_path ); // Legal request to self hosted file
		}

		if ( $raw === false ) {
			$content_dir = wp_normalize_path( WP_CONTENT_DIR );
			$relative    = ltrim( str_replace( $content_dir, '', $fs_path ), '/\\' );

			$http_url = content_url( $relative );
			...
			if ( is_wp_error( $response ) ) {
				wp_send_json_error( array(
					'message' => 'Failed to fetch preset: ' . $response->get_error_message(),
					'path'    => $fs_path,
					'url'     => $http_url,
				), 500 );
			}
		...
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			wp_send_json_error( array(
				'message' => 'Invalid JSON in preset file: ' . json_last_error_msg(),
				'file'    => $file_name,
			), 500 );
		}
	...
	public function template( $ajax = false ) {
        ...
        switch ( $this->post['template'] ) {
				...
				case 'multiple-headers':
					require_once( ET_CORE_DIR . 'app/models/customizer/builder/template-parts/conditions/multiple-'.sanitize_file_name($this->post['builder']).'.php' );
					break;
				case 'multiple-condition':
					require_once( ET_CORE_DIR . 'app/models/customizer/builder/template-parts/conditions/multiple-condition-'.sanitize_file_name($this->post['builder']).'.php' );
					break;
				case 'product-single':
	...
	public function et_multiple_headers(){
		...
		if ( count( $condition ) ) {
			...
			if ( isset( $headers[$condition['header']] ) ) {
				...
				wp_localize_script( apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general'), 'etMultiple', array( 'Header'=>$condition['header'] ) );
				// Enqueue specific styles
				wp_enqueue_style(
	...
	public function et_multiple_single_product(){
		...
		if ( count( $condition ) ) {
			...
			if ( isset( $headers[$condition['header']] ) ) {
				...
				wp_localize_script( apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general'), 'etMultiple', array( 'singleProduct'=>$condition['header'] ) );
//				$this->set_theme_mods($headers[$condition['header']]['options']);
				wp_dequeue_style( 'xstore-kirki-styles' );
				// Enqueue specific styles
				wp_enqueue_style(
	...
	public function set_theme_mods($data){
		foreach ($data as $key => $value) {
			...
			if ( ! in_array( $key, array('header_top_elements', 'header_main_elements', 'header_bottom_elements', 'connect_block_package','header_mobile_top_elements','header_mobile_main_elements','header_mobile_bottom_elements','connect_block_mobile_package') ) ) {
				...
				// Reset array with properly key
				if ($key == 'promo_text_package') {
					if (is_customize_preview()) {
						if (is_array($value)) {
							$sorted_value = array();
	 ...
	 * This functions in "framework/builder/js/builder.js".
	 * Callbacks for add/save/edit/configure/remove actions.
	 *
	 * @since   1.5.1
	 * @version 1.0.5
	 * @return  {json} string with data needed for action.
		...
		switch ( $this->post['type'] ) {
			case 'add':
				...
				if ( $this->post['builder'] == 'header' ) {
					...
					$headers[ $id ]['options']['cart_footer_content_et-desktop'] = isset( $headers[ $id ]['options']['cart_footer_content_et-desktop'] ) ? base64_decode( $this->post['options']['cart_footer_content_et-desktop'] ) : '';

					// Fonts options

					if (isset($this->post['options']) && isset($this->post['options']['menu_item_fonts_et-desktop']) )
						$headers[ $id ]['options']['menu_item_fonts_et-desktop'] = wp_unslash( $this->post['options']['menu_item_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['top_header_fonts_et-desktop']) )
						$headers[ $id ]['options']['top_header_fonts_et-desktop'] = wp_unslash( $this->post['options']['top_header_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['main_header_fonts_et-desktop']) )
						$headers[ $id ]['options']['main_header_fonts_et-desktop'] = wp_unslash( $this->post['options']['main_header_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['bottom_header_fonts_et-desktop']) )
						$headers[ $id ]['options']['bottom_header_fonts_et-desktop'] = wp_unslash( $this->post['options']['bottom_header_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['header_vertical_menu_content_fonts_et-desktop']) )
						$headers[ $id ]['options']['header_vertical_menu_content_fonts_et-desktop'] = wp_unslash( $this->post['options']['header_vertical_menu_content_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['menu_2_item_fonts_et-desktop']) )
						$headers[ $id ]['options']['menu_2_item_fonts_et-desktop'] = wp_unslash( $this->post['options']['menu_2_item_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['menu_dropdown_fonts_et-desktop']) )
						$headers[ $id ]['options']['menu_dropdown_fonts_et-desktop'] = wp_unslash( $this->post['options']['menu_dropdown_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['secondary_title_fonts_et-desktop']) )
						$headers[ $id ]['options']['secondary_title_fonts_et-desktop'] = wp_unslash( $this->post['options']['secondary_title_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['mobile_menu_content_fonts_et-desktop']) )
						$headers[ $id ]['options']['mobile_menu_content_fonts_et-desktop'] = wp_unslash( $this->post['options']['mobile_menu_content_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['button_fonts_et-desktop']) )
						$headers[ $id ]['options']['button_fonts_et-desktop'] = wp_unslash( $this->post['options']['button_fonts_et-desktop'] );
				} else {
					// html blocks
					if(isset($this->post['options']) && $this->post['options']['single_product_html_block1'] )
						$headers[$id]['options']['single_product_html_block1'] = wp_unslash( $this->post['options']['single_product_html_block1'] );
					if(isset($this->post['options']) && $this->post['options']['single_product_html_block2'] )
						$headers[$id]['options']['single_product_html_block2'] = wp_unslash( $this->post['options']['single_product_html_block2'] );
					if(isset($this->post['options']) && $this->post['options']['single_product_html_block3'] )
						$headers[$id]['options']['single_product_html_block3'] = wp_unslash( $this->post['options']['single_product_html_block3'] );
					if(isset($this->post['options']) && $this->post['options']['single_product_html_block4'] )
						$headers[$id]['options']['single_product_html_block4'] = wp_unslash( $this->post['options']['single_product_html_block4'] );
					if(isset($this->post['options']) && $this->post['options']['single_product_html_block5'] )
						$headers[$id]['options']['single_product_html_block5'] = wp_unslash( $this->post['options']['single_product_html_block5'] );

					// Fonts options
					if (isset($this->post['options']) && isset($this->post['options']['single_product_button_fonts_et-desktop']) )
						$headers[$id]['options']['single_product_button_fonts_et-desktop'] = wp_unslash( $this->post['options']['single_product_button_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_breadcrumbs_fonts_et-desktop']) )
						$headers[$id]['options']['product_breadcrumbs_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_breadcrumbs_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_breadcrumbs_return_to_previous_fonts_et-desktop']) )
						$headers[$id]['options']['product_breadcrumbs_return_to_previous_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_breadcrumbs_return_to_previous_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_breadcrumbs_product_title_fonts_et-desktop']) )
						$headers[$id]['options']['product_breadcrumbs_product_title_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_breadcrumbs_product_title_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_add_to_cart_fonts_et-desktop']) )
						$headers[$id]['options']['product_add_to_cart_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_add_to_cart_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_tabs_navigation_fonts_et-desktop']) )
						$headers[$id]['options']['product_tabs_navigation_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_tabs_navigation_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_title_fonts_et-desktop']) )
						$headers[$id]['options']['product_title_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_title_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['single_product_widget_area_1_title_fonts_et-desktop']) )
						$headers[$id]['options']['single_product_widget_area_1_title_fonts_et-desktop'] = wp_unslash( $this->post['options']['single_product_widget_area_1_title_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['request_quote_button_fonts_et-desktop']) )
						$headers[$id]['options']['request_quote_button_fonts_et-desktop'] = wp_unslash( $this->post['options']['request_quote_button_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['request_quote_popup_content_et-desktop']) )
						$headers[$id]['options']['request_quote_popup_content_et-desktop'] = wp_unslash( $this->post['options']['request_quote_popup_content_et-desktop'] );
				}
			...
			case 'save':
				...
				if ( $this->post['builder'] == 'header' ) {
					...
					if (isset($this->post['options']) && isset( $this->post['options']['header_socials_package_et-desktop'] ) ) {
					...
					if (isset($this->post['options']) && isset( $this->post['options']['contacts_package_et-desktop'] ) ) {
					...
					if (isset($this->post['options']) && isset( $this->post['options']['promo_text_package'] ) ) {
					...
					if (isset($this->post['options']) && isset( $this->post['options']['header_vertical_section1_content'] ) && $this->post['options']['header_vertical_section1_content'] == 'empty' ) {
						$headers[ $this->post['header'] ]['options']['header_vertical_section1_content'] = array();
					}

					if (isset($this->post['options']) && isset( $this->post['options']['header_vertical_section2_content'] ) && $this->post['options']['header_vertical_section2_content'] == 'empty' ) {
						$headers[ $this->post['header'] ]['options']['header_vertical_section2_content'] = array();
					}

					if (isset($this->post['options']) && isset( $this->post['options']['header_vertical_section3_content'] ) && $this->post['options']['header_vertical_section3_content'] == 'empty' ) {
						$headers[ $this->post['header'] ]['options']['header_vertical_section3_content'] = array();
					}

					if (isset($this->post['options']) && isset( $headers[ $this->post['header'] ]['options']['html_block1'] ) && isset( $this->post['options']['html_block1'] ) ) {
						$headers[ $this->post['header'] ]['options']['html_block1'] = base64_decode( $this->post['options']['html_block1'] );
					}
					if (isset($this->post['options']) && isset( $headers[ $this->post['header'] ]['options']['html_block2'] ) && isset( $this->post['options']['html_block2'] ) ) {
						$headers[ $this->post['header'] ]['options']['html_block2'] = base64_decode( $this->post['options']['html_block2'] );
					}
					if (isset($this->post['options']) && isset( $headers[ $this->post['header'] ]['options']['html_block3'] ) && isset( $this->post['options']['html_block3'] ) ) {
						$headers[ $this->post['header'] ]['options']['html_block3'] = base64_decode( $this->post['options']['html_block3'] );
					}
					if (isset($this->post['options']) && isset( $headers[ $this->post['header'] ]['options']['html_block4'] ) && isset( $this->post['options']['html_block4'] ) ) {
						$headers[ $this->post['header'] ]['options']['html_block4'] = base64_decode( $this->post['options']['html_block4'] );
					}
					if (isset($this->post['options']) && isset( $headers[ $this->post['header'] ]['options']['html_block5'] ) && isset( $this->post['options']['html_block5'] ) ) {
						$headers[ $this->post['header'] ]['options']['html_block5'] = base64_decode( $this->post['options']['html_block5'] );
					}

					if (isset($this->post['options']) && isset( $headers[ $this->post['header'] ]['options']['cart_footer_content_et-desktop'] ) && isset( $this->post['options']['cart_footer_content_et-desktop'] ) ) {
						$headers[ $this->post['header'] ]['options']['cart_footer_content_et-desktop'] = base64_decode( $this->post['options']['cart_footer_content_et-desktop'] );
					}

					if (isset($this->post['options']) && isset( $this->post['options']['search_placeholder_et-desktop'] ) && isset( $this->post['options']['search_placeholder_et-desktop'] ) ) {
					...
					// Fonts options
					if (isset($this->post['options']) && isset($this->post['options']['menu_item_fonts_et-desktop']) )
						$headers[ $this->post['header'] ]['options']['menu_item_fonts_et-desktop'] = wp_unslash( $this->post['options']['menu_item_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['top_header_fonts_et-desktop']) )
						$headers[ $this->post['header'] ]['options']['top_header_fonts_et-desktop'] = wp_unslash( $this->post['options']['top_header_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['main_header_fonts_et-desktop'] ) )
						$headers[ $this->post['header'] ]['options']['main_header_fonts_et-desktop'] = wp_unslash( $this->post['options']['main_header_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset( $this->post['options']['bottom_header_fonts_et-desktop'] ))
						$headers[ $this->post['header'] ]['options']['bottom_header_fonts_et-desktop'] = wp_unslash( $this->post['options']['bottom_header_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['header_vertical_menu_content_fonts_et-desktop'] ) )
						$headers[ $this->post['header'] ]['options']['header_vertical_menu_content_fonts_et-desktop'] = wp_unslash( $this->post['options']['header_vertical_menu_content_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['menu_2_item_fonts_et-desktop'] ))
						$headers[ $this->post['header'] ]['options']['menu_2_item_fonts_et-desktop'] = wp_unslash( $this->post['options']['menu_2_item_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['menu_dropdown_fonts_et-desktop'] ))
						$headers[ $this->post['header'] ]['options']['menu_dropdown_fonts_et-desktop'] = wp_unslash( $this->post['options']['menu_dropdown_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['secondary_title_fonts_et-desktop'] ))
						$headers[ $this->post['header'] ]['options']['secondary_title_fonts_et-desktop'] = wp_unslash( $this->post['options']['secondary_title_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['mobile_menu_content_fonts_et-desktop'] ))
						$headers[ $this->post['header'] ]['options']['mobile_menu_content_fonts_et-desktop'] = wp_unslash( $this->post['options']['mobile_menu_content_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['button_fonts_et-desktop'] ))
						$headers[ $this->post['header'] ]['options']['button_fonts_et-desktop'] = wp_unslash( $this->post['options']['button_fonts_et-desktop'] );
				} else {
					if (isset($this->post['options']) && isset($this->post['options']['single_product_html_block1']))
						$headers[ $this->post['header'] ]['options']['single_product_html_block1'] = wp_unslash( $this->post['options']['single_product_html_block1'] );
					if (isset($this->post['options']) && isset($this->post['options']['single_product_html_block2']))
						$headers[ $this->post['header'] ]['options']['single_product_html_block2'] = wp_unslash( $this->post['options']['single_product_html_block2'] );
					if (isset($this->post['options']) && isset($this->post['options']['single_product_html_block3']))
						$headers[ $this->post['header'] ]['options']['single_product_html_block3'] = wp_unslash( $this->post['options']['single_product_html_block3'] );
					if (isset($this->post['options']) && isset($this->post['options']['single_product_html_block4']))
						$headers[ $this->post['header'] ]['options']['single_product_html_block4'] = wp_unslash( $this->post['options']['single_product_html_block4'] );
					if (isset($this->post['options']) && isset($this->post['options']['single_product_html_block5']))
						$headers[ $this->post['header'] ]['options']['single_product_html_block5'] = wp_unslash( $this->post['options']['single_product_html_block5'] );

					// Fonts options
					if (isset($this->post['options']) && isset($this->post['options']['single_product_button_fonts_et-desktop']) && $this->post['options']['single_product_button_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['single_product_button_fonts_et-desktop'] = wp_unslash( $this->post['options']['single_product_button_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_breadcrumbs_fonts_etproduct_breadcrumbs_fonts_et-desktop']) && $this->post['options']['product_breadcrumbs_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['product_breadcrumbs_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_breadcrumbs_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_breadcrumbs_return_to_previous_fonts_et-desktop']) && $this->post['options']['product_breadcrumbs_return_to_previous_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['product_breadcrumbs_return_to_previous_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_breadcrumbs_return_to_previous_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_breadcrumbs_product_title_fonts_et-desktop']) && $this->post['options']['product_breadcrumbs_product_title_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['product_breadcrumbs_product_title_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_breadcrumbs_product_title_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_add_to_cart_fonts_et-desktop']) && $this->post['options']['product_add_to_cart_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['product_add_to_cart_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_add_to_cart_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_tabs_navigation_fonts_et-desktop']) && $this->post['options']['product_tabs_navigation_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['product_tabs_navigation_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_tabs_navigation_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['product_title_fonts_et-desktop']) && $this->post['options']['product_title_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['product_title_fonts_et-desktop'] = wp_unslash( $this->post['options']['product_title_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset($this->post['options']['single_product_widget_area_1_title_fonts_et-desktop']) && $this->post['options']['single_product_widget_area_1_title_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['single_product_widget_area_1_title_fonts_et-desktop'] = wp_unslash( $this->post['options']['single_product_widget_area_1_title_fonts_et-desktop'] );
					if (isset($this->post['options']) && isset( $this->post['options']['request_quote_button_fonts_et-desktop']) && $this->post['options']['request_quote_button_fonts_et-desktop'] )
						$headers[ $this->post['header'] ]['options']['request_quote_button_fonts_et-desktop'] = wp_unslash( $this->post['options']['request_quote_button_fonts_et-desktop'] );
				}
...
//				//}
	...
	public function switch_default(){
		check_ajax_referer('etheme_switch_default', 'security');
		if (current_user_can( 'manage_options' )){
	...
	public function default_header(){
		...
		if ( false === get_theme_mod('header_top_elements', false) && false === get_theme_mod('header_main_elements', false) && false === get_theme_mod('header_bottom_elements', false) ) {
			...
			$data = file_get_contents( $url ); // Legal request to self hosted file
