...
if(!function_exists('etheme_load_admin_styles')) {
    ...
    add_action('customize_controls_enqueue_scripts', 'etheme_load_admin_styles', 150 );
	function etheme_load_admin_styles() {
		...
		if ( strpos($screen_id, 'et-panel') || strpos($screen_id, 'xstore-options') ) {
...
//	    wp_enqueue_script('et_masonry', get_template_directory_uri().'/js/jquery.masonry.min.js',array(),false,true);
	    ...
	    $localize_admin_script = array(
			...
			'wp_customize_url' => apply_filters('etheme_theme_options_permalink', wp_customize_url(), 'main')
		);
...
if ( !function_exists('et_ajax_required_plugins_popup') ) {
    function et_ajax_required_plugins_popup() {
        ...
        if(! in_array($_POST['type'], $avaliable_files)){
...
if ( get_option('xstore_sales_booster_settings_fake_product_sales', false) || get_option( 'xstore_sales_booster_settings_fake_sale_popup', false ) ) {
