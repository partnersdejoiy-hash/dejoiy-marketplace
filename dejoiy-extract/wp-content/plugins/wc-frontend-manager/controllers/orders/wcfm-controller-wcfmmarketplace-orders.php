    ...
    public function processing() {
        ...
        if (!empty($order_summary)) {
            ...
            foreach ($order_summary as $order) {
                ...
                $datatable_json['data'] = apply_filters('wcfm_orders_custom_columns_data_after', $datatable_json['data'], $index, $order->order_id, $order, $the_order);
                ...
                $datatable_json['data'][$index][] =  $gross_sales;

                // Gross Sales Amount
                if ($order->is_partially_refunded || in_array(1, $is_refundeds)) {
                    $datatable_json['data'][$index][] =  $gross_sales_amt - (float) $order->refunded_amount;
                } elseif ($order->is_refunded) {
                    $datatable_json['data'][$index][] = 0;
                } else {
                    $datatable_json['data'][$index][] =  $gross_sales_amt;
                }
                ...
                if (($order->is_refunded && !in_array(0, $is_refundeds)) || in_array($order_status, array('failed', 'cancelled', 'refunded', 'request', 'proposal', 'proposal-sent', 'proposal-expired', 'proposal-rejected', 'proposal-canceled', 'proposal-accepted'))) {
                    ...
                    $datatable_json['data'][$index][] =  apply_filters('wcfm_vendor_order_total', wc_price($total, array('currency' => $order_currency)) . '<br />' . $status, $order->order_id, $order->product_id, $gross_sales_amt, $total, $status, $order_currency);
                ...
                $datatable_json['data'] = apply_filters('wcfm_orders_custom_columns_data_before', $datatable_json['data'], $index, $order->order_id, $order, $the_order);

                // Date
                $order_date = (version_compare(WC_VERSION, '2.7', '<')) ? $the_order->order_date : $the_order->get_date_created();
                if ($order_date) {
                    $datatable_json['data'][$index][] = apply_filters('wcfm_order_date_display', $order_date->date_i18n(wc_date_format() . ' ' . wc_time_format()), $order->order_id, $order);
                } else {
                    $datatable_json['data'][$index][] = apply_filters('wcfm_order_date_display', '&ndash;', $order->order_id, $order);
                }
                ...
                $datatable_json['data'][$index][] =  apply_filters('wcfmmarketplace_orders_actions', $actions, $user_id, $order, $the_order, $this->vendor_id);

                $index++;
            }
        }

        $datatable_json['data'] = apply_filters('wcfm_orders_controller_data', $datatable_json['data'], 'vendor', $this->vendor_id);
