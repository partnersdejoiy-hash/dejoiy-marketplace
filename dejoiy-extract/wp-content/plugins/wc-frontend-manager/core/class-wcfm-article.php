...
class WCFM_Article {
    ...
    function articles_wcfm_menus($menus) {
        ...
        $articles_menus = array('wcfm-articles' => array(
            'label'  => __('Articles', 'wc-frontend-manager'),
        ...
        if (isset($_POST['controller'])) {
            ...
            switch ($controller) {
                ...
                case 'wcfm-articles-manage':
                    ...
                    if (!apply_filters('wcfm_is_allow_manage_articles', true)) {
                        wp_send_json_error(esc_html__('You don&#8217;t have permission to do this.', 'woocommerce'));
                    }

                    $article_id = 0;
                    if (isset($_POST['wcfm_articles_manage_form'])) {
                        $form_data = array();
                        parse_str($_POST['wcfm_articles_manage_form'], $form_data);
                        if (isset($form_data['article_id'])) {
                            $article_id = absint($form_data['article_id']);
                        }
                    }

                    if ($article_id) {
                        $wcfm_article = get_post($article_id);
                        if (!$wcfm_article || $wcfm_article->post_type !== 'post') {
                            wp_send_json_error(__('Invalid Article.', 'wc-frontend-manager'));
                        }

                        if (!apply_filters('wcfm_is_allow_edit_articles', true)) {
                            wp_send_json_error(__('You don&#8217;t have permission to do this.', 'wc-frontend-manager'));
                        }

                        // check resource ownership
                        $resource_owner_id = $wcfm_article->post_author;
                        if (!wcfm_user_can_perform_request($resource_owner_id, 'article_edit')) {
                            wp_send_json_error(__('You don&#8217;t have permission to do this.', 'wc-frontend-manager'));
                        }
                    } else {
                        if (!apply_filters('wcfm_is_allow_add_articles', true)) {
                            wp_send_json_error(__('You don&#8217;t have permission to add article.', 'wc-frontend-manager'));
                        }
                        if (!apply_filters('wcfm_is_allow_article_limit', true)) {
                            wp_send_json_error(__('Article Limit Reached', 'wc-frontend-manager'));
                        }
                        if (!apply_filters('wcfm_is_allow_space_limit', true)) {
                            wp_send_json_error(__('Space Limit Reached', 'wc-frontend-manager'));
                        }
                    }
    ...
    public function delete_wcfm_article() {
        ...
        $articleid = isset($_POST['articleid']) ? absint($_POST['articleid']) : 0;
        if (!$articleid) {
            wp_send_json_error(__('Invalid Article.', 'wc-frontend-manager'));
        }

        $wcfm_article = get_post($articleid);
        if (!$wcfm_article || $wcfm_article->post_type !== 'post') {
            wp_send_json_error(__('Invalid Article.', 'wc-frontend-manager'));
        }

        $resource_owner_id = $wcfm_article->post_author;
        if (!wcfm_user_can_perform_request($resource_owner_id, 'article_delete')) {
            wp_send_json_error(esc_html__('You don&#8217;t have permission to do this.', 'woocommerce'));
        }

        do_action('wcfm_before_article_delete', $articleid);

        $permanent_delete = apply_filters('wcfm_is_allow_article_delete', false);
        $result = $permanent_delete ? wp_delete_post($articleid, true) : wp_trash_post($articleid);

        if ($result) {
            wp_send_json_success('success');
        } else {
            wp_send_json_error(__('Deletion failed.', 'wc-frontend-manager'));
        }
    }
}
