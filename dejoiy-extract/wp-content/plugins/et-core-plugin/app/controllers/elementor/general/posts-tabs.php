...
class Posts_Tabs extends \Elementor\Widget_Base {
	...
	public function get_style_depends() {
	    return apply_filters('etheme_elementor_widget_style_depends', ['etheme-elementor-posts']);
	}
	...
	public function get_script_depends() {
        return apply_filters('etheme_elementor_widget_script_depends', ['etheme_elementor_tabs']);
	}
	...
	protected function register_controls() {
        ...
        $this->register_general_controls($local_post_type_details);

        // post controls
        $this->register_post_controls($local_post_type_details);
		
		$this->register_tabs_controls($local_post_type_details);
		...
		// post
		$this->register_post_style_controls($local_post_type_details);
		...
		// date label
        $this->register_post_date_label_style_controls();
		
		$this->register_categories_label_style_controls();
		
		// tags
		$this->register_tags_label_style_controls();
		
		// Section Shape Divider
		$this->register_image_shape_divider_style_controls();
    ...
    protected function register_general_controls($local_post_type_details) {
...
//                ],
    ...
    protected function register_post_controls($local_post_type_details) {
        $this->start_controls_section(
...
//					);
    ...
    protected function register_tabs_controls($local_post_type_details) {
        $this->start_controls_section(
            'section_tabs',
            [
...
//				'title_field' => '{{{ tab_title }}}',
                'title_field' => '{{{ elementor.helpers.renderIcon( this, tab_title_selected_icon, {}, "i", "panel" ) || \'<i class="{{ tab_title_icon }}" aria-hidden="true"></i>\' }}} <span style="text-transform: capitalize;">{{{ tab_title }}}</span>',
                'default' => [
                    [
...
//				],
    ...
    protected function register_post_style_controls($local_post_type_details) {
        $this->start_controls_section(
            'section_post_style',
            [
                'label' => $local_post_type_details['post_type_name'],
    ...
    protected function register_categories_label_style_controls() {
        $this->start_controls_section(
...
//		);
    ...
    protected function register_tags_label_style_controls() {
        $this->start_controls_section(
...
//				$this->add_render_attribute( 'tab-mobile', 'class', 'etheme-elementor-tab-active' );
		                    ...
		                    $this->add_render_attribute( $button_attributes, [
				                    ...
				                    'data-query-settings' => esc_attr(wp_json_encode(array_merge(
					                    ...
					                    array(
                                            ...
                                            'query_post_type' => $item_settings['query_post_type'],
		                    ...
		                    switch ($settings['navigation']) {
			                    ...
			                    case 'scroll':
                                    ...
                                    <div <?php $this->print_render_attribute_string( 'load-more-button-wrapper'.$item_settings['_id'] ); ?>>
                                        <?php if ( $settings['navigation'] == 'button' && $settings['navigation_button_type'] == 'advanced' ) :
                                            $this->get_progress_bar($item_settings['_id'], $found_posts, $item_settings['posts_per_page']);
                                        endif; ?>
			                    ...
			                    case 'pagination':
				                    ...
				                    if ( !!$settings['navigation_pagination_ajax'] ) {
					                    ...
					                    $this->add_render_attribute( 'pagination-wrapper'.$item_settings['_id'], [
						                    'class' => 'etheme-elementor-pagination-ajax' . $pagination_classes,
	...
	public function get_content_post($settings) {
		...
		foreach ($this->get_post_elements() as $key => $string_text) {
			...
			switch ($key) {
				case 'image':
                    ...
                    $post_content[$key] = apply_filters('etheme_posts_post_element_'.$key, $thumbnail_html, get_the_ID(), $local_settings);
					
				break;
                case 'title':
                    $post_content[$key] = $this->render_post_title();
                    break;
                case 'excerpt':
                    $post_content[$key] = $this->render_post_excerpt();
                    break;
				case 'meta':
				    ob_start();
					...
					break;
                case 'categories':
                    $post_content[$key] = $this->render_post_categories_tags($local_settings['post_'.$key.'_limit']);
                    break;
                case 'tags':
                    $post_content[$key] = $this->render_post_categories_tags($local_settings['post_'.$key.'_limit'], 'post_tag');
                    break;
                case 'button':
                    $post_content[$key] = $this->render_post_read_more();
                    break;
				case 'share':
				    ob_start();
		...
		if ( array_key_exists('image', $post_content) ) {
			$image_tag = 'div';
            if ( apply_filters('etheme_posts_post_image_link', $local_settings['image_link'], get_the_ID(), $local_settings) ) {
				$image_tag = 'a';
	...
	protected function render_post_title() {
        ...
        $post_title_html .= '<'.$tag .' class="etheme-post-title">';
        $post_title_html .= '<a href="' . get_the_permalink() .'">';
        $post_title_html .= $this->limit_string(get_the_title());
        $post_title_html .= '</a>';
        $post_title_html .= '</'.$tag.'>';
    ...
    protected function render_post_excerpt() {
        $post_excerpt = '';
        $post_excerpt .= '<div class="etheme-post-excerpt">';
        $post_excerpt .= $this->limit_string(get_the_excerpt(), 'excerpt');
        $post_excerpt .= '</div>';
        return $post_excerpt;
    }
    ...
    protected function get_post_categories($id, $args) {
        $terms = array();
    ...
    protected function render_post_categories_tags($number = 0, $type = 'category') {
        ...
        $cats_tags_content = '';
        $cats_tags_content .= '<span class="etheme-post-terms etheme-post-' . esc_attr($class) . '">';
        foreach ($terms as $term) {
            $term = is_object($term) ? $term : get_term_by( 'id', $term, $type );
            $cats_tags_content .= '<a href="' . esc_url( get_term_link( $term ) ) . '">' . $term->name . '</a>';
        }
        $cats_tags_content .= '</span>';
        return $cats_tags_content;
    }
...
<path d="M22.56 1.68h-2.568v-1.032c0-0.264-0.24-0.576-0.576-0.576h-2.76c-0.264 0-0.576 0.24-0.576 0.576v1.032h-8.28v-1.032c0-0.264-0.24-0.576-0.576-0.576h-2.712c-0.264 0-0.576 0.24-0.576 0.576v1.032h-2.496c-0.264 0-0.552 0.24-0.552 0.576v21.096c0 0.264 0.24 0.576 0.576 0.576h21.096c0.264 0 0.576-0.24 0.576-0.576v-21.12c-0.024-0.312-0.264-0.552-0.576-0.552zM22.032 7.080v15.72h-20.016v-15.72h20.016zM5.136 3.24v-2.040h1.632v2.064h-1.632zM17.232 3.24v-2.040h1.632v2.064h-1.632zM4.608 4.392h2.76c0.264 0 0.576-0.24 0.576-0.576v-1.032h8.256v1.032c0 0.264 0.24 0.576 0.576 0.576h2.736c0.264 0 0.576-0.24 0.576-0.576v-1.032h1.992v3.216h-20.064v-3.216h2.040v1.032c-0.024 0.264 0.216 0.576 0.552 0.576zM19.584 9.096h-15.168v11.664h15.192v-11.664zM18.48 17.232v2.424h-2.424v-2.424h2.424zM18.48 13.704v2.448h-2.424v-2.448h2.424zM18.48 10.176v2.448h-2.424v-2.448h2.424zM14.976 17.232v2.424h-2.4v-2.424h2.4zM14.976 13.704v2.448h-2.4v-2.448h2.4zM14.976 10.176v2.448h-2.4v-2.448h2.4zM9.024 12.624v-2.448h2.424v2.448h-2.424zM9.024 16.152v-2.448h2.424v2.448h-2.424zM11.448 17.232v2.424h-2.424v-2.424h2.424zM7.92 17.232v2.424h-2.424v-2.424h2.424zM7.944 13.704v2.448h-2.424v-2.448h2.424zM7.944 10.176v2.448h-2.424v-2.448h2.424z"></path>
    ...
    protected function render_post_read_more() {
        global $local_settings;
        $post_read_more_html = '';
        $post_read_more_html .= '<a class="' . ('button' == $local_settings['post_button_type'] ? 'elementor-button ' : '') . 'etheme-post-button" href="' . get_permalink(get_the_ID()) . '">';

        //				if ( empty($local_settings['post_button_custom_selected_icon']['value']) || $local_settings['post_button_icon_align'] == 'right') {
        $post_read_more_html .= '<span class="button-text">' . $local_settings['post_button_text'] . '</span>';
        //				}
        if ( $local_settings['post_button_type'] == 'text' ) {
            $post_read_more_html .= '<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor">'.
                (is_rtl() ?
        ...
        $post_read_more_html .= '</a>';

        return $post_read_more_html;
    }
     ...
     * @since 5.2
     */
    public function get_progress_bar($prefix, $found_posts, $posts_per_page) {
        $this->add_render_attribute( 'load-more-button-progress'.$prefix, [
            'class' => [
                'etheme-elementor-lazy-progress-wrapper',
            ],
        ]);

        $this->add_render_attribute( 'load-more-button-progress-text'.$prefix, [
            'class' => [
        ...
        $this->add_render_attribute( 'load-more-progress-bar'.$prefix, [
            'class' => [
                'etheme-elementor-lazy-progress-bar',
            ] ]);

        $this->add_render_attribute( 'load-more-progress-bar-inner'.$prefix, [
            'class' => [
        ...
        ?>
        <div <?php $this->print_render_attribute_string( 'load-more-button-progress'.$prefix ); ?>>
            <div <?php $this->print_render_attribute_string( 'load-more-button-progress-text'.$prefix ); ?>>
                <?php echo sprintf(esc_html__('Showing %s of %s items', 'xstore-core'), $posts_per_page, $found_posts); ?>
            </div>
            <span <?php $this->print_render_attribute_string( 'load-more-progress-bar'.$prefix ); ?>>
                <span <?php $this->print_render_attribute_string( 'load-more-progress-bar-inner'.$prefix ); ?>></span>
            </span>
        </div>
        <?php
    }
...
//		$settings = $this->get_active_settings();
		...
		?>">
			...
			echo file_get_contents( $shape_path ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped Legal request to self hosted file
			?>
        </div>
		<?php
	}
