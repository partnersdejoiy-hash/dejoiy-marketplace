...
class Etheme_WooCommerce_Product_Reviews_Images {
	...
	public function hooks() {
        ...
        add_filter('etheme_comment_reviews_images', array( $this, 'images_output_on_review' ), 10, 2);
...
//                add_meta_box( 'woocommerce-rating', __( 'Rating', 'xstore' ), 'WC_Meta_Box_Product_Reviews::output', 'comment', 'normal', 'high' );
	...
	public function upload_image_form( $submit_field ) {
        ...
        if ( apply_filters('etheme_sales_booster_'.self::$option_name.'_required_login', true) && !is_user_logged_in() ) {
		...
		wp_enqueue_script( apply_filters('etheme_elementor_force_localize_global_assets', 'et_reviews_images',  'woocommerce') );
		wp_localize_script( apply_filters('etheme_elementor_force_localize_global_assets', 'et_reviews_images',  'woocommerce'), 'et_reviews_images_config', array(
	...
	public function allow_media_types() {
		add_filter( $this->filter_name, array( $this, 'filter_upload_mimes' ), 200 );
	}
	...
	public function disallow_media_types() {
		remove_filter( $this->filter_name, array( $this, 'filter_upload_mimes' ), 200 );
	}
	 ...
	 *
	 */
	public function images_output( $comment_content, $force_display = false, $comment_id = 0 ) {
		if ( ! $this->check_existed_images($comment_id) || !$force_display && (is_admin() || ! is_singular( 'product' )) ) {
    ...
    /**
     * Display images in Testimonials widget via filter
     *
     * @param $comment_text
     * @param $comment_id
     * @return string
     ...
     * @since 9.6.2
     */
    public function images_output_on_review($comment_text, $comment_id) {
        return $this->images_output($comment_text, true, $comment_id);
    }
