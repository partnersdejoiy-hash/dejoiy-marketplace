			...
			<div class="store-data-container rgt">
				<div class="store-data">
					...
					<?php if ( $store_address && ( $store_info['store_hide_address'] == 'no' ) && wcfm_vendor_has_capability( $store_id, 'vendor_address' ) ): ?>
						<p class="store-address"><?php echo wp_kses_post(wp_unslash($store_address)); ?></p>
					<?php endif ?>
					...
					if( $distance !== '' ) {
						...
						?>
						<p class="store-phone">
							<i class="wcfmfa fa-map-marker-alt" aria-hidden="true"></i> <?php echo wp_kses_post($msg); ?>
						</p>
					<?php } 
					} ?>
					...
					<?php if ( $store_description && apply_filters( 'wcfm_is_allow_store_list_about', false ) ) { ?>
						<p class="store-phone">
							...
							echo wp_kses_post( substr( wp_unslash( $store_description ), 0, $pos ) . '...' ); 
							?>
						</p>
					<?php } ?>
