...
final class Customizer extends Base_Controller {
	...
	public function hooks() {
        ...
        add_filter( 'kirki/fonts/standard_fonts', array($this, 'add_custom_fonts_filter'), 20 );

        add_filter( 'kirki/fonts/standard_fonts', array($this, 'add_custom_adobe_fonts_filter'), 20 );
    ...
    // **********************************************************************//
    public function add_custom_fonts_filter( $standard_fonts ){
    ...
    // **********************************************************************//
    // ! Add custom adobe fonts to customizer typography
    // **********************************************************************//
    public function add_custom_adobe_fonts_filter( $standard_fonts ) {
        $custom_fonts      = array();
        $etheme_fonts = get_theme_mod( 'etheme_typekit_fonts', '' );
        if ( $etheme_fonts ) {
            $typekit = explode( ',', $etheme_fonts );
            foreach ( $typekit as $font ) {
                $custom_fonts[$font] = array(
                    'label' => ucfirst( trim( $font ) ),
                    'variant' => '400',
                    'stack' => $font
                );
            }
	...
	public function enqueue_control_script() {
        ...
        wp_register_script('et-wp-tinymce',
		...
		wp_register_script(
			...
			array_merge(array(
				...
				'customize-base',
				'customize-controls'
            ), (apply_filters('etheme_is_fse_theme', false) ? array('et-wp-tinymce') : []),
                array(
				// 'wcag_colors',
		...
		// style
		wp_enqueue_style( 'builder',  ET_CORE_URL . 'app/models/customizer/builder/css/builder-style.css', array('etheme_admin_css') );
		if ( is_rtl() ) {
			wp_enqueue_style( 'builder-rtl',  ET_CORE_URL . 'app/models/customizer/builder/css/builder-style-rtl.css', array('etheme_admin_css', 'builder') );
		}
	...
	public function get_search_result( $search ) {
		...
		foreach ( \Kirki_Extended::$fields as $key => $field ) {
            ...
            $field['tooltip']     = isset( $field['tooltip'] ) ? $field['tooltip'] : '';
            $field['data_tags'] = isset( $field['data_tags'] ) ? $field['data_tags'] : '';
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		...
		if ( preg_match( \Etheme_Lazy_Default_Setting::$lazy_pattern, $setting_id, $matches ) ) {
			...
			$args = array(
				'id'           		=> $setting_id,
				'type'              => array_key_exists('option_type', $option) ? $option['option_type'] : 'theme_mod',
				'default'           => array_key_exists('default', $option) ? $option['default'] : '',
				'capability'        => array_key_exists('capability', $option) ? $option['capability'] : 'edit_theme_options',
				'transport'         => array_key_exists('transport', $option) ? $option['transport'] : 'refresh',
				'sanitize_callback' => array_key_exists('sanitize_callback', $option) ? $option['sanitize_callback'] : [],
			);
