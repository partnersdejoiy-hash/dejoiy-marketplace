...
class Plugins{
	 ...
	 * ADDED: ET_CORE_THEME_MIN_VERSION
	 * @loa 1.0.2
	 * ADDED: $no_tgm plugin activation
	 *
	 */
	public function ajax_plugins() {
		...
		if (!etheme_api_connection_check()){
			wp_send_json_error( array( 'error' => 31, 'message' => esc_html__( 'We are sorry to inform you that the XStore theme is unable to connect to the XStore API. Please check your internet connection, server whitelists and SSL certificate.
', 'xstore' ) ) );
		}
		...
		foreach ( $no_tgm as $slug => $plugin ) {
			if ( $_POST['slug'] == $slug ) {
				$result = activate_plugin( $plugin['dir'] . '/' . $plugin['file'] );
				$array = array(
					'done' => 1,
					'slug' => $_POST['slug'],
				...
				if ( is_wp_error( $result ) ) {
					$array = array(
						'done' => 0,
						'slug' => $_POST['slug'],
						'message' => esc_html__( 'WP error!', 'xstore' )
					);
				}
				wp_send_json($array);
				exit;
	 ...
	 */
	public function get_popup_plugin_list($version, $engine = 'wpb'){
		$version_import_data = function_exists( 'etheme_get_demo_import_data' )
			? etheme_get_demo_import_data( $version, $engine )
			: array();
		$version_plugins = isset( $version_import_data['plugins'] ) && is_array( $version_import_data['plugins'] )
			? $version_import_data['plugins']
			: array();
		$instance = call_user_func( array( get_class( $GLOBALS['tgmpa'] ), 'get_instance' ) );
		$plugins  = array(
			'all'      => array(), // Meaning: all plugins which still have open actions.
...
//			if ( $new_is_plugin_active && false === $instance->does_plugin_have_update( $slug ) ) {
		...
		$to_show = array();
		foreach ( $version_plugins as $item ) {
