...
$element_options['in_mobile_menu'] = isset($et_builder_globals['in_mobile_menu']) && $et_builder_globals['in_mobile_menu'];
...
if ( get_theme_mod('bold_icons', 0) ) {
	$element_options['search_icon_et-desktop'] = $element_options['search_icon_default_et-desktop'] = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path d="M23.64 22.176l-5.736-5.712c1.44-1.8 2.232-4.032 2.232-6.336 0-5.544-4.512-10.032-10.032-10.032s-10.008 4.488-10.008 10.008c-0.024 5.568 4.488 10.056 10.032 10.056 2.328 0 4.512-0.792 6.336-2.256l5.712 5.712c0.192 0.192 0.456 0.312 0.72 0.312 0.24 0 0.504-0.096 0.672-0.288 0.192-0.168 0.312-0.384 0.336-0.672v-0.048c0.024-0.288-0.096-0.552-0.264-0.744zM18.12 10.152c0 4.392-3.6 7.992-8.016 7.992-4.392 0-7.992-3.6-7.992-8.016 0-4.392 3.6-7.992 8.016-7.992 4.392 0 7.992 3.6 7.992 8.016z"></path></svg>';
}
else {
	$element_options['search_icon_et-desktop'] = $element_options['search_icon_default_et-desktop'] = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24"><path d="M23.784 22.8l-6.168-6.144c1.584-1.848 2.448-4.176 2.448-6.576 0-5.52-4.488-10.032-10.032-10.032-5.52 0-10.008 4.488-10.008 10.008s4.488 10.032 10.032 10.032c2.424 0 4.728-0.864 6.576-2.472l6.168 6.144c0.144 0.144 0.312 0.216 0.48 0.216s0.336-0.072 0.456-0.192c0.144-0.12 0.216-0.288 0.24-0.48 0-0.192-0.072-0.384-0.192-0.504zM18.696 10.080c0 4.752-3.888 8.64-8.664 8.64-4.752 0-8.64-3.888-8.64-8.664 0-4.752 3.888-8.64 8.664-8.64s8.64 3.888 8.64 8.664z"></path></svg>';
}
...
if ( $element_options['icon_type_et-desktop'] == 'custom' && $element_options['icon_custom'] != '' ) {
    ...
    elseif ( function_exists('etheme_get_image') ) {
		$icon_id = $element_options['icon_custom'];

		if($icon_id){
			// fix for wpb_getImageBySize function 
			if( function_exists('wpb_getImageBySize') ){
				if(get_post( $icon_id )){
					$element_options['search_icon_et-desktop'] = etheme_get_image( $icon_id, 'thumbnail' );
				}
			} else {
				$element_options['search_icon_et-desktop'] = etheme_get_image( $icon_id, 'thumbnail' );
			}
...
elseif ( $element_options['icon_type_et-desktop'] == 'none' ) {
    $element_options['search_icon_et-desktop'] = '';
}
...
$element_options['search_placeholder_et-desktop'] = get_theme_mod( 'search_placeholder_et-desktop', esc_html__('Search for...', 'xstore-core') );
...
if ( $element_options['search_category_et-desktop'] && $element_options['etheme_search_results'] ) {
	...
	$element_options['search_all_categories_text_et-desktop'] = get_theme_mod('search_all_categories_text_et-desktop', esc_html__('All categories', 'xstore-core'));
...
$element_options['wrapper_class'] .= ( ($element_options['in_mobile_menu'] || (isset($et_builder_globals['in_mobile_panel']) && $et_builder_globals['in_mobile_panel'])) ? '' : ' et_element-top-level' );
$element_options['wrapper_class'] .= ( $element_options['is_popup'] ) ? ' search-full-width' : '';

if ( $element_options['search_by_icon-click'] && !$element_options['in_mobile_menu'] && (get_query_var('is_mobile', false) || $element_options['is_popup'])) {
...
$element_options['should_redirect_to_archive'] = true;
if ( defined('ELEMENTOR_VERSION') && defined( 'ELEMENTOR_PRO_VERSION' ) ) {
    ...
    if ( count($created_templates) ) {
        $element_options['should_redirect_to_archive'] = false;
...
if ( $element_options['search_ajax'] ) {
    ...
    if ( !$element_options['in_mobile_menu'] && function_exists('etheme_enqueue_style')) {
...
<div class="et_element et_b_header-search flex align-items-center <?php echo $element_options['wrapper_class']; ?>" <?php echo implode( ' ', $element_options['attributes'] ); ?>>
	<?php if ( $element_options['search_by_icon-click'] ) { ?>
        <span class="flex et_b_search-icon <?php echo esc_attr($element_options['icon_class']); ?>">
            ...
            if ( $element_options['search_label'] ) : ?>
            <span class="et-element-label inline-block <?php echo ( !$element_options['in_mobile_menu'] ) ? $element_options['label_class'] : ''; ?>">
        ...
        <form action="<?php echo esc_url( $url ); ?>" role="search" data-min="<?php echo esc_attr($element_options['min_symbols']); ?>" data-per-page="<?php echo esc_attr($element_options['posts_per_page']); ?>"
            ...
            <?php endif; ?>
                ...
                <div class="input-row flex align-items-center <?php echo implode(' ', $element_options['input_row_class']); ?>" data-search-mode="<?php echo (get_theme_mod('search_category_select_color_scheme_et-desktop', 'dark')) ?>">
                    ...
                    <input type="text" value="<?php if(apply_filters( 'etheme_search_input_keep_last_value', true ) && $element_options['search_query']) echo esc_attr($element_options['search_query']); ?>"
					...
					<?php if ( $element_options['is_woocommerce'] && $element_options['should_redirect_to_archive'] ): ?>
                        <input type="hidden" name="post_type" value="<?php echo ( $element_options['is_woocommerce'] ) ? 'product': 'post'; ?>">
					<?php endif ?>

                    <?php if ( $element_options['should_redirect_to_archive'] ) : ?>
                    ...
                    <button type="submit" class="search-button flex justify-content-center align-items-center pointer" aria-label="<?php echo esc_attr__('Search button', 'xstore-core'); ?>">
                        <span class="et_b-loader"></span>
                    <?php echo $element_options['search_icon_et-desktop']; // echo $element_options['search_icon_default_et-desktop']; ?>
                    ...
                    <div class="ajax-search-tags full-width align-<?php echo !$element_options['is_mobile'] ? 'center' : 'start'; ?>">
                        ...
                        <?php if ( $element_options['is_popup'] && $element_options['is_mobile'] ) echo '<div class="ajax-search-tags-inner">';
                            foreach ( $element_options['search_tags'] as $search_tag ) {
                                $search_tag_query_args = array('s' => $search_tag,  'post_type' => ($element_options['is_woocommerce'] ? 'product': 'post'));
                                if ( $element_options['should_redirect_to_archive'] )
                                    $search_tag_query_args['et_search'] = 'true';
                                    ?>
                                <a href="<?php echo esc_url( add_query_arg($search_tag_query_args, $url )); ?>"><?php echo esc_html($search_tag); ?></a>
                            <?php } ?>
                    ...
                    <div class="ajax-search-history full-width align-<?php echo !$element_options['is_mobile'] ? 'center' : 'start'; ?> <?php if (count($history)) echo 'active'; ?>">
                        ...
                        <?php if ( $element_options['is_popup'] && $element_options['is_mobile'] ) echo '<div class="ajax-search-tags-inner">'; ?>
                           ...
                           <?php $_i = 0; foreach ( $history as $item ) : $_i++; ?>
                               ...
                               $search_tag_query_args = array('s' => $item,  'post_type' => ($element_options['is_woocommerce'] ? 'product': 'post'));
                               if ( $element_options['should_redirect_to_archive'] )
                                   $search_tag_query_args['et_search'] = 'true'; ?>
                               <a href="<?php echo esc_url( add_query_arg($search_tag_query_args, $url )); ?>" data-s="<?php echo esc_js($item); ?>"><?php echo esc_html($item); ?></a>
                           <?php endforeach; ?>
