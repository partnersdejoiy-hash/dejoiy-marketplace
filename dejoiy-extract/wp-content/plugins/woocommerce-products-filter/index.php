...
//if activated another copy of the plugin - stop
if ( defined( 'WOOF_LOADED' ) ) {
    return;
}
define( 'WOOF_LOADED', true );

// Check if the free version (woocommerce-products-filter) is active.
if ( defined( 'WOOF_VERSION' ) && version_compare( WOOF_VERSION, '3.0.0', '<' ) ) {
    // Show admin notice to deactivate the free version first.
    add_action( 'admin_notices', function() {
        echo '<div class="notice notice-error"><p>'
            . '<strong>HUSKY</strong>: Please deactivate the free version before using full version of HUSKY.'
            . '</p></div>';
    });
    return;
}
...
// classes
require WOOF_PATH . 'classes/request.php';
require WOOF_PATH . 'classes/storage.php';
require WOOF_PATH . 'classes/helper.php';
require WOOF_PATH . 'classes/cron.php';
require WOOF_PATH . 'classes/hooks.php';
require WOOF_PATH . 'classes/ext.php';
// ***
require WOOF_PATH . 'classes/counter.php';
require WOOF_PATH . 'classes/widgets.php';
// ***
require WOOF_PATH . 'lib/alert/index.php';
// ***
...
final class HUSKY {
	...
	public function init() {
		...
		if ( ( defined( 'REST_REQUEST' ) && REST_REQUEST ) ||
			( isset( $_SERVER['REQUEST_URI'] ) && false !== strpos( sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ), '/wp-json/' ) ) ) {
			return;
	    }
	...
	public function woof_save_options() {
		...
		$data = WOOF_HELPER::sanitize_array( json_decode( WOOF_REQUEST::get( 'formdata' ), true ) );
	...
	public function print_plugin_options() {
		...
		if ( apply_filters( 'woof_init_archive_by_default', true ) ) {
			...
			$wpdb->query(
				$wpdb->prepare(
					"UPDATE {$wpdb->prefix}woocommerce_attribute_taxonomies SET attribute_public = %d",
					1
				)
			);
			flush_rewrite_rules();
	...
	public function enqueue_scripts_styles() {
		...
		if ( isset( $this->settings['by_price']['show'] ) and intval( $this->settings['by_price']['show'] ) === 1 ) {
		...
		if ( $icheck_skin != 'none' ) {
			...
			if ( $icheck_skin != 'none' ) {
				...
				wp_enqueue_style( 'icheck-jquery-color-flat', WOOF_LINK . 'js/icheck/skins/flat/_all.css', array(), WOOF_VERSION );
				wp_enqueue_style( 'icheck-jquery-color-square', WOOF_LINK . 'js/icheck/skins/square/_all.css', array(), WOOF_VERSION );
				wp_enqueue_style( 'icheck-jquery-color-minimal', WOOF_LINK . 'js/icheck/skins/minimal/_all.css', array(), WOOF_VERSION );
			}
		}
		...
		wp_enqueue_style( 'woof-switcher23', WOOF_LINK . 'css/switcher.css', array(), WOOF_VERSION );
	}
	...
	private function get_term_from_cleared_url( $cleared_url ) {
		$path  = trim( wp_parse_url( $cleared_url, PHP_URL_PATH ), '/' );
		$parts = explode( '/', $path );
		$slug  = end( $parts ); // last segment = term slug e.g. "pullover"
	...
	public function wp_head() {
		...
		if ( ! defined( 'DOING_AJAX' ) && ! is_page() ) {
			...
			$queried_obj = get_queried_object();
			 ...
			 // When SEO URL mode is active, taxonomy may be removed from query_vars by url_request
			$seo_url_enabled = isset( $this->settings['woof_url_request']['enable'] ) && 
                       intval( $this->settings['woof_url_request']['enable'] );
			
			$taxonomy_check = $seo_url_enabled 
				? is_object( $queried_obj ) && get_class( $queried_obj ) == 'WP_Term'
				: isset( $wp_query->query_vars['taxonomy'] ) && is_object( $queried_obj ) && get_class( $queried_obj ) == 'WP_Term';
			
			
			if ( $taxonomy_check && ! isset( $request_data['really_curr_tax'] ) ) {
				if ( is_object( $queried_obj ) ) {
					$this->set_really_current_term( $queried_obj );
				}
			} elseif ( $seo_url_enabled && ! isset( $request_data['really_curr_tax'] )
					   && isset( WOOF_EXT::$includes['applications']['url_request'] ) ) {
				// Fallback: queried object is not WP_Term but SEO URL may hide the taxonomy context
				$url_ext     = WOOF_EXT::$includes['applications']['url_request'];
				$cleared_url = $url_ext->url_parser->get_cleared_url( $url_ext->url_parser->get_request_uri() );
		...
		} elseif ( $this->is_really_current_term_exists() ) {
				$this->set_really_current_term();
		}
	}
	...
	public function wp_load_js() {
		...
		?>
		const woof_front_nonce = "<?php echo esc_html( wp_create_nonce( 'woof_front_nonce' ) ); ?>";
		var woof_is_permalink =<?php echo intval( (bool) $this->is_permalink_activated() ); ?>;
		var woof_shop_page = "";
		<?php if ( ! $this->is_permalink_activated() ) : ?>
			woof_shop_page = "<?php echo esc_url_raw( home_url( '/?post_type=product' ) ); ?>";
		<?php endif; ?>
		...
		'clear_all': "<?php echo wp_kses_post( apply_filters( 'woof_clear_all_text', $clear_all_text ) ); ?>",
		...
		var woof_toggle_closed_text = "<?php echo esc_html( ( isset( $this->settings['toggle_closed_text'] ) and ! empty( $this->settings['toggle_closed_text'] ) ) ? trim( WOOF_HELPER::wpml_translate( null, $this->settings['toggle_closed_text'] ) ) : '+' ); ?>";
		var woof_toggle_opened_text = "<?php echo esc_html( ( isset( $this->settings['toggle_opened_text'] ) and ! empty( $this->settings['toggle_opened_text'] ) ) ? trim( WOOF_HELPER::wpml_translate( null, $this->settings['toggle_opened_text'] ) ) : '-' ); ?>";
		...
		$stxt .= PHP_EOL . " function woof_js_after_ajax_done() { jQuery(document).trigger('woof_ajax_done'); " . PHP_EOL
			. ( isset( $this->settings['js_after_ajax_done'] ) ? stripcslashes( html_entity_decode( $this->settings['js_after_ajax_done'] , ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ) : '' ) .
			PHP_EOL . '}';
	...
	public function get_all_filter_titles() {
		...
		foreach ( $items_order as $key ) {
			if ( in_array( $key, $this->items_keys ) ) {
				...
				} elseif ( isset( $this->settings[ $key ]['show'] ) && $this->settings[ $key ]['show'] != 0 ) {
						$options[ $key ] = $key;
				}
	...
	public function dynamic_count( $curr_term, $type, $additional_taxes = '', $meta_term = array(), $custom_type = '' ) {
		...
		if ( isset( $_GET['min_price'] ) && isset( $_GET['max_price'] ) ) {
			$old_price['min_price'] = floatval( wp_unslash( $_GET['min_price'] ) );
			$old_price['max_price'] = floatval( wp_unslash( $_GET['max_price'] ) );
		}
	...
	public function get_request_data($apply_filters = true) {
		...
		$data = map_deep( wp_unslash( $_GET ), 'sanitize_text_field' );
	...
	public function woof_products_ids_prediction($atts) {
		return $this->woof_products($atts, true);// nosemgrep
	}
	...
	public function woof_products($atts, $is_prediction = false) {
		...
		extract(shortcode_atts(array(// nosemgrep
			'columns' => apply_filters( 'loop_shop_columns', max( 1, (int) wc_get_loop_prop( 'columns', wc_get_default_products_per_row() ) ) ),
			'orderby' => 'no',
			'order' => 'no',
			'page' => 1,
		...
		if (WOOF_REQUEST::isset('link')) {
			$link = wp_parse_url(WOOF_REQUEST::get('link'), PHP_URL_QUERY);
		...
		$products = apply_filters('woof_draw_products_html', do_shortcode($shortcode_str), $shortcode_str);
		...
		return $this->render_html(WOOF_PATH . 'views/shortcodes/woof_show_btn.php', $atts);// nosemgrep
		}

		public function show_mobile_btn($atts) {
		if (wp_is_mobile()) {
			return $this->render_html(WOOF_PATH . 'views/shortcodes/woof_mobile_btn.php', $atts);// nosemgrep
			}
		...
		if (isset($atts['filter_blur'])) {
			$args['filter_blur'] = $atts['filter_blur'];
			} elseif (isset($this->settings['filter_blur'])) {
			$args['filter_blur'] = $this->settings['filter_blur'];
			}
		...
		return $this->render_html(WOOF_PATH . 'views/shortcodes/woof_price_filter_' . $type . '.php', $args);// nosemgrep
		}
		...
		if (isset($_SERVER['SCRIPT_URI'])) {
			$uri = wp_parse_url(trim(WOOF_HELPER::get_server_var('SCRIPT_URI')));
...
//***
	...
	$lastSegment = basename(wp_parse_url(WOOF_HELPER::get_server_var('SCRIPT_URI'), PHP_URL_PATH));
