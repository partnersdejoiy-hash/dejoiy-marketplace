...
if (get_query_var('et_breadcrumbs', false)): ?>
	...
	if( is_category() || is_tag() ) {
		...
		if( $term_id && $image = get_term_meta( $term_id, '_et_page_heading', true ) ) {
			$styles = 'style=" background-image: var(--page-heading-bg, url('. $image .')); margin-bottom: var(--page-heading-margin-bottom, 25px);"';
		}
	}

    ?>

	<div class="page-heading bc-type-<?php echo esc_attr( get_query_var('et_breadcrumbs-type', false) ); ?> bc-effect-<?php echo esc_attr( get_query_var('et_breadcrumbs-effect', false) ); ?> bc-color-<?php echo esc_attr( get_query_var('et_breadcrumbs-color', false) ); ?>" <?php echo esc_html($styles); ?>>
		<div class="container">
			<div class="row">
