...
function etheme_mobile_panel() {
    ...
    if ( get_query_var('et_is-cart-page-elementor-shortcode', false) || get_query_var('et_is-checkout-page-elementor-shortcode', false) ) {
        return;
    }
...
function etheme_header_wrapper_start() {
    ...
    $sticky_header = apply_filters('et_sticky_headers_should_render', ($sticky_header_mob || $sticky_header_dt));
	$sticky_header = $sticky_header ? 'sticky' : '';
	$class         .= $sticky_header;
	if ( $sticky_header != '' ) {
 ...
 * @version 1.0.2
 *
 * 1.0.1 - prevent start hooks if AMP version loaded
 * 1.0.2 - prevent start hooks if Customizer header disabled
 */
add_action( 'init', function () {
	if ( apply_filters( 'xstore_theme_amp', false ) || get_option( 'etheme_disable_customizer_header_builder', false )) {
		return;
	}
...
if ( ! function_exists( 'etheme_wishlist_quantity' ) ) {
	 ...
	 * @since   1.4.0
	 */
	function etheme_wishlist_quantity($count_only = false) {
        ...
        <div class="<?php esc_attr_e( $wishlist_class ); ?>" <?php echo implode( ' ', $wishlist_attr ); ?>>
			<?php if ( ! empty( $products ) ) : ?>
                ...
                <ul class="cart-widget-products">
                    ...
                    $yit_wpml_object_id_exists = function_exists( 'yit_wpml_object_id' );
					...
					foreach ( $products as $item ) {
                        ...
                        <li class="">
                            ...
                            <div class="product-item-right" data-row-id="<?php esc_attr_e( $item['prod_id'] ); ?>">
                                ...
                                <<?php echo apply_filters('etheme_woocommerce_mini_cart_product_title_tag', 'h4'); ?> class="product-title">
                                    <a href="<?php echo esc_url( $_product->get_permalink() ); ?>"><?php echo wp_specialchars_decode( $product_name ); ?></a>
                                </<?php echo apply_filters('etheme_woocommerce_mini_cart_product_title_tag', 'h4'); ?>>
								...
								<?php if ( $add_remove_ajax ) : ?>
                                    <a href="<?php echo add_query_arg( 'remove_from_wishlist', $item['prod_id'], esc_url( $wishlist_url ) ); ?>"
                                       ...
                                       title="<?php echo esc_attr( sprintf( __( 'Remove %s from wishlist', 'xstore-core' ), $product_name ) ); ?>"><i
                                                class="et-icon et-delete et-remove-type1"></i><i
                                                class="et-trash-wrap et-remove-type2"><img
                                                    src="<?php echo $trash_bin; ?>"
                                                    alt="<?php echo esc_attr( sprintf( __( 'Remove %s from wishlist', 'xstore-core' ), $product_name ) ); ?>"></i></a>
								<?php endif; ?>
                    ...
                    <div class="et-content">
                        ...
                        <form class="woocommerce-form woocommerce-form-login login <?php if ( $with_tabs ) {
							echo 'et_b-tab-content active';
                              ...
                              action="<?php echo apply_filters( 'header_account_link', get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ); ?>">
...
if ( ! function_exists( 'et_b_mobile_menu_content' ) ) {
	function et_b_mobile_menu_content() {
		...
		if ( isset( $_POST['multiple'] ) && isset( $_POST['multiple']['Header'] ) ) {
			$multiples         = new Etheme_Customize_header_Builder();
			$multiple_header = $multiples->get_json_option( 'et_multiple_headers' );
			$multiples->set_theme_mods( $multiple_header[ $_POST['multiple']['Header'] ]['options'] );
		}
		...
		<?php foreach ( $mob_menu_element_options['mobile_menu_content'] as $key => $value ) {
			if ( $value == 'menu' && $mob_menu_element_options['mobile_menu_2_state'] ) {
                ...
                <div class="et_b-tab-content <?php echo ( ! $mob_menu_element_options['mobile_menu_2_categories_primary'] ) ? 'active' : ''; ?>"
                     data-tab-name="menu">
					<?php
					$mob_menu_element_options['menu_getter'] = apply_filters( 'et_menu_cache', $args, $args['menu'] );
                ...
                <div class="et_b-tab-content <?php echo ( $mob_menu_element_options['mobile_menu_2_categories_primary'] ) ? 'active' : ''; ?>"
					...
					if ( $mob_menu_element_options['mobile_menu_2'] == 'categories' ) {
						...
						$mob_menu_element_options['menu_getter'] = apply_filters( 'et_menu_cache', $args_2, $args_2['menu'] );
						if ( $mob_menu_element_options['menu_getter'] != '' ) {
							?>
                            <div class="et_element et_b_header-menu header-mobile-menu flex align-items-center"
                                 data-title="<?php esc_html_e( 'Menu', 'xstore-core' ); ?>">
...
/**
 * Disable wpml admin check for ajax requests.
 *
 * @return  {bull} false
 * @version 1.0.1
 * @since   5.0.0
...
add_filter( 'wpml_ls_exclude_in_menu', function( $isExcluded ) {
	if ( isset( $_POST['action'] ) && 'etheme_ajax_popup_content' === $_POST['action'] ) {
		return false;
	}
	return $isExcluded;
} );
...
if ( ! function_exists( 'etheme_search_icon_mobile' ) ) {
	function etheme_search_icon_mobile() {
		return get_theme_mod( 'search_icon_et-mobile', get_theme_mod( 'search_icon_et-desktop', 'type1' ) );
	}
}
...
if ( ! function_exists( 'etheme_woocommerce_template_single_attributes' ) ) {
    /**
     * Single product attributes function.
     *
     * @return string
     * @since   5.1.1
     * @version 1.0.0
    ...
    function etheme_woocommerce_template_single_attributes() {
        if ( function_exists('etheme_display_product_attributes') )
            etheme_display_product_attributes();
    }
}
...
if ( ! function_exists( 'etheme_woocommerce_template_single_add_to_cart' ) ) {
	...
	function etheme_woocommerce_template_single_add_to_cart() {
		...
		if ( $is_builder ) :
			...
			$before_adc_content = html_blocks_callback(array(
			...
			echo '' != $before_adc_content || get_query_var( 'et_is_customize_preview', false ) ? '<div class="etheme-before-adc-content'.(get_query_var( 'et_is_customize_preview', false ) ? ' pos-relative': '').'">'.$before_adc_content.'</div>' : '';
		endif;
		...
		if ( $is_builder ) :
            ...
            $after_adc_content = html_blocks_callback(array(
                'section' => 'after_adc_html_block_section',
                'sections' => 'after_adc_html_block_sections',
                'html_backup' => 'after_adc_html_block',
                'section_content' => true
            ));
            echo '' != $after_adc_content || get_query_var( 'et_is_customize_preview', false ) ? '<div class="etheme-after-adc-content'.(get_query_var( 'et_is_customize_preview', false ) ? ' pos-relative': '').'">'.$after_adc_content.'</div>' : '';
		endif;
	}
}
...
if ( ! function_exists( 'etheme_set_related_product_args' ) ) {
	...
	function etheme_set_related_product_args( $args ) {
		...
		$args['posts_per_page'] = absint(get_theme_mod( 'products_related_limit_et-desktop', 7 ));
...
if ( ! function_exists( 'etheme_set_related_product_slider_args' ) ) {
	...
	function etheme_set_related_product_slider_args( $slider_args ) {
        ...
        $slider_args['tablet_land']     = apply_filters('products_related_per_view_tablet_land', 3, $slider_args);
...
if ( ! function_exists( 'etheme_set_upsells_product_args' ) ) {
	...
	function etheme_set_upsells_product_args( $args ) {
		...
		$args['posts_per_page'] = absint(get_theme_mod( 'products_upsell_limit_et-desktop', 7 ));
...
if ( ! function_exists( 'etheme_set_upsells_product_slider_args' ) ) {
	...
	function etheme_set_upsells_product_slider_args( $slider_args ) {
        ...
        $slider_args['tablet_land']     = apply_filters('products_upsell_per_view_tablet_land', 3, $slider_args);
...
if ( ! function_exists( 'etheme_set_cross_sells_product_args' ) ) {
	...
	function etheme_set_cross_sells_product_args( $args ) {
		...
		$args['posts_per_page'] = absint(get_theme_mod( 'products_cross_sell_limit_et-desktop', 7 ));
...
if ( ! function_exists( 'etheme_set_cross_sells_product_slider_args' ) ) {
	...
	function etheme_set_cross_sells_product_slider_args( $slider_args ) {
        ...
        $slider_args['tablet_land']     = apply_filters('products_cross_sell_per_view_tablet_land', 3, $slider_args);
...
if ( ! function_exists( 'etheme_product_single_waitlist' ) ) {
    /**
     * Single product waitlist function.
     *
     * @return string
     * @since   5.1.7
     * @version 1.0.0
    ...
    function etheme_product_single_waitlist() {
        require( ET_CORE_DIR . 'app/models/customizer/templates/woocommerce/single-product/etheme-product-waitlist.php' );
    }
}
...
//		}
		...
		$actions['add'] = array(
            ...
            array(
                'action'   => 'etheme_woocommerce_template_single_attributes',
                'function' => 'etheme_woocommerce_template_single_attributes',
                'priority' => 10
            ),
			array(
            ...
            array(
                'action'   => 'etheme_product_single_waitlist',
                'function' => 'etheme_product_single_waitlist',
                'priority' => 10
            ),
			array(
...
if ( ! function_exists( 'etheme_sticky_add_to_cart' ) ) {
	function etheme_sticky_add_to_cart($sticky_cart_options = array()) {
        $theme_builder_element = apply_filters('etheme_elementor_theme_builder', false);
        if ( !$theme_builder_element ) {
            if (!get_query_var('is_single_product', false)) {
                return;
            }
        }
		$sticky_cart_enabled = apply_filters('etheme_sticky_cart_enabled', get_theme_mod( 'sticky_add_to_cart_et-desktop', 0 ));
		$ghost          = get_query_var( 'et_is_customize_preview', false ) && ! $sticky_cart_enabled;
		$is_mobile      = get_query_var( 'is_mobile', false );
		if ( ! $sticky_cart_enabled && ! $ghost ) {
			return;
		}

		do_action('etheme_sticky_add_to_cart_before');

		if ( !$theme_builder_element && function_exists( 'etheme_enqueue_style' ) ) {
			etheme_enqueue_style( 'single-product-sticky-cart' );
            wp_enqueue_script( 'et_single_product_sticky_cart' );
		}
		...
		$buy_now = has_action( 'woocommerce_after_add_to_cart_button', 'etheme_buy_now_btn' );
		$without_buy_now      = apply_filters( 'etheme_sticky_add_to_cart_buy_now_hide_mobile', $is_mobile && $buy_now );
		$with_price_mobile = apply_filters( 'etheme_sticky_add_to_cart_price_hide_mobile', $is_mobile );

        $sticky_cart_options = shortcode_atts(
            array(
                'show_image' => true,
                'image_size' => 'post-thumbnail',
                'show_title' => true,
                'show_price' => !$with_price_mobile,
                'show_wishlist' => true,
                'show_compare' => true,
                'show_button' => true,
                'show_buy_now_button' => !$without_buy_now,
                'always_shown' => false,
                'boxed' => true
            ), $sticky_cart_options);

        if ( !$sticky_cart_options['show_wishlist'] ) {
            add_filter('xstore_wishlist_print_single_product_button', '__return_false');
        }
        if ( !$sticky_cart_options['show_compare'] ) {
...
//		add_action( 'woocommerce_external_add_to_cart', 'etheme_custom_add_to_cart', 30 );
        ...
        if ( $theme_builder_element ) {
            ...
            if (!$sticky_cart_options['show_buy_now_button'])
                remove_action( 'woocommerce_after_add_to_cart_button', 'etheme_buy_now_btn', 10 );
        }
        else {
            if (!$sticky_cart_options['show_buy_now_button']) {
                if (!$buy_now && $product_type == 'variable')
                    remove_action('woocommerce_after_add_to_cart_button', 'etheme_buy_now_btn', 10);
            }
        }
        $sticky_cart_options['wrapper_class'] = array(
            'etheme-sticky-cart etheme-sticky-panel flex align-items-center container-width-inherit'
        );
        if ($sticky_cart_options['always_shown'])
            $sticky_cart_options['wrapper_class'][] = 'always-shown';
        else
            $sticky_cart_options['wrapper_class'][] = 'outside';
        if ( $ghost )
            $sticky_cart_options['wrapper_class'][] = 'dt-hide mob-hide';
        ?>
        <div class="<?php echo implode(' ', $sticky_cart_options['wrapper_class']); ?>">
            <div class="et-row-container<?php if ($sticky_cart_options['boxed']) { ?> et-container<?php } ?> pos-relative">
                <div class="et-wrap-columns flex align-items-center">
                    <div class="et_column et_col-xs-5 flex-inline align-items-center mob-hide">
                        <?php if ( $sticky_cart_options['show_image'] ) : ?>
                            <a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"
                               class="flex-inline"><?php echo (has_post_thumbnail() ? get_the_post_thumbnail(null, $sticky_cart_options['image_size']) : wc_placeholder_img()); ?></a>
                        <?php endif; ?>
                        <?php if ( $sticky_cart_options['show_title'] ) : ?>
                    ...
                    <div class="pos-static et_column et_col-xs-7 flex-inline align-items-center justify-content-end mob-full-width mob-justify-content-center">
						<?php
						if ( $sticky_cart_options['show_price'] ) {
							woocommerce_template_single_price();
						}

                        if ( $sticky_cart_options['show_button'] ) {
                            if (!function_exists('etheme_is_catalog') || !etheme_is_catalog()) {
                                if ($is_mobile && $product_type != 'variable') {
...
//		add_action( 'woocommerce_external_add_to_cart', 'woocommerce_external_add_to_cart', 30 );
		...
		unset($sticky_cart_enabled);
        unset($sticky_cart_options);
	}
}
...
if ( ! function_exists( 'etheme_single_product_custom_tabs' ) ) {
	function etheme_single_product_custom_tabs( $tabs ) {
		...
		$element_options                                     = array();
		$element_options['product_tabs_sortable']            = apply_filters('etheme_single_product_tabs_list', get_theme_mod( 'product_tabs_sortable',
			array(
				'description',
		...
		$element_options['product_tabs_custom_tab_01_title'] = apply_filters('etheme_single_product_custom_tab_01_title', get_theme_mod( 'product_tabs_custom_tab_01_title_et-desktop', get_theme_mod( 'custom_tab_title', '' ) ));
		$element_options['product_tabs_custom_tab_02_title'] = apply_filters('etheme_single_product_custom_tab_02_title', get_theme_mod( 'product_tabs_custom_tab_02_title_et-desktop', '' ));
		...
		if ( in_array( 'single_custom_tab_01', $element_options['product_tabs_sortable'] ) && $element_options['custom_tab'] ) {
			...
			$tabs['single_custom_tab_01'] = array(
				...
				'callback' => function ( $custom ) use ( $custom_content ) {
                    echo do_shortcode(apply_filters('etheme_single_product_tab_content', $custom_content, 'single_custom_tab_01'));
				}
			);
		}
		...
		foreach ( (array) $element_options['product_tabs_sortable'] as $key ) {
			if ( apply_filters( 'etheme_single_product_builder_tabs_skip', true) && ! isset( $tabs[ $key ] ) ) {
				continue;
			}
...
if ( ! function_exists( 'etheme_single_product_custom_tab_01_content' ) ) {
	...
	function etheme_single_product_custom_tab_01_content() {
		...
		if ( $element_options['product_tabs_custom_tab_01_section_et-desktop'] != '' && function_exists( 'etheme_static_block' ) ) {
			...
			echo do_shortcode( apply_filters('etheme_single_product_custom_tab_01_content_filter', $element_options['product_tabs_custom_tab_01_content_et-desktop']) );
		}
	}
}
...
if ( ! function_exists( 'etheme_single_product_custom_tab_02_content' ) ) {
	...
	function etheme_single_product_custom_tab_02_content() {
		...
		if ( $element_options['product_tabs_custom_tab_02_section_et-desktop'] != '' && function_exists( 'etheme_static_block' ) ) {
			...
			echo do_shortcode( apply_filters('etheme_single_product_custom_tab_02_content_filter', $element_options['product_tabs_custom_tab_02_content_et-desktop']) );
		}
...
//
				...
				if ( $product_reviews && post_type_supports( 'product', 'comments' ) ) {
					...
					if ( ((get_theme_mod( 'product_reviews_collapsed_et-mobile', false ) && get_query_var( 'is_mobile', false )) || (get_theme_mod( 'product_reviews_collapsed_et-desktop', false ) && !get_query_var( 'is_mobile', false ))) && function_exists( 'etheme_comments_template_mobile_button' ) ) {
