			...
			if ( isset( woof()->settings['by_featured']['view'] ) && woof()->settings['by_featured']['view'] === 'switcher' ) {
				...
				<div class="switcher23-container">
					...
					<label for="<?php echo esc_attr( $unique_id ); ?>" class="switcher23-toggle">
						<div class="switcher23-title2"><?php echo esc_html( $woof_ext_featured_label ); ?></div>
						<span></span>                    
					</label>
				</div>
				<?php
				...
				?>
				<input type="checkbox" class="woof_checkbox_featured" id="woof_checkbox_featured" name="product_visibility" value="0" <?php checked( 'featured', woof()->is_isset_in_request_data( 'product_visibility' ) ? 'featured' : '', true ); ?> />&nbsp;&nbsp;<label for="woof_checkbox_featured"><?php echo esc_html( $woof_ext_featured_label ); ?></label><br />
				<?php
			}
			?>
		</div>
