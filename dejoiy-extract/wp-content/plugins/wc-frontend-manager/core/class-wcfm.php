...
class WCFM {
	...
	function init() {
		...
		register_taxonomy(
			...
			apply_filters(
				'wcfm_knowledgebase_category_taxonomy_args',
				array(
					'hierarchical'      => true,
					'show_ui'           => false,
		...
		register_post_status(
			'archived',
			apply_filters(
				'wcfm_product_archive_post_status_args',
				array(
...
<?php
	...
	public function load_plugin_textdomain() {
		...
		load_textdomain('wc-frontend-manager', WP_LANG_DIR . "/plugins/wc-frontend-manager-$locale.mo");
	}
	...
	function restrict_wcfm_vendor_backend() {
		...
		if (is_user_logged_in()) {
			...
			$is_export = false;
			$is_import = false;
			if (isset($_GET['page']) && (wc_clean($_GET['page']) == 'product_exporter')) {
				$is_export = true;
			}
			if (isset($_GET['page']) && (wc_clean($_GET['page']) == 'product_importer')) {
				$is_import = true;
			}
			if (isset($_GET['page']) && (wc_clean($_GET['page']) == 'store-setup')) {
				$is_setup = true;
			}
	...
	static function activate_wcfm() {
		...
		update_option('wcfm_updated_6_6_8', 1);
	...
	static function update_wcfm() {
		...
		if (!get_option('wcfm_updated_6_6_8')) {
			$options = get_option('wcfm_options', array());
			/** By default set the Store Branch Module as disabled */
			$options['module_options']['multi_store'] = 'yes';
			update_option('wcfm_options', $options);
			update_option('wcfm_updated_6_6_8', 1);
		}
	...
	function get_wcfm_menus() {
		$wcfm_menus = apply_filters(
			...
			array(
				...
				'wcfm-coupons' => array(
					'label'      => __('Coupons', 'wc-frontend-manager'),
				...
				'wcfm-orders' => array(
					'label'  => __('Orders', 'wc-frontend-manager'),
					'url'        => get_wcfm_orders_url(),
					'icon'       => 'shopping-cart',
					'priority'   => 35
				),
				'wcfm-reports' => array(
					'label'      => __('Reports', 'wc-frontend-manager'),
					'url'        => get_wcfm_reports_url(),
					'icon'       => 'chart-pie',
					'priority'   => 70
				),
				'wcfm-settings' => array(
					'label'      => __('Settings', 'wc-frontend-manager'),
					'url'        => get_wcfm_settings_url(),
					'icon'       => 'cogs',
					'priority'   => 75
				),
				'wcfm-capability' => array(
					'label'  => __('Capability', 'wc-frontend-manager'),
	...
	function wcfm_create_custom_css() {
		...
		foreach ($color_options as $color_option_key => $color_option) {
			...
			if (substr($wcfm_options[$color_option['name']], 0, 1) !== "#") {
				$wcfm_options[$color_option['name']] = '#' . $wcfm_options[$color_option['name']];
			}

			$custom_color_data .= $color_option['element'] . '{ ' . "\n";
			$custom_color_data .= "\t" . $color_option['style'] . ': ';
			if (isset($wcfm_options[$color_option['name']])) {
				$custom_color_data .= $wcfm_options[$color_option['name']];
			} else {
