...
if ( $show ) :
	...
	if ( $from ) {
		...
		$top_panel_txt .= sprintf( esc_html__( 'from: %s', 'woocommerce-products-filter' ), gmdate( $format_, $from ) );
	}
	if ( $to ) {
		$top_panel_txt .= ' ';
		/* translators: %s: date value */
		$top_panel_txt .= sprintf( esc_html__( 'to: %s', 'woocommerce-products-filter' ), gmdate( $format_, $to ) );
	}
	?>
	...
	?>
				...
				<div class="<?php echo esc_attr( $css_classes ); ?>">
					...
					<div class="woof_meta_datepicker_container">
						...
						<input id="<?php echo esc_attr( $date_id ); ?>_to" data-format="<?php echo esc_html( $format ); ?>" type="text" readonly="readonly" data-meta-key="<?php echo esc_attr( $meta_key ); ?>" class="woof_calendar woof_calendar_to" placeholder="<?php esc_html_e( 'to', 'woocommerce-products-filter' ); ?>" />
