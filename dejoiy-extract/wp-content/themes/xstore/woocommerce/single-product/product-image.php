...
if ( apply_filters('etheme_is_fse_theme', false ) ) {
    wc_get_template( 'single-product/product-image-basic.php' );
    return;
}
...
$product_video_placeholder = etheme_get_option('product_video_placeholder', false);
...
if ( etheme_get_option( 'thumbs_autoheight', '1' ) && (! isset( $etheme_global['quick_view_canvas'] ) || apply_filters('etheme_quick_view_canvas_gallery_autoheight', false))) {
					...
					if ( $video_position == 'first') :
						if ( $external_video ): ?>
                            <?php
                                ...
                                $video_content = '<p>'.esc_html__( 'video', 'xstore' ).'</p>';
                                $video_class = $type_slider;

                                if ($product_video_placeholder){
                                    $external_video_placeholder = etheme_get_external_video_placeholder($external_video);
                                    if ($external_video_placeholder){
                                        $video_content = '<img src="'.$external_video_placeholder.'" alt="Video Placeholder">';
                                        $video_class .= ' with-placeholder';
                                    }
                                }

                                echo sprintf(
                                    '<li class="video-thumbnail thumbnail-item %s">
                                        <a href="#product-video-popup" class="open-video-popup">
                                            <span class="et-icon et-play-button"></span>
                                            <p>%s</p>
                                        </a>
                                    </li>',
                                    esc_attr( $video_class ),
                                    $video_content
                                );
