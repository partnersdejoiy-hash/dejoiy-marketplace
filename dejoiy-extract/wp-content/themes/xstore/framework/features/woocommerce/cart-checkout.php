...
class Etheme_WooCommerce_Cart_Checkout {
	...
	public function disable_advanced_layout_actions() {
		...
		if ( !get_query_var('et_cart-checkout-header-builder', false) ) {
            add_filter('etheme_header_template_basic', '__return_true');
		    $actions_list = array(
			    array(
...
//			),
		...
		if ( !get_query_var('et_cart-checkout-default-footer', false) ) {
            add_filter('etheme_footer_template_basic', '__return_true');
			$actions_list = array_merge( $actions_list, array(
				array(
					'actions' => true,
		...
		if ( get_query_var('et_cart-checkout-layout', 'default') != 'default' && !(get_query_var('et_is-cart-page-elementor-shortcode', false) || get_query_var('et_is-checkout-page-elementor-shortcode', false))) {
			$actions_list[] = array(
...
//        }
	...
	public function header_steps($no_wrapper = false) {
		...
		$texts = apply_filters('et_cart-checkout-nav-texts', array(
			'cart' => esc_html__('Shopping cart', 'xstore'),
			'order' => esc_html__('Order status', 'xstore'),
			'order_received' => esc_html__('Your order has been received', 'xstore'),
			'checkout' => esc_html__('Checkout', 'xstore'),
			'shipping' => esc_html__('Shipping', 'xstore'),
			'payment' => esc_html__('Payment', 'xstore'),
		));

		if ( !$no_wrapper )
			echo '<div class="cart-checkout-nav'.(( get_query_var('et_cart-checkout-breadcrumbs', 'default') != 'default' ) ? '-simple': '').'">';

		if ( $check_pages['is_order'] ) :
			echo '<a class="active"><i class="et-icon et_b-icon et-checked"></i><span>' . $texts['order_received'] . '</span></a>';
        elseif ( get_query_var('et_cart-checkout-breadcrumbs', 'default') == 'default' ) :
			echo '<a href="'. $cart_url .'" class="' . esc_attr($classes['cart']) . '" data-step="1"> ' . $texts['cart'] . '</a>' . $arrow;

			echo '<a href="' . $checkout_url . '" class="'. esc_attr($classes['checkout']) . '" data-step="2"> ' . $texts['checkout'] . '</a>' . $arrow;

			echo '<a href="#" class="'. esc_attr($classes['order']) . '" data-step="3">'. $texts['order'] . '</a>';
		else :

			echo '<a href="'. $cart_url .'" class="' . ($check_pages['is_cart'] ? 'active' : '') . '" data-step="shopping-cart"> ' . $texts['cart'] . '</a>' . $arrow;

			echo '<a href="' . $checkout_url . '" class="' . (($check_pages['is_billing'] && !($check_pages['is_payment'] || $check_pages['is_order_pay'])) ? 'active' : '') . '" '.($check_pages['is_checkout'] ? 'data-step="billing"' : '') . '> ' . esc_html__('Billing details', 'xstore') . '</a>' . $arrow;

			if ( WC()->cart->needs_shipping() )
				echo '<a href="' . add_query_arg('step', 'shipping', $checkout_url) . '" class="' . ($check_pages['is_shipping'] ? 'active' : '') . '"' . ($check_pages['is_checkout'] ? 'data-step="shipping"' : '') . '> ' . $texts['shipping'] . '</a>' . $arrow;

			echo '<a href="' . add_query_arg('step', 'payment', $checkout_url) . '" class="' . (($check_pages['is_payment'] || $check_pages['is_order_pay']) ? 'active' : '') . '"' . ($check_pages['is_checkout'] ? 'data-step="payment"' : '') . '> ' . $texts['payment'] . '</a>' . $arrow;

			echo '<a href="#" class="'. esc_attr($classes['order']) . '" data-step="order">'. $texts['order'] . '</a>';
...
//			etheme_woocommerce_above_checkout_form
        ...
        $is_builder_element = get_query_var('et_is-cart-page-elementor-shortcode', false) || get_query_var('et_is-checkout-page-elementor-shortcode', false);
...
//			add_action('woocommerce_checkout_before_customer_details', array($this, 'sales_booster_countdown_output'), 2);
		...
		if ( get_theme_mod('cart_checkout_advanced_email_first', false) && !$is_builder_element ) {
...
//                if ( isset( $fields['shipping'] ) ) $fields['shipping']['shipping_first_name']['autofocus'] = false;
		...
		if ( get_theme_mod('cart_checkout_advanced_form_label', false) && !$is_builder_element ) {
...
//                }
        ...
        if ( apply_filters('etheme_cart_checkout_advanced_layout_breadcrumbs_style', (get_query_var('et_cart-checkout-layout', 'default') == 'default') ) )
		    etheme_enqueue_style('special-cart-breadcrumbs');

        add_filter('etheme_et_js_config', function ($config) {
            if ( !array_key_exists('advancedCartCheckout', $config) )
                $config['advancedCartCheckout'] = array();
            $config['advancedCartCheckout']['sticky_sidebar'] = get_theme_mod('cart_checkout_advanced_sticky_sidebar', true);
           return $config;
        });
	}
	
	public function etheme_cart_checkout_header_content(){
        $should_wrap_header = get_option( 'etheme_disable_customizer_header_builder', false ) && !has_action('etheme_header', 'etheme_header_dt_wrapper_start');
        if ( $should_wrap_header ) {
            etheme_header_wrapper_start();
            etheme_header_dt_wrapper_start();
        }
        get_template_part( 'templates/woocommerce/cart-checkout/header');
        if ( $should_wrap_header ) {
            etheme_header_dt_wrapper_end();
            etheme_header_wrapper_end();
        }
	}
	...
	public function sales_booster_countdown_output($custom_settings = array()) {
		$countdown = $this->sales_booster_countdown($custom_settings);
		if ( count( $countdown ) ) :
			echo '<'.$countdown['tag'] . ' ' . $countdown['attributes'] . '>' . $countdown['content'] . '</'.$countdown['tag'].'>';
		endif;
	}
	public function sales_booster_countdown($custom_settings = array()) {
        if ( !apply_filters('et_sales_booster_cart_checkout_countdown_enabled', true) ) return array();
        if ( ! function_exists( 'WC' ) || ! property_exists( WC(), 'session' ) || ! is_object( WC()->session ) ) return array();
		...
		if (count($xstore_sales_booster_settings) && isset($xstore_sales_booster_settings['cart_checkout'.$postfix])) {
			$xstore_sales_booster_settings = wp_parse_args($xstore_sales_booster_settings['cart_checkout'.$postfix],
				$xstore_sales_booster_settings_default);
			
			$xstore_sales_booster_settings_cart_checkout_countdown = $xstore_sales_booster_settings;
		}
		
		$xstore_sales_booster_settings_cart_checkout_countdown = wp_parse_args( $custom_settings, array(
            ...
            'tag' => $xstore_sales_booster_settings_cart_checkout_countdown['tag'],
            'force_load_assets' => $xstore_sales_booster_settings_cart_checkout_countdown['force_load_assets']
		));
		...
		if ( $last_time_added ) {
            ...
            $countdown['tag'] = $xstore_sales_booster_settings_cart_checkout_countdown['tag'];
			$attr    = array();
			$classes = array(
            ...
            if ( $xstore_sales_booster_settings_cart_checkout_countdown['force_load_assets'] ) {
                wp_enqueue_script('cart_checkout_countdown');
                etheme_enqueue_style( 'sale-booster-cart-checkout-countdown' );
            }
	...
	public function sales_booster_progress_bar() {
        if ( !apply_filters('et_sales_booster_cart_checkout_progress_bar_enabled', true) ) return;
		...
		if ( count($element_options['xstore_sales_booster_settings']) && isset($element_options['xstore_sales_booster_settings']['cart_checkout'.$postfix])) {
			$element_options['xstore_sales_booster_settings'] = wp_parse_args( $element_options['xstore_sales_booster_settings']['cart_checkout'.$postfix],
		...
		$element_options['cart_options'] = array(
			...
			'progress_bar_cart_ignore_discount' => $element_options['xstore_sales_booster_settings_cart_checkout']['progress_bar_cart_ignore_discount'],
		);
...
//			$amount = WC()->cart->cart_contents_total + WC()->cart->tax_total;
		...
		if ($element_options['cart_options']['progress_bar_cart_ignore_discount'] == 'yes'){
...
//            $amount = WCPay\MultiCurrency\MultiCurrency::instance()->get_price($amount, 'exchange_rate');
            $element_options['cart_options']['booster_progress_price'] = WCPay\MultiCurrency\MultiCurrency::instance()->get_price($element_options['cart_options']['booster_progress_price'], 'exchange_rate');
        }
