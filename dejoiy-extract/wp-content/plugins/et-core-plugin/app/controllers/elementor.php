...
//        });
        ...
        add_action('etheme_mini_cart_fragments', array($this, 'woocommerce_fragments'));
        ...
        add_filter('etheme_elementor_widget_style_depends', array($this, 'modify_widget_styles'), 10, 2);

        add_filter('etheme_elementor_widget_script_depends', array($this, 'modify_widget_scripts'), 10, 2);

        add_action( 'elementor/frontend/after_enqueue_scripts', array($this, 'change_loading_scripts'), 100);

        add_action( 'elementor/frontend/after_enqueue_styles', array($this, 'change_loading_styles'), 100);
    ...
    public function add_system_status_data( $response, $system_status, $request ) {
        // If response is not what we expect, just return it untouched.
            ...
            case 'theme-etheme_cart':
                if ( isset($_POST['nonce_type']) && $_POST['nonce_type'] == 'cart' ) {
                    ...
                    $filters = array(
                            ...
                            'etheme_cart_after_footer_content_shown' => '__return_false'
                    );
                    if ($linked_products) {
                    ...
                    if ( !$empty_cart ) {
                        $show_view_page = true;
                        $show_view_page_extra = true;
                        $footer_content = '';
                        if (isset($element['settings']['show_view_page']))
                            $show_view_page = !!$element['settings']['show_view_page'];
                        if (isset($element['settings']['show_view_page_extra']))
                            $show_view_page_extra = !!$element['settings']['show_view_page_extra'];
                        if (isset($element['settings']['footer_content']) && !!$element['settings']['footer_content'])
                            $footer_content = $element['settings']['footer_content'];
                        $local_settings = array(
                                'show_view_page' => $show_view_page,
                                'show_view_page_extra' => $show_view_page_extra,
                                'footer_content' => $footer_content
                        );
                        ob_start();
                        $cart->render_main_content_prefooter_content($local_settings);
                        $fragment_data['html'][] = ob_get_clean();
                        $fragment_data['selector'][] = 'div.elementor-element-' . $element['id'] . ' div.etheme-elementor-off-canvas-content-prefooter-inner';
                    }

                    $product_count = WC()->cart->get_cart_contents_count();
                    ob_start();
                    $cart->render_icon_qty($product_count);
                    $fragment_data['html'][] = ob_get_clean();
                    $fragment_data['selector'][] = 'div.elementor-element-' . $element['id'] . ' .elementor-button-icon-qty';
                    ob_start();
                    $cart->render_subtotal();
                    $fragment_data['html'][] = ob_get_clean();
                    $fragment_data['selector'][] = 'div.elementor-element-' . $element['id'] . ' .etheme-elementor-off-canvas-total-inner';
...
//            $cart_fragments[$cart_widget_selector . ' div.widget_shopping_cart_content'] = '<div class="widget_shopping_cart_content">' . ob_get_clean() . '</div>';
    ...
    public function modify_widget_styles($styles, $is_woocommerce = false) {
        global $etheme_el_styles;
        if ( !is_array($etheme_el_styles) )
            $etheme_el_styles = [];
        if ( apply_filters('etheme_should_enqueue_global_styles', false ) ) {
            $etheme_el_styles = array_unique(array_merge($etheme_el_styles, $styles));
            $styles = ['etheme-elementor-all-widgets-style', 'etheme-general-all-style', 'elementor-icons-fa-solid', 'elementor-icons-fa-regular', 'elementor-icons-fa-brands'];
            if ( $is_woocommerce ) {
                $styles[] = 'etheme-woocommerce-all-style';
            }
        }
        return $styles;
    }

    public function modify_widget_scripts($scripts, $is_woocommerce = false) {
        global $etheme_el_scripts;
        if ( !is_array($etheme_el_scripts) )
            $etheme_el_scripts = [];
        if ( apply_filters('etheme_should_enqueue_global_scripts', false) ) {
            $etheme_el_scripts = array_unique(array_merge($etheme_el_scripts, $scripts));
            $scripts = ['etheme_general_all_js', 'etheme_elementor_all_widgets_js'];
            if ( $is_woocommerce ) {
                $scripts[] = 'etheme_woocommerce_all_scripts';
            }
        }
        return $scripts;
    }
    ...
    /**
     * Disable all styles that are collected in editor mode for each widget
     *
     * @return void
     */
    public function change_loading_styles() {
        global $etheme_el_styles;

        if (isset($etheme_el_styles) && count($etheme_el_styles) ) {
            $styles_skipper = [ 'etheme-elementor-all-widgets-style', 'etheme-general-all-style', 'etheme-woocommerce-all-style' ];
            foreach ($etheme_el_styles as $etheme_el_style) {
                if ( in_array($etheme_el_style, $styles_skipper) ) continue;
                wp_dequeue_style($etheme_el_style);
            }
        }
    }

    /**
     * Disable all scripts that are collected in editor mode for each widget
     *
     * @return void
     */
    public function change_loading_scripts() {
        global $etheme_el_scripts;
        if (isset($etheme_el_scripts) && count($etheme_el_scripts) ) {
            $scripts_skipper = ['etheme_general_all_js', 'etheme_elementor_all_widgets_js', 'etheme_woocommerce_all_scripts'];
            foreach ($etheme_el_scripts as $etheme_el_script) {
                if ( in_array($etheme_el_script, $scripts_skipper) ) continue;
                wp_dequeue_script($etheme_el_script);
            }
        }
    }
...
//			    }
    ...
    public function enqueue_editor_styles() {
        ...
        wp_add_inline_style( 'et-core-elementor-style', '
            ...
            .elementor-editor-active:not(.elementor-editor-single-post) .elementor-panel .elementor-element:has(.et-elementor-post-widget-icon-only) .eight_theme-elementor-icon:not(.eight_theme-elementor-deprecated, .et-elementor-product-widget-icon-only):after,
            .elementor-editor-active:not(.elementor-editor-product) .elementor-panel .elementor-element:has(.et-elementor-product-widget-icon-only) .eight_theme-elementor-icon:not(.eight_theme-elementor-deprecated, .et-elementor-post-widget-icon-only):after,
            ...
            .elementor-editor-active:not(.elementor-editor-product-archive) .elementor-panel .elementor-element-wrapper:has(.et-elementor-product-builder-widget-icon-only), 
            .elementor-editor-active:not(.elementor-editor-archive) .elementor-panel .elementor-element-wrapper:has(.et-elementor-archive-builder-widget-icon-only), 
            .elementor-editor-active:not(.elementor-editor-header) .elementor-panel .elementor-element-wrapper:has(.et-elementor-header-builder-widget-icon-only), 
            .elementor-editor-active:not(.elementor-editor-single-post) .elementor-panel .elementor-element-wrapper:has(.et-elementor-post-widget-icon-only:not(.et-elementor-product-widget-icon-only)), 
            .elementor-editor-active:not(.elementor-editor-product) .elementor-panel .elementor-element-wrapper:has(.et-elementor-product-widget-icon-only:not(.et-elementor-post-widget-icon-only)), 
            .elementor-editor-active:not(.elementor-editor-search-results) .elementor-panel .elementor-element-wrapper:has(.et-elementor-search-results-widget-icon-only) {
                display: none;
            }');
...
//        }
    ...
    public function get_location_widgets_assets($location = 'header', $edit_mode = false) {
    ...
    public function enqueue_frontend_styles() {
        ...
        foreach ($locations_2_check as $location) {
            if ( $this->check_location_be_loaded($location) ) {
                $this->get_location_widgets_assets($location, $is_edit || $is_preview);
            }
        }
    ...
    public function enqueue_scripts_hide_key($url){
        ...
        wp_register_script(
                'etheme-google-map',
                ET_CORE_URL . 'app/assets/js/google-map.js',
                array( apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general') ),
                ET_CORE_VERSION,
                true
        );
    }
