...
/**
 * Nexus flow active on this request (query flag or cart session on library/cart/checkout).
 *
 * @return bool
 */
function dejoiy_library_is_nexus_flow_request() {
	...
	if ( ! dejoiy_library_has_nexus_flow_session() ) {
		return false;
	}
	...
	if ( did_action( 'wp' ) && function_exists( 'is_checkout' ) && is_checkout() ) {
		if ( is_wc_endpoint_url( 'order-pay' ) || is_wc_endpoint_url( 'order-received' ) ) {
			...
			if ( function_exists( 'dejoiy_library_order_has_nexus_items' ) && function_exists( 'wc_get_order' ) ) {
...
/**
 * Remember Nexus flow in the cart session (survives cart/checkout URLs without query args).
 */
function dejoiy_library_set_nexus_flow_session() {
	if ( ! function_exists( 'WC' ) || ! WC()->session ) {
		return;
	}
	WC()->session->set( 'dejoiy_library_flow', '1' );
}

/**
 * @return bool
 */
function dejoiy_library_has_nexus_flow_session() {
	return function_exists( 'WC' )
		&& WC()->session
		&& '1' === (string) WC()->session->get( 'dejoiy_library_flow' );
}

/**
 * Persist ?dejoiy_library=1 into the cart session early in the request.
 */
function dejoiy_library_capture_nexus_flow_session() {
	if ( isset( $_GET[ DEJOIY_LIBRARY_FLOW ] ) && '1' === (string) $_GET[ DEJOIY_LIBRARY_FLOW ] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
add_action( 'woocommerce_init', 'dejoiy_library_capture_nexus_flow_session', 20 );
...
/**
 * Detect WooCommerce cart page without calling is_cart() (safe inside WC conditional filters).
 *
 * @return bool
 */
function dejoiy_library_is_wc_cart_page_raw() {
...
/**
 * Detect WooCommerce checkout page without calling is_checkout() (avoids woocommerce_is_checkout recursion).
 *
 * @return bool
 */
function dejoiy_library_is_wc_checkout_page_raw() {
	if ( function_exists( 'is_wc_endpoint_url' ) && is_wc_endpoint_url() ) {
		return false;
	}
	if ( ! function_exists( 'wc_get_page_id' ) ) {
...
function dejoiy_library_use_cart_template() {
	return dejoiy_library_is_wc_cart_page_raw()
		&& function_exists( 'dejoiy_library_is_nexus_flow_request' )
		&& dejoiy_library_is_nexus_flow_request();
}
...
function dejoiy_library_use_checkout_template() {
	return dejoiy_library_is_wc_checkout_page_raw()
		&& function_exists( 'dejoiy_library_is_nexus_flow_request' )
		&& dejoiy_library_is_nexus_flow_request();
}
...
function dejoiy_library_get_cart_url() {
	...
	if ( function_exists( 'dejoiy_channel_strip_cart_query_args' ) ) {
		$url = dejoiy_channel_strip_cart_query_args( $url );
	}
...
function dejoiy_library_handle_nexus_add_to_cart() {
	...
	if ( function_exists( 'dejoiy_library_ensure_nexus_product_sellable' ) ) {
		if ( ! dejoiy_library_ensure_nexus_product_sellable( $product_id ) ) {
			wc_add_notice(
				__( 'This book must be published in WooCommerce before it can be sold on Nexus. Open the product in your vendor dashboard, set a title and price, then publish.', 'dejoiy' ),
				'error'
			);
...
/**
 * Append unique Nexus book hits from posts.
 *
 * @param WP_Post[]                     $posts    Posts.
 * @param int                           $limit    Max items.
 * @param array<int, array<string,mixed>> $books   Books list (by ref).
 * @param array<int, bool>              $seen_ids Seen product IDs (by ref).
 */
function dejoiy_library_search_append_posts( $posts, $limit, &$books, &$seen_ids ) {
	foreach ( $posts as $p ) {
		$id = $p instanceof WP_Post ? (int) $p->ID : (int) $p;
		...
		$books[]         = array(
			...
			'url'    => dejoiy_library_product_url( $id ),
...
/**
 * WP_Query WHERE: match title, excerpt, or content by keyword (partial words).
 *
 * @param string   $where WHERE clause.
 * @param WP_Query $query Query.
 * @return string
 */
function dejoiy_library_posts_where_nexus_keyword( $where, $query ) {
	$term = $query->get( 'dejoiy_library_keyword' );
	if ( ! is_string( $term ) || '' === trim( $term ) || 'product' !== $query->get( 'post_type' ) ) {
		return $where;
	}

	global $wpdb;
	$parts = preg_split( '/\s+/u', strtolower( trim( $term ) ) );
	$likes = array();
	...
	return $where . ' AND (' . implode( ' OR ', $likes ) . ')';
}

/**
 * Query Nexus books by title / excerpt keyword (works with DEJOIY Library tax_query).
 *
 * @param string $term  Search string.
 * @param int    $limit Max results.
 * @return WP_Query
 */
function dejoiy_library_query_books_by_keyword( $term, $limit = 24 ) {
	add_filter( 'posts_where', 'dejoiy_library_posts_where_nexus_keyword', 10, 2 );
	$q = dejoiy_library_query_books(
		array(
			'posts_per_page'         => $limit,
			'dejoiy_library_keyword' => $term,
			'orderby'                => 'relevance',
		)
	);
	remove_filter( 'posts_where', 'dejoiy_library_posts_where_nexus_keyword', 10 );
	return $q;
}

/**
 * Query Nexus books by author meta or SKU.
 *
 * @param string $term  Search string.
 * @param int    $limit Max results.
 * @return WP_Query
 */
function dejoiy_library_query_books_by_meta( $term, $limit = 24 ) {
	...
	return dejoiy_library_query_books(
		array(
			'posts_per_page' => $limit,
...
function dejoiy_library_search_nexus_books( $term, $limit = 24 ) {
	...
	$passes = array(
		dejoiy_library_query_books(
			array(
				's'              => $term,
				'posts_per_page' => $limit,
			)
		),
		dejoiy_library_query_books_by_keyword( $term, $limit ),
		dejoiy_library_query_books_by_meta( $term, $limit ),
	);

	foreach ( $passes as $q ) {
		if ( $q instanceof WP_Query && ! empty( $q->posts ) ) {
			dejoiy_library_search_append_posts( $q->posts, $limit, $books, $seen_ids );
		}
		wp_reset_postdata();
		if ( count( $books ) >= $limit ) {
			return $books;
	...
	// Typo tolerance: pehchaan → pehchan (drop last character per word).
	if ( empty( $books ) ) {
		...
		foreach ( $words as $w ) {
			$w = trim( (string) $w );
			if ( strlen( $w ) > 4 ) {
				$relaxed_words[] = substr( $w, 0, -1 );
			}
		}
		if ( ! empty( $relaxed_words ) ) {
			$relaxed_term = implode( ' ', $relaxed_words );
			if ( $relaxed_term !== $term ) {
				$q3 = dejoiy_library_query_books_by_keyword( $relaxed_term, $limit );
				if ( ! empty( $q3->posts ) ) {
					dejoiy_library_search_append_posts( $q3->posts, $limit, $books, $seen_ids );
				}
				wp_reset_postdata();
			}
		}
	...
	// Fallback: keyword category paths when title search returns nothing.
	$map = array(
	...
	wp_send_json_success(
		array(
			'message' => $books
				...
				: __( 'No Nexus books matched — try another topic or creator name.', 'dejoiy' ),
			'books'   => $books,
			'term'    => $term,
			'home'    => add_query_arg(
				array(
					DEJOIY_LIBRARY_FLOW => '1',
					'dlu_q'             => rawurlencode( $term ),
				),
				dejoiy_library_get_landing_url()
			),
		)
...
function dejoiy_library_filter_is_checkout( $is_checkout ) {
	...
	if ( dejoiy_library_request_has_flow_flag() && dejoiy_library_is_wc_checkout_page_raw() ) {
		return true;
	}
	return $is_checkout;
}
