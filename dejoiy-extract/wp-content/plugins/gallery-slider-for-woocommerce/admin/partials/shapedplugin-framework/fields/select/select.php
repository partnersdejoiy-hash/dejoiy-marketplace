		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				if ( ! empty( $args['ajax'] ) ) {
					...
					$args['settings']['data']['nonce'] = wp_create_nonce( 'wcgs_chosen_ajax_nonce' );
				...
				$chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' wcgs--sortable' : '';
				$chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' wcgs-chosen-ajax' : '';
				$placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '"' : '';
				$field_class      = ( $args['chosen'] ) ? ' class="wcgs-chosen' . esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) . '"' : '';
				...
				if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
					...
					if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
						...
						echo '<select name="' . esc_attr( $field_name ). '" class="wcgs-hide-select hidden"' . $multiple_attr . $field_attr . '>';// phpcs:ignore
					...
					echo ! empty( $this->field['empty_message'] ) ? wp_kses_post( $this->field['empty_message'] ) : esc_html__( 'No data available.', 'gallery-slider-for-woocommerce' );
				}
			}
