...
(function () {
  ...
  function renderNexusSearchResults(res, out) {
    ...
    if (books.length) {
      ...
      books.forEach(function (b) {
        html += '<a href="' + (b.url || '#') + '"><img src="' + (b.cover || '') + '" alt="" loading="lazy" /><span>' + (b.title || '') + '</span></a>';
      });
      html += '</div>';
      if (res.data.home) {
  ...
  function fetchLiveSearch(term, panel) {
    ...
    var sep = url.indexOf('?') >= 0 ? '&' : '?';
    fetch(url + sep + 'q=' + encodeURIComponent(term), { credentials: 'same-origin' })
      .then(function (r) { return r.json(); })
  ...
  function renderCartPanelBody(data) {
    ...
    items.forEach(function (item) {
      ...
      html += '<span class="dlu-cart-panel-price">' + (item.price || '') + (item.qty > 1 ? ' × ' + item.qty : '') + '</span>';
    ...
    body.querySelectorAll('.dlu-cart-panel-remove').forEach(function (link) {
      link.addEventListener('click', function (e) {
        if (link.href && link.href.indexOf('remove_item=') !== -1) {
          e.preventDefault();
    ...
    if (foot && count > 0) {
      ...
      if (checkoutUrl) {
        foot.innerHTML = '<a href="' + checkoutUrl + '" class="dlu-btn-primary dlu-cart-checkout-btn">Complete Your Collection</a>' +
          '<a href="' + (data.cart_url || CFG.cart_url || '#') + '" class="dlu-btn-secondary">View full shelf</a>';
      }
