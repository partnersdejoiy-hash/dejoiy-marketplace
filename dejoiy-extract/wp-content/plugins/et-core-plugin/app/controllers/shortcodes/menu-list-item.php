    ...
    function menu_list_item_shortcode( $atts ) {
       ...
       // create output css 
        ...
        if ( !$atts['is_elementor'] ) {
            ...
            if ( count(explode(',', $atts['link'][0])) !== count(explode(',', $atts['link'][1])) ) return;

            if (
                empty($atts['link']) ||
                !is_array($atts['link']) ||
                !isset($atts['link'][0], $atts['link'][1]) ||
                $atts['link'][0] === null ||
                $atts['link'][1] === null
            ) {
                return;
            ...
            $link0 = trim((string)$atts['link'][0]);
            $link1 = trim((string)$atts['link'][1]);

            if ($link0 === '' || $link1 === '') {
                return;
            }

            $link0_parts = explode(',', $link0);
            $link1_parts = explode(',', $link1);

            if (count($link0_parts) !== count($link1_parts)) {
                return;
            }

            $atts['link'] = array_combine($link0_parts, $link1_parts);
