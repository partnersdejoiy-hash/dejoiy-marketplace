...
class Custom_Product_Categories_Masonry extends \Elementor\Widget_Base {
	...
	public function get_style_depends() {
		return apply_filters('etheme_elementor_widget_style_depends', ['etheme-elementor-categories', 'etheme-elementor-custom-masonry'], true);
	}
...
//		);
        ...
        $repeater->add_control(
            'product_category_slug', [
                ...
                'label_block' => 'true',
                'multiple'    => false,
                'options'     => Elementor::get_terms( 'product_cat', false, false, true ),
            ]
        );
			...
			<?php foreach ( $tabs as $tab ) {
                ...
                <div class="et-masonry-item grid-<?php echo $i; ?>" data-hover="<?php echo $image_hover; ?>"
					...
					if ( ! $skeleton && ((isset($tab[ $post_type ]) && $tab[ $post_type ]) || (isset($tab[ $post_type . '_slug']) && !!$tab[ $post_type . '_slug'])) ) {
                        $by_slug = isset($tab[ $post_type . '_slug']) && !!$tab[ $post_type . '_slug'];
                        $slug_or_id = $by_slug ? $tab[ $post_type . '_slug'] : $tab[ $post_type ];

                        $category     = get_term_by( ($by_slug ? 'slug' : 'id'), $slug_or_id, 'product_cat' );
						$thumbnail_id = is_object($category) && property_exists( $category, 'term_id' ) ? get_term_meta( $category->term_id, 'thumbnail_id', true ) : false;
					...
					$content       = array();
					$extra_content = array();
					if ( ! $skeleton && (isset($tab[ $post_type ]) || !!$tab[ $post_type . '_slug']) && is_object($category) ) {
