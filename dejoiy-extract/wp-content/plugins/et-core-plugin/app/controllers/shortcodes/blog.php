    ...
    function blog_shortcode( $atts, $content ) {
	    ...
	    if ( function_exists( 'etheme_enqueue_style' ) ) {
            ...
            switch ( $atts['blog_layout'] ) {
                ...
                case 'timeline':
                case 'timeline2':
                    etheme_enqueue_style( 'post-timeline', true );
                    break;
                case 'small':
                case 'chess':
                    ...
                    break;
                default:
                    etheme_enqueue_style( 'post-' . $atts['blog_layout'], true );
                    break;
            }
        }
		...
		if ( in_array( $et_loop['blog_layout'], array( 'timeline', 'grid', 'small') ) ) 
			$et_loop['columns'] = 1;
		...
		if ( $options['wp_query']->max_num_pages > 0 ) {
			...
			if(!get_theme_mod( 'disable_pjax_navigation', false ) ){
				$options['pagination_args']['class'] .= ' pjax-pagination'; 
			}
        ...
        if ( $atts['is_preview'] ) {
        	echo '<script>jQuery(document).ready(function(){ etTheme.global_image_lazy();}); </script>';
        }
