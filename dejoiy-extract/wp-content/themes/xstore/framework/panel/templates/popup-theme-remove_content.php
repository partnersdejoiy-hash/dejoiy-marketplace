...
$is_embedded_remove_content = ! empty( $is_embedded_remove_content );
$remove_content_wrapper_classes = $is_embedded_remove_content
    ? 'et_popup-content-remove et_popup-content-remove-embedded'
    : 'et_panel-popup-inner with-scroll et_popup-content-remove';

?>
<div class="<?php echo esc_attr( $remove_content_wrapper_classes ); ?>">
    <?php // echo '<div class="image-block">'.$settings['logo'].'</div>' ?>
			<div class="steps-block-content">
                ...
                <form class="et_remove_content_form">
                    ...
                    <span class="remove-content-block">
                        <input type="checkbox" id="et_remove-testimonials" name="et_remove-content" value="testimonials">
                        <label for="et_remove-testimonials"><?php esc_html_e('Testimonials', 'xstore');?> (<?php echo isset($et_imported_data['testimonials']) ? count($et_imported_data['testimonials']) : '0';?>)</label>
                    </span>
                    <span class="remove-content-block">
                    ...
                    <?php
                        $gutenberg_templates_total = 0;
                        if ( isset( $et_imported_data['gutenberg_html_templates'] ) && is_array( $et_imported_data['gutenberg_html_templates'] ) ) {
	                        $gutenberg_templates_total = count( $et_imported_data['gutenberg_html_templates'] );
                        }
                    ?>
                    <span class="remove-content-block">
                        <input type="checkbox" id="et_remove-gutenberg_html_templates" name="et_remove-content" value="gutenberg_html_templates">
                        <label for="et_remove-gutenberg_html_templates"><?php esc_html_e('Gutenberg files', 'xstore');?> (<?php echo esc_html($gutenberg_templates_total);?>)</label>
                    </span>

                    <?php
                        $gutenberg_posts_total = 0;
                        $gutenberg_post_keys = array(
                            'gutenberg_patterns',
                            'gutenberg_template_parts',
                            'gutenberg_styles',
                            'gutenberg_navigation',
                            'gutenberg_templates',
                        );
                        foreach ( $gutenberg_post_keys as $key ) {
                            if ( isset( $et_imported_data[ $key ] ) && is_array( $et_imported_data[ $key ] ) ) {
                                $gutenberg_posts_total += count( $et_imported_data[ $key ] );
                            }
                        }
                    ?>
                    <span class="remove-content-block">
                        <input type="checkbox" id="et_remove-gutenberg_templates" name="et_remove-content" value="gutenberg_templates">
                        <label for="et_remove-gutenberg_templates"><?php esc_html_e('Gutenberg templates', 'xstore');?> (<?php echo esc_html($gutenberg_posts_total);?>)</label>
                    </span>
