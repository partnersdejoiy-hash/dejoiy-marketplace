...
class WOOF_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			__CLASS__,
			esc_html__( 'HUSKY - Products Filter Professional for WooCommerce', 'woocommerce-products-filter' ),
			array(
				'classname'   => __CLASS__,
		...
		<div class="widget widget-woof">
			...
			if ( ! empty( $instance['title'] ) ) {
				...
				if ( isset( $args['before_title'] ) ) {
					...
					?>
					<<?php echo esc_attr( apply_filters( 'woof_widget_title_tag', 'h3' ) ); ?> class="widget-title"><?php echo esc_html( $instance['title'] ); ?></<?php echo esc_attr( apply_filters( 'woof_widget_title_tag', 'h3' ) ); ?>>
					<?php
				}
			}
			?>
