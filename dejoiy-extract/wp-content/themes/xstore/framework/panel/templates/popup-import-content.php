...
if(isset($_POST['version'])){
    ...
    $engines = isset( $versions[ $_POST['version'] ]['engine'] ) && is_array( $versions[ $_POST['version'] ]['engine'] )
        ? $versions[ $_POST['version'] ]['engine']
        : array( 'wpb' );
    $is_multi_engine = count( $engines ) > 1;

    $requested_engine = isset( $_POST['engine'] ) ? sanitize_key( wp_unslash( $_POST['engine'] ) ) : '';
    if ( in_array( $requested_engine, array( 'wpb', 'elementor', 'gutenberg', 'elementor_builders' ), true ) ) {
        $selected_engine = true;
        $data_engine_default = $requested_engine;
    }
    elseif ( in_array( 'elementor_builders', $engines, true ) ) {
        $data_engine_default = 'elementor_builders';
    }
    elseif ( in_array( 'elementor', $engines, true ) ) {
        $data_engine_default = 'elementor';
    } elseif ( isset( $engines[0] ) ) {
        $data_engine_default = $engines[0];
    } else {
        $data_engine_default = 'wpb';
    }

    if ( ! $is_multi_engine && ! $selected_engine ) {
        $selected_engine = true;
        $requested_engine = $data_engine_default;
    }

    $version_import_data = function_exists( 'etheme_get_demo_import_data' )
        && $selected_engine
        ? etheme_get_demo_import_data( $_POST['version'], $requested_engine )
        : array();
...
$is_multi_engine = isset( $is_multi_engine ) ? $is_multi_engine : ( isset( $engines ) && is_array( $engines ) && count( $engines ) > 1 );
$install_button_engine = ( $selected_engine || ! $is_multi_engine ) ? $data_engine_default : '';
$elementor_engine_value = ( in_array( 'elementor_builders', $engines, true ) && ! in_array( 'elementor', $engines, true ) )
    ? 'elementor_builders'
    : 'elementor';
...
<!--                <br/><h3 class="et_step-title">--><?php //echo sprintf(esc_html__('Welcome to %s Demo Setup Wizard', 'xstore'), $brand_title); ?><!--</h3>-->
	    ...
	    <?php // step remove_content ?>
		    <?php if (in_array('remove_content', $_POST['steps']) ): ?>
			    <?php
	                ...
	                $is_remove = false;
	                $et_imported_data = is_array( $et_imported_data ) ? $et_imported_data : array();

	                if ( count( $et_imported_data ) ) {
	                    foreach ( $et_imported_data as $type ) {
	                        if ( is_countable( $type ) && count( $type ) ) {
	                            $is_remove = true;
	                        }
	                    }
	                }
		    ...
		    <?php if ($is_remove): ?>
                <div class="et_popup-step et_step-remove_content text-left <?php echo esc_attr($classes['et_step-remove_content']); ?>">
                    ...
                    <div class="text-center">
                        ...
                        <span type="submit" class="et-button-arrow et_skip-remove_content">
                ...
                <div class="et_popup-step et_step-remove_content_form text-left hidden">
                    <br/>
                    <?php
                        $is_embedded_remove_content = true;
                        include locate_template( 'framework/panel/templates/popup-theme-remove_content.php', false, false );
                        unset( $is_embedded_remove_content );
                    ?>
                    <p class="text-center">
                        <span type="button" class="et-button-arrow et_skip-remove_content">
                            <?php esc_html_e('Skip this step', 'xstore'); ?>
                            <svg class="arrow-icon" xmlns="http://www.w3.org/2000/svg" width="1.3em" height="1.3em" viewBox="0 0 32 32">
                              <g fill="none" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round" stroke-miterlimit="10">
                                <circle class="arrow-icon--circle" cx="16" cy="16" r="15.12"></circle>
        ...
        <?php if ( in_array('child_theme', $_POST['steps']) ): ?>
            ...
            <div class="et_popup-step et_step-child_theme text-left <?php echo esc_attr($classes['et_step-child_theme']); ?>">
                ...
                <form id="et_create-child_theme-form" action="" method="POST">
                    ...
                    <div class="text-center">
                        <p>
                            <button type="submit" id="et_create-child_theme" class="et-button et-button-arrow" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
        ...
        <?php // step engine ?>
        <?php if ( in_array('engine', $_POST['steps']) && $is_multi_engine ): ?>
            <div class="et_popup-step et_step-engine <?php echo esc_attr($classes['et_step-engine']); ?>">
                ...
                <?php if(in_array('elementor', $engines) || in_array('elementor_builders', $engines)): ?>   
                    <input
                        type="radio"
                        id="<?php echo esc_attr( $elementor_engine_value ); ?>"
                        name="engine"
                        value="<?php echo esc_attr( $elementor_engine_value ); ?>"
                        <?php checked( $data_engine_default, $elementor_engine_value ); ?>
                    >
                    <label class="engine-selector text-center <?php echo esc_attr( $data_engine_default === $elementor_engine_value ? 'active' : '' ); ?>" for="<?php echo esc_attr( $elementor_engine_value ); ?>">
                        <svg height="50px" width="40px" version="1.1" viewBox="0 0 512 512" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><g id="_x31_09-elementor"><g><path d="M462.999,26.001H49c-12.731,0-22.998,10.268-22.998,23v413.998c0,12.732,10.267,23,22.998,23    h413.999c12.732,0,22.999-10.268,22.999-23V49.001C485.998,36.269,475.731,26.001,462.999,26.001" style="fill:#D63362;"/><rect height="204.329" style="fill:#FFFFFF;" width="40.865" x="153.836" y="153.836"/><rect height="40.866" style="fill:#FFFFFF;" width="122.7" x="235.566" y="317.299"/><rect height="40.865" style="fill:#FFFFFF;" width="122.7" x="235.566" y="235.566"/><rect height="40.865" style="fill:#FFFFFF;" width="122.7" x="235.566" y="153.733"/></g></g><g id="Layer_1"/></svg>
                        <span class="engine-title">Elementor</span>
                    </label>
                <?php endif; ?>

                <?php if(in_array('gutenberg', $engines)): ?>   
                    <input type="radio" id="gutenberg" name="engine" value="gutenberg" <?php checked( $data_engine_default, 'gutenberg' ); ?>>
                    <label class="engine-selector text-center <?php echo esc_attr( $data_engine_default === 'gutenberg' ? 'active' : '' ); ?>" for="gutenberg">
                        <svg width="40px" height="50px" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M16 0C17.1046 0 18 0.895431 18 2V16C18 17.1046 17.1046 18 16 18H2C0.895431 18 8.05332e-09 17.1046 0 16V2C0 0.895431 0.895431 8.05319e-09 2 0H16ZM8.33887 3C7.03942 3 5.99382 3.43432 5.19824 4.29395C4.44545 5.09721 4.05294 6.15472 4.00781 7.45312L4 7.72656L4.00781 10.2656H4C4.00005 11.7006 4.40301 12.8492 5.19824 13.7061C5.99382 14.563 7.04207 15 8.33887 15C9.43126 15 10.333 14.7379 11.0596 14.2051C11.3759 13.9816 11.6445 13.6951 11.8486 13.3643C12.0527 13.0335 12.1878 12.6643 12.2471 12.2793L12.2744 9.44043C13.0911 9.24222 14.1991 8.79216 14.8965 7.72656C14.9379 7.67529 14.9687 7.61574 14.9854 7.55176C15.002 7.48781 15.0042 7.42064 14.9932 7.35547C14.9821 7.29041 14.9576 7.22846 14.9209 7.17383C14.8841 7.11909 14.8362 7.07254 14.7803 7.03809C14.6726 6.96535 14.5403 6.93878 14.4131 6.96387C14.2861 6.98897 14.1742 7.06383 14.1016 7.17188C13.2163 8.52083 11.3373 8.59172 11.2373 8.5918H11.1924C8.87459 8.5918 7.98858 10.5898 7.9541 10.6729C7.92182 10.7482 7.90826 10.8303 7.91504 10.9121C7.92183 10.994 7.94887 11.0727 7.99316 11.1416C8.0375 11.2105 8.09763 11.2681 8.16895 11.3076C8.24035 11.3472 8.32091 11.3684 8.40234 11.3691C8.58793 11.3691 8.76812 11.2585 8.84766 11.0684C8.86251 11.04 9.47154 9.67595 11.0332 9.58789V12.1426C11.0075 12.4058 10.9238 12.6599 10.7891 12.8867C10.6542 13.1136 10.4709 13.3082 10.2529 13.4551C9.7783 13.7896 9.14464 13.9609 8.36523 13.9609C7.43713 13.9609 6.67353 13.6341 6.11133 12.9941C5.53851 12.3514 5.25196 11.441 5.25195 10.2627L5.25977 7.44531C5.30486 6.40638 5.5834 5.58449 6.1084 4.99805C6.68387 4.35533 7.4344 4.03125 8.36523 4.03125C9.1447 4.03127 9.77829 4.2025 10.2529 4.53711C10.7276 4.87186 10.9959 5.3325 11.041 5.92969V5.99121C11.041 6.15668 11.1058 6.3156 11.2217 6.43262C11.3376 6.54963 11.4953 6.61523 11.6592 6.61523C11.8229 6.61513 11.9799 6.5495 12.0957 6.43262C12.2116 6.3156 12.2764 6.1567 12.2764 5.99121V5.92676C12.2416 5.50261 12.1142 5.09115 11.9033 4.72266C11.6924 4.35423 11.4032 4.0371 11.0566 3.79492C10.3328 3.25953 9.43117 3.00005 8.33887 3Z" fill="currentColor"/>
                        </svg>

                        <span class="engine-title">Gutenberg</span>
                    </label>
                <?php endif; ?>

                <?php if(in_array('wpb', $engines)): ?>  
                    <input type="radio" id="wpb" name="engine" value="wpb" <?php checked( $data_engine_default, 'wpb' ); ?>>
                    <label class="engine-selector text-center <?php echo esc_attr( $data_engine_default === 'wpb' ? 'active' : '' ); ?>" for="wpb">
                        <svg width="40px" height="50px" viewBox="0 0 66 50" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <defs></defs>
        ...
        <?php //if ( ! $is_multi_engine ): ?>
        <?php // remove later ?>
        <?php if (false): ?>
            <input type="radio" name="engine" value="<?php echo esc_attr( $data_engine_default ); ?>" checked style="display:none;">
        <?php endif; ?>
        ...
        <?php if ( in_array('plugins', $_POST['steps']) ): ?>
            <?php if ( $selected_engine ) : ?>
                <?php
                    ...
                    $plugins = $plugins->get_popup_plugin_list($_POST['version'], $data_engine_default);// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
                 ?>
                <?php if ( count( $plugins ) ): ?>
                    ...
                    <ul class="et_popup-import-plugins with-scroll">
                        ...
                        <?php foreach ($plugins as $key => $value): ?>
                            <li class="et_popup-import-plugin flex justify-content-between align-items-center install-with-all selected-to-install move-right">
                                <?php
            ...
            <?php else : ?>
                <div class="et_popup-step et_step-plugins hidden">
                <span class="hidden" data-engine-content="pending"></span>
                </div>
            <?php endif; ?>
        <?php endif ?>
                ...
                <form class="et_install-demo-form with-scroll" action="">
                    ...
                    <div class="et_hidden-setup hidden">
                        ...
                        <?php if ( isset( $to_import['elementor_globals'] ) && ! empty( $to_import['elementor_globals'] ) ): // not working by our code removes ?>
                            <input type="checkbox" id="elementor_globals" name="elementor_globals" value="elementor_globals" checked>
                            <label for="elementor_globals"><?php echo esc_html__('Elementor globals', 'xstore'); ?></label>
        ...
        <?php if(!isset($_POST['navigation']) || $_POST['navigation'] !== 'false' ) : ?>
            <div class="et_popup-import-navigation">
                ...
                <span class="et_navigate-install et-button et-button-arrow text-center hidden" data-version="<?php echo esc_attr( $_POST['version'] ); ?>" data-engine-default="<?php echo esc_attr( $install_button_engine ); ?>">
    ...
    <style>
        .et_popup-step.et_step-engine.active.loading:before{
            content: '';
            width: 100%;
            height: 100%;
            z-index: 9999999;
    ...
    <script>
        (function($) {
            var $popup = $(document).find('.et_popup-import-content');

            if (!$popup.length) {
                return;
            }
            var activeFilter = $(document).find('.et-filters-builders .engine-filter.active').attr('data-filter');

            if (!activeFilter || activeFilter === 'all') {
                return;
            }
            var targetEngine = activeFilter;
            if (activeFilter === 'elementor') {
                if ($popup.find('input[name="engine"][value="elementor"]').length) {
                    targetEngine = 'elementor';
                } else if ($popup.find('input[name="engine"][value="elementor_builders"]').length) {
                    targetEngine = 'elementor_builders';
                }
            }

            var $engineInput = $popup.find('input[name="engine"][value="' + targetEngine + '"]');
            if (!$engineInput.length || $engineInput.is(':checked')) {
                return;
            }
            $engineInput.prop('checked', true).trigger('change');
        })(jQuery);
    </script>
</div>
