			...
			switch ( $type ) {
					...
					<select name="woof_settings[<?php echo esc_attr( $key ); ?>]" id="<?php echo esc_attr( $key ); ?>">
						...
						if ( ! empty( $select_options ) ) {
							foreach ( $select_options as $opt_key => $opt_title ) {
								?>
								<option <?php selected( $woof_settings[ $key ], $opt_key ); ?> value="<?php echo esc_attr( $opt_key ); ?>"><?php echo esc_html( $opt_title ); ?></option>
								<?php
							}
						}
						?>
				...
				case 'image':
					?>
						...
						<a href="#" class="woof-button woof_select_image"><?php echo esc_html( $placeholder ); ?></a>                    
					<?php
					break;
