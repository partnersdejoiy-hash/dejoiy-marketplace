...
final class CEI_Core {
	...
	{
//		load_plugin_textdomain( 'xstore', false, basename( CEI_PLUGIN_DIR ) . '/lang/' );
	}
	...
	static public function controls_enqueue_scripts()
	{
		...
		wp_localize_script( 'cei-js', 'CEIl10n', array(
			'emptyImport'	=> __( 'Please choose a file to import.', 'xstore-core' )
		));
	...
	static public function register( $wp_customize )
	{
		...
		$wp_customize->add_section( 'cei-section', array(
			'title'	   => __( 'Export/Import', 'xstore-core' ),
			'priority' => 10000000
		));
	...
	static private function _export( $wp_customize )
	{
		...
		if ( class_exists('Kirki') ) {
			$fields = Kirki::$fields;
			$mods_options = 'options';
			if ( 'theme_mod' === Kirki::$config[ 'et_kirki_options' ]['option_type'] ) {
				$mods_options = 'mods';
			}
			foreach ( $fields as $key => $value ) {
				if ( $value['type'] == 'kirki-custom' ) continue;
				$data[$mods_options][ $key ] = Kirki::get_option($key);
			}
		}
		
		$theme_security_keys = array(
			'facebook_app_id',
			'facebook_app_secret',
			'google_app_id',
			'google_app_secret',
		...
		foreach ( $theme_security_keys as $theme_security_key ) {
			$data['mods'][$theme_security_key] = '';
		}
	...
	static private function _import( $wp_customize )
	{
		...
		if ( ! file_exists( $file['file'] ) ) {
			$cei_error = __( 'Error importing settings! Please try again.', 'xstore-core' );
			return;
		}

		// Get the upload data.
		$raw  = file_get_contents( $file['file'] ); // Legal request to self hosted file
		...
		if ( 'array' != gettype( $data ) ) {
			$cei_error = __( 'Error importing settings! Please check that you uploaded a customizer export file.', 'xstore-core' );
			return;
		}
		...
		if ( $data['template'] != $template ) {
			$cei_error = __( 'Error importing settings! The settings you uploaded are not for the current theme.', 'xstore-core' );
			return;
		}
