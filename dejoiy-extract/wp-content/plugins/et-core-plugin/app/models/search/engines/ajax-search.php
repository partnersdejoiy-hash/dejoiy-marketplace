<?php
namespace ETC\App\Models\Search\Engines;

use ETC\App\Models\Search\Analytics\Recorder;
 ...
 * @log     1.0.3
 * FIXED: product variations if parent is draft
 * @log     1.0.4
 * ADDED: Search analytics recorder
 */
...
//		);
		...
		if ( etheme_get_option( 'search_product_variation_et-desktop', 0 ) ) {
			...
			$args['post_parent__not_in'] = etheme_get_draft_products_ids();
	...
	public function search_results() {
        ...
        if ( Multilingual::is_multilingual() ) {
            $lang = Multilingual::get_current_language();
        }
		...
		foreach ( $search_options as $key => $value ) {
			...
			if ( in_array( $value, array( 'products', 'product' ) ) ) {
				...
				if ( $search_sku ) {
                    ...
                    $found_products_ids = array_column($post_types_results[ $value ], 'ID');
                    $sku_products_ids = array_column($sku_products, 'ID');
                    // unset the duplications of same found products by ID
                    foreach (array_intersect($sku_products_ids, $found_products_ids) as $same_product_id) {
                        $found_key = array_search($same_product_id, $sku_products_ids);
                        unset($sku_products[$found_key]);
                    }
					$post_types_results[ $value ] = array_merge($post_types_results[ $value ], $sku_products);
				}

                do_action(
                    'etheme_ajax_search_stats/analytics/after_searching',
                    $query,
                    count($post_types_results[ $value ]),
                    'product',
                    $lang
                );
...
//				}
				...
				$product_args = array(
					...
					'img'       => ( $product_image ? $product_image : wc_placeholder_img_src( 'woocommerce_thumbnail' ) ),
					'arrow'     => $arrow,
...
//					'date'  => '',
				...
				if ( isset( $this->request['product_category'] ) ) {
					$product_subcategory_path = false;
					if (isset($this->request['product_subcategory_path'])) {
						$product_subcategory_path = $this->request['product_subcategory_path'];
					}
					$product_category = $this->get_category($product->get_ID(), 'product_cat', !!$product_subcategory_path);
					if ( $product_category ) {
						$product_args = array_merge($product_args, array(
...
//						$args['excerpt'] = ob_get_clean();
    ...
    public function get_category($productID, $taxonomy, $term_parents_list = false) {
        $category = array(
            'title' => false,
            'url' => false
        );
        $has_category = false;
        if ( $taxonomy == 'product_cat' && function_exists('etheme_get_custom_field')) {
            $primary_cat = etheme_get_custom_field('primary_category', $productID);
            if (!empty($primary_cat) && $primary_cat != 'auto') {
                ...
                if ( ! is_wp_error( $primary_cat ) ) {
                    ...
                    if ( ! is_wp_error( $primary_cat_link ) ) {
                        $category['title'] = $primary_cat->name;
                        $category['url'] = esc_url( $primary_cat_link );
                        if ( $term_parents_list ) {
                            $category['term_path'] = $this->get_category_path($primary_cat);
                            $category['title'] = $category['term_path'] . $category['title'];
                        }
                        $has_category = true;
                    }
                }
        ...
        if ( !$has_category ) {
            $product_cats = get_the_terms( $productID, $taxonomy );
            ...
            foreach ( $product_cats as $product_cat ) {
                $link = get_term_link( $product_cat, $taxonomy );
                if ( is_wp_error( $link ) ) {
                    continue;
                }
                $category['title'] = $product_cat->name;
                $category['url'] = esc_url( $link );
                if ( $term_parents_list ) {
                    $category['term_path'] = $this->get_category_path($product_cat);
                    $category['title'] = $category['term_path'] . $category['title'];
                }
                $has_category = true;
                break;
        ...
        if ( $has_category && !isset( $this->request['full_screen'] ) ) {
            $category['title'] = sprintf(esc_html__('Category: %s', 'xstore-core'), $category['title']);
        }
        return $has_category ? json_encode($category) : false;
    }

    public function get_category_path($category) {
        $separator = is_rtl() ? '<' : '>';
        $cat_path = get_term_parents_list($category->term_id, $category->taxonomy, array(
            'separator' => ' '.$separator.' ',
            'link' => false,
            'format' => 'name',
            'inclusive' => false,
        ...
        return trim($cat_path, $separator); // the trim() removes the trailing/last slash
    }
	/**
	 * Parse suggestions
	 *
