 ...
 */
class Product_Upsells extends \Elementor\Widget_Base {
    ...
    public function get_name() {
        return 'woocommerce-product-etheme_upsells';
    }
    ...
    public function get_title() {
        return __( 'Upsells Products', 'xstore-core' );
    }
    ...
    public function get_keywords() {
        return [ 'woocommerce', 'slider', 'shop', 'store', 'upsell', 'product' ];
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .upsell-products-title' => 'color: {{VALUE}}',
                ],
            ]
        );
        ...
        $this->add_group_control(
            ...
            [
                ...
                'selector' => '{{WRAPPER}} .upsell-products-title',
            ]
        );
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .upsell-products-title' => 'text-align: {{VALUE}}',
                ],
            ]
        );
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'size_units' => [ 'px', 'em', 'rem', 'custom' ],
                'selectors' => [
                    '{{WRAPPER}} .upsell-products-title' => 'margin-bottom: {{SIZE}}{{UNIT}}',
                ],
            ]
        );
    ...
    protected function render() {
        ...
        $upsells = wc_products_array_orderby( array_filter( array_map( 'wc_get_product', $product->get_upsell_ids() ), 'wc_products_array_filter_visible' ), $settings['orderby'], $settings['order'] );
        $upsells = $settings['limit'] > 0 ? array_slice( $upsells, 0, $settings['limit'] ) : $upsells;

        if ( !count($upsells) ) return;
        ...
        if ( $products && $products->have_posts() ) {
            ...
            if ( !!$settings['show_heading'] ) {
                echo '<'.$settings['html_heading_tag'].' class="products-title upsell-products-title"><span>' .
                    apply_filters( 'woocommerce_product_upsell_products_heading', !empty($settings['heading_text']) ? $settings['heading_text'] : esc_html__( 'You may also like...', 'xstore-core' ) ) .
...
//	    if ( 'yes' === get_option( 'woocommerce_hide_out_of_stock_items' ) ) {
    ...
    public function product_loop_start_filter($html) {
        $class = 'products elementor-grid products-loop products-grid upsells-products';
