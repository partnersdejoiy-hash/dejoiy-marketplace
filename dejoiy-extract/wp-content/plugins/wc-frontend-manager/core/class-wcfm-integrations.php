...
class WCFM_Integrations {
	...
	public function __construct() {
		...
		if (apply_filters('wcfm_is_allow_woo_product_size_chart', true)) {
			if (WCFM_Dependencies::wcfm_woo_product_size_chart_plugin_active_check()) {
				add_action('wp_enqueue_scripts', array($this, 'wcfm_add_scfw_enqueue_scripts'));
  ...
  /**
	 ...
	 */
	function wcfm_wcrental_product_manage_fields( $product_id = 0, $product_type = null ) {
		global $WCFM;
	 ...
	 * @since 3.0.4
	 */
	function wcfm_yithauction_free_product_manage_fields( $product_id = 0, $product_type = null ) {
		global $WCFM;
   ...
   */
	...
	/**
	 * 	Size chart for WooCommerce script
	 */
	public function wcfm_add_scfw_enqueue_scripts() {
		global $WCFM;

		wp_enqueue_style('wcfm_scfw_css', $WCFM->library->css_lib_url . 'products-manager/wcfm-script-scfw-products-manage.css', array(), $WCFM->version);
		wp_enqueue_script('wcfm_scfw_js', $WCFM->library->js_lib_url . 'products-manager/wcfm-script-scfw-products-manage.js', array('jquery', 'select2_js'), $WCFM->version, true);
		wp_localize_script('wcfm_scfw_js', 'wcfm_scfw_js_object', apply_filters('wcfm_scfw_js_object', [
			'l10n' => [
				'placeholder' => __('Type the size chart name', 'size-chart-for-woocommerce')
			],
			'select2Args' => [
				'container_css_class' 	=> 'wcfm_scfw_prod_chart',
				'minimum_input_length'	=> 3,
				'ajax' 					=> [
					'url'		=> admin_url('admin-ajax.php'),
					'action'	=> 'size_chart_search_chart',
					'security'	=> wp_create_nonce( 'size_chart_search_nonce' ),
					'dataType'	=> 'json',
					'delay'		=> 1000
				]
			],
	 ...
	 */
	public function wcfm_woo_product_size_chart_product_manage_views($product_id) {
		global $WCFM;
		...
		if ($product_id) {
			$chart_id = scfw_size_chart_get_product( $product_id );
			$chart_id = (is_array($chart_id)) ? $chart_id : [$chart_id];
			
			foreach( $chart_id as $chart_val ) {
				$chart_list[esc_attr( $chart_val )] = esc_html( get_the_title( $chart_val ) );
			}
		}

		$WCFM->wcfm_fields->wcfm_generate_form_field(apply_filters('wcfm_woo_product_size_chart_fields', array(
			"wcfm_prod_chart" => array(
				'label' 		=> __('Search/Select Size Chart', 'size-chart-for-woocommerce'), 
				'type' 			=> 'select', 
				'options' 		=> $chart_list, 
				'class' 		=> 'wcfm-select wcfm_ele wcfm_full_ele catalog_visibility_ele simple variable external grouped booking ', 
