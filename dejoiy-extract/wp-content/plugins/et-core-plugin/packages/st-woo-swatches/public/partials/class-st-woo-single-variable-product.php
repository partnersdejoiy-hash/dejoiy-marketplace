...
class St_Woo_Single_Variable_Product extends St_Woo_Swatches_Base {
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			foreach( array_keys( $attributes ) as $taxonomy ) {
				...
				if( array_key_exists( $attribute_type,  $this->attribute_types ) ) {
					...
					$use_product_editor_order = apply_filters(
						'sten_wc_single_swatch_use_product_editor_variation_order',
						false,
						$product,
						$taxonomy,
						$available_options
					);
					if ( $use_product_editor_order ) {
						$terms = $this->sort_terms_by_product_editor_order( $product, $terms, $taxonomy, $available_options );
					}
					...
					foreach( $terms as $term ) {
						...
						if ( in_array( $term->slug, $variations, true ) ) {
							...
							if (!$image){
								...
								$image = '<img src="'.apply_filters('et_sten_placeholder_url',$this->plugin_public_dir_path_url . 'images/placeholder.png' ) . '"/>';
							}
	...
	/**
	 * Sort taxonomy terms by variation options order from product editor.
	 */
	public function sort_terms_by_available_options( $terms, $available_options ) {
		if ( empty( $terms ) || empty( $available_options ) || ! is_array( $terms ) || ! is_array( $available_options ) ) {
			return $terms;
		}

		$order_map = array();
		foreach ( $available_options as $index => $slug ) {
			$order_map[ (string) $slug ] = (int) $index;
		}

		usort(
			$terms,
			function( $left, $right ) use ( $order_map ) {
				$left_order  = array_key_exists( $left->slug, $order_map ) ? $order_map[ $left->slug ] : PHP_INT_MAX;
				$right_order = array_key_exists( $right->slug, $order_map ) ? $order_map[ $right->slug ] : PHP_INT_MAX;
	...
	/**
	 * Sort taxonomy terms by variation rows order from product editor.
	 */
	public function sort_terms_by_product_editor_order( $product, $terms, $taxonomy, $available_options ) {
		...
		if ( is_array( $variation_ids ) && ! empty( $variation_ids ) ) {
			foreach ( $variation_ids as $variation_id ) {
				$slug = (string) get_post_meta( $variation_id, $meta_key, true );
				if ( '' !== $slug && ! in_array( $slug, $order_slugs, true ) ) {
					$order_slugs[] = $slug;
				}
			}
		}
		...
		if ( is_array( $available_options ) && ! empty( $available_options ) ) {
			foreach ( $available_options as $slug ) {
				$slug = (string) $slug;
				if ( '' !== $slug && ! in_array( $slug, $order_slugs, true ) ) {
					$order_slugs[] = $slug;
				}
		...
		return $this->sort_terms_by_available_options( $terms, $order_slugs );
	}
}
