...
function dejoiy_ecosystem_native_product_permalink( $product_id ) {
	...
	if ( function_exists( 'dejoiy_marketplace_filter_product_permalink' ) ) {
...
/**
 * WooCommerce single product URL for purchase (add to cart / checkout).
 *
 * @param int $product_id Product ID.
 * @return string
 */
function dejoiy_product_purchase_url( $product_id ) {
	$product_id = (int) $product_id;
	if ( function_exists( 'dejoiy_marketplace_product_uses_pretty_urls' ) && dejoiy_marketplace_product_uses_pretty_urls( $product_id ) ) {
		if ( function_exists( 'dejoiy_marketplace_product_url' ) ) {
			$url = dejoiy_marketplace_product_url( $product_id );
			if ( $url ) {
				return $url;
...
/**
 * Use native product permalinks in listing contexts (home, shop, recommendations).
 *
 * @return bool
 */
function dejoiy_ecosystem_use_purchase_permalinks() {
...
function dejoiy_ecosystem_filter_permalink( $permalink, $product ) {
	...
	if ( function_exists( 'dejoiy_ecosystem_use_purchase_permalinks' ) && dejoiy_ecosystem_use_purchase_permalinks() ) {
		$native = dejoiy_ecosystem_native_product_permalink( $product->get_id() );
		return $native ? $native : $permalink;
	}
