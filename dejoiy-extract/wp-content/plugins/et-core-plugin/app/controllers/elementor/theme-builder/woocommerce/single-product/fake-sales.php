 ...
 */
class Fake_Sales extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'woocommerce-product-etheme_sales_booster_fake_sales';
	}
	...
	public function get_keywords() {
        return [ 'woocommerce', 'shop', 'store', 'sale', 'customer', 'payment', 'order', 'booster', 'product' ];
	}
...
//    }
	...
	protected function register_controls() {
        $activated_option = get_option('xstore_sales_booster_settings_fake_product_sales');
            ...
            $this->add_control(
                ...
                [
                    'raw' => sprintf(esc_html__('To use this widget, please, activate %1s within the %2s section.', 'xstore-core'),
                        '<a href="' . add_query_arg('etheme-sales-booster-tab', 'fake_product_sales', admin_url('admin.php?page=et-panel-sales-booster')) . '" target="_blank">' . esc_html__('Fake product sales', 'xstore-core') . '</a>',
                        '<a href="' . add_query_arg('etheme-sales-booster-tab', 'fake_product_sales', admin_url('admin.php?page=et-panel-sales-booster')) . '" target="_blank">' . esc_html__('Sales Booster', 'xstore-core') . '</a>'),
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .sales-booster-total-sales' => 'text-align: {{VALUE}};',
                ],
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => '',
                'selectors' => [
                    '{{WRAPPER}} .sales-booster-total-sales' => 'color: {{VALUE}};',
                ],
            ]
        );
        ...
        $this->add_group_control(
            ...
            [
                'name' => 'typography',
                'selector' => '{{WRAPPER}} .sales-booster-total-sales',
            ]
        );
        ...
        if ( !get_option('xstore_sales_booster_settings_fake_product_sales') ) {
            if ( $edit_mode ) {
                echo Elementor::elementor_frontend_alert_message(esc_html__('This message is shown only in edit mode.', 'xstore-core') . '<br/>' . sprintf(esc_html__('To use this widget, please, activate %1s within the %2s section.', 'xstore-core'),
                        '<a href="' . add_query_arg('etheme-sales-booster-tab', 'fake_product_sales', admin_url('admin.php?page=et-panel-sales-booster')) . '" target="_blank"><strong>' . esc_html__('Fake product sales', 'xstore-core') . '</strong></a>',
                        '<a href="' . add_query_arg('etheme-sales-booster-tab', 'fake_product_sales', admin_url('admin.php?page=et-panel-sales-booster')) . '" target="_blank"><strong>' . esc_html__('Sales Booster', 'xstore-core') . '</strong></a>'));
            }
            return;
        }
        $settings = $this->get_settings_for_display();

		$this->add_render_attribute( 'wrapper', 'class', 'sales-booster-total-sales' );
