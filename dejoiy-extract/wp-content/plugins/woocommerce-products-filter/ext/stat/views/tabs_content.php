			...
			<section id="woof-stat-1">
				...
				<div class="woof-control-section">
					...
					<?php if ( ! empty( $stat_min_date ) ) : ?>
						<div class="woof_fix4">
							<?php
							/* translators: 1: month name, 2: year number */
							printf( esc_html__( '(Statistic collected from: %1$s %2$d)', 'woocommerce-products-filter' ), esc_html( $wp_locale->get_month( $stat_min_date[1] ) ), esc_html( $stat_min_date[0] ) )
							?>
						</div>
					<?php endif; ?>
					<br />
							...
							<div class="select-wrap">
								<select name="woof_settings[woof_stat][is_enabled]" class="chosen_select">
									<?php foreach ( $stat_activated_mode as $key => $value ) : ?>
										<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $is_enabled == $key ); ?>><?php echo esc_html( $value ); ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						...
						<div class="woof-control">
							...
							<div class="select-wrap">
								<select multiple="" name="woof_settings[woof_stat][items_for_stat][]" class="chosen_select">
									<?php foreach ( $all_items as $key => $value ) : ?>
										<option value="<?php echo esc_attr( $key ); ?>" <?php selected( in_array( $key, $items_for_stat ) ); ?>><?php echo esc_html( $value ); ?></option>
									<?php endforeach; ?>
								</select>
							</div>
							...
							<div class="select-wrap">
								<select name="woof_settings[woof_stat][cron_system]" class="chosen_select woof_cron_system">
									<?php foreach ( $cron_systems as $key => $value ) : ?>
										<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $cron_system == $key ); ?>><?php echo esc_html( $value ); ?></option>
									<?php endforeach; ?>
								</select>
							</div>
							...
							<div class="select-wrap">
								<select name="woof_settings[woof_stat][wp_cron_period]" class="chosen_select">
									<?php foreach ( $wp_cron_periods as $key => $value ) : ?>
										<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $wp_cron_period == $key ); ?>><?php echo esc_html( $value ); ?></option>
									<?php endforeach; ?>
								</select>
							</div>
