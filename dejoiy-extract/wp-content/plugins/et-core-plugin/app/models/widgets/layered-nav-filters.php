...
class Layered_Nav_Filters extends WC_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $filter_cat ){
			$base_link = add_query_arg( 'filter_cat', $filter_cat, $base_link );
		}
		...
		if (
            ...
            if ($clear){
                ...
                echo '<li class="chosen"><a rel="nofollow noopener" class="etheme-ajax-filter etheme-clear-all-filters" href="' . $url . '"><span>' . esc_html__('Clear All', 'xstore-core') . '</span></a></li>';
            }
			...
			if ( ! empty( $_chosen_attributes ) ) {
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					...
					foreach ( $data['terms'] as $term_slug ) {
						...
						$links[] = $link;
						echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . esc_html( $term->name ) . '</a></li>';
					}
				}
			}
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Min %s', 'xstore-core' ), wc_price( $min_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}
			if ( $max_price ) {
				$link = remove_query_arg( 'max_price', $base_link );
				/* translators: %s: maximum price */
				echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Max %s', 'xstore-core' ), wc_price( $max_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}
			if ( ! empty( $rating_filter ) ) {
				foreach ( $rating_filter as $rating ) {
					...
					echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Rated %s out of 5', 'xstore-core' ), esc_html( $rating ) ) . '</a></li>';
				}
			}
			...
			if ($filter_stock){
				switch ($filter_stock) {
					case 'in_stock':
						...
						echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . $strings['in_stock'] . '</a></li>';

						break;
					case 'out_of_stock':
						$link = remove_query_arg( 'stock_status', $base_link );
						echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . $strings['out_of_stock'] . '</a></li>';
						break;
					case 'out_of_stock,in_stock':
						...
						echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $in_stock ) . '">' . $strings['in_stock'] . '</a></li>';
						echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $out_of_stock ) . '">' . $strings['out_of_stock'] . '</a></li>';
			...
			if ( $filter_sale ) {
				...
				echo '<li class="chosen"><a rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . esc_html__( 'On sale', 'xstore-core' ) . '</a></li>';
			}
			...
			if ( $filter_brand ) {
				...
				foreach ( $brands as $key => $value ) {
					...
					unset($all[array_search($value, $brands)]);
					...
					if ( $term && ! is_wp_error( $term ) ) {
						/* translators: %s: maximum price */
						echo '<li class="chosen"><a class="remove-brand" rel="nofollow noopener" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . $term->name . '</a></li>'; // WPCS: XSS ok.
					}
				}
			}
			...
			// ! Use for cat
			if ( $filter_cat ) {
				$cats = explode(',', $filter_cat);
				$link = remove_query_arg( 'filter_cat', $base_link );

				foreach ( $cats as $key => $value ) {
					$all = $cats;

					unset($all[array_search($value, $cats)]);

					if ( $all ) {
						$link = add_query_arg( 'filter_cat', implode( ',', $all ), $link );
					}
					...
					if ( $term && ! is_wp_error( $term ) ) {
						/* translators: %s: maximum price */
						echo '<li class="chosen"><a rel="nofollow noopener" rel="nofollow" aria-label="' . $strings['remove_filter'] . '" href="' . esc_url( $link ) . '">' . $term->name . '</a></li>'; // WPCS: XSS ok.
					}
				}
			}
