...
class Popup_Search extends \ETC\App\Controllers\Elementor\General\Search {
    ...
    public function get_script_depends() {
        return apply_filters('etheme_elementor_widget_script_depends', array_merge(parent::get_script_depends(), [ 'etheme_modal_popup' ]), true);
    }
    ...
    public function get_style_depends() {
        return apply_filters('etheme_elementor_widget_style_depends', array_merge(parent::get_style_depends(), [ 'etheme-elementor-modal-popup' ]));
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'product_categories_button_text',
            [
                'label' => __( 'Button Text', 'xstore-core' ),
                'type' => \Elementor\Controls_Manager::TEXT,
                'default' => __( 'View all categories', 'xstore-core' ),
                'condition' => [
                    'popup_additional_content' => 'product_categories',
                ]
            ]
        );

        $this->add_control(
            'product_categories_button_link',
            [
                'label' => __( 'Button Link', 'xstore-core' ),
                'type' => \Elementor\Controls_Manager::TEXT,
                'default' => esc_url( class_exists('WooCommerce') ? wc_get_page_permalink( 'shop' ) : home_url() ),
                'condition' => [
                    'popup_additional_content' => 'product_categories',
                ]
            ]
        ...
        switch ( $settings['popup_additional_content'] ) {
            case 'product_categories':
                if ( class_exists('WooCommerce') ) {
                    ...
                    $link = (isset($settings['product_categories_button_link'])) ? $settings['product_categories_button_link'] : wc_get_page_permalink('shop');
...
//                $ready_content .= do_shortcode('[product_categories limit=5 columns=5]');
                    ...
                    $ready_content .= '<a href="' . esc_url($link) . '" class="btn black big">';
                    $ready_content .= (isset($settings['product_categories_button_text']) ) ? $settings['product_categories_button_text'] : esc_html__('View all categories', 'xstore-core');
                    $ready_content .= '</a>';
