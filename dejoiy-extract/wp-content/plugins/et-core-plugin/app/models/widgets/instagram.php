...
class Instagram extends Widgets {
    ...
    function __construct() {
        ...
        if ( is_admin() ) {
            add_action( 'admin_footer-widgets.php', array( $this, 'print_admin_scripts' ) );
            add_action( 'admin_footer-customize.php', array( $this, 'print_admin_scripts' ) );
        }
    }
    ...
    function widget( $args, $instance ) {
        ...
        $img_source = empty( $instance['img_source'] ) ? 'instagram' : $instance['img_source'];
        $images_link = empty( $instance['images_link'] ) ? '' : $instance['images_link'];
        $likes_custom_count = empty( $instance['likes_custom_count'] ) ? '' : $instance['likes_custom_count'];
        $comments_custom_count = empty( $instance['comments_custom_count'] ) ? '' : $instance['comments_custom_count'];
        ...
        // meta args
        $show_likes = isset( $instance['show_likes'] )
            ? ! empty( $instance['show_likes'] )
            : ! empty( $instance['like_count'] );
        $show_comments = isset( $instance['show_comments'] )
            ? ! empty( $instance['show_comments'] )
            : ! empty( $instance['comments_count'] );
	    $hide_empty_meta = ( empty( $instance['hide_empty_meta'] ) ) ? false : $instance['hide_empty_meta'];
        ...
        $is_elementor              = (!isset($instance['is_elementor']) || empty( $instance['is_elementor'] )) ? false : true;
        ...
        if ( ! $user && $img_source == 'instagram') {
            ...
            if($img_source == 'instagram'){
                ...
                $likes    = explode( '-', $likes_custom_count );
                $comments = explode( '-', $comments_custom_count );
                foreach ($gallery as $value) {
                    $image = wp_get_attachment_image_src( $value, $size );

                    $instagram['data'][] = array(
                        'media_url' => $image[0],
                        'permalink' => $images_link,
                        'media_type' => 'image',
                        'like_count'    => isset($likes[1]) ? wp_rand( $likes[0], $likes[1] ) : $likes[0],
                        'comments_count'    => isset($comments[1]) ? wp_rand( $comments[0], $comments[1] ) : $comments[0],
                    );
                }
            }
...
//                                if(! $slider &&  $img_type == 'squared'){
//                                    $img_style = 'position: relative; height: 0px;';
//                                } else {
//                                    $img_style = 'visibility: hidden; opacity: 0; position: absolute;';
//                                }
                                ...
                                if ( $this->API_type == 'old' || $this->force_tag ) {
                                    ...
                                    if ( $this->account_type == 'PERSONAL' ) {
                                        ...
                                        printf(
                                            ...
                                            ( $slider ) ? 'class="swiper-slide"' : '',
                                            (isset($item['permalink'])) ? esc_url( $item['permalink'] ) : '',
                                            esc_attr( $target ),
                                            'et-image-type-'.$img_type,
                                            $style,
                                            ( $slider ) ? etheme_loader( false, 'swiper-lazy-preloader' ) : '',
	                                        ...
	                                        $comments,
                                            isset($item['album_count']) ? $item['album_count'] . ' ' . _nx( 'item', 'items', $item['album_count'], 'instagram album count text', 'xstore-core' ) : '',
                                            (isset($item['media_type'])) ? ( strtolower( $item['media_type'] ) ) : ''
                                        );
                                    }
    ...
    function form( $instance ) {
        $instance = wp_parse_args( (array) $instance, array(
            ...
            'ajax'        => false,
            'img_source'  => 'instagram', 
            'gallery'        => '',
            'images_link'    => '',
            'likes_custom_count'    => '',
            'comments_custom_count'  => '',
            'show_likes'     => false,
            'show_comments'  => false,
        ) );
        ...
        $tag_type   = isset( $instance['tag_type'] ) ? esc_attr( $instance['tag_type'] ) : 'recent_media';
        $ajax       = isset( $instance['ajax'] ) ? (bool) $instance['ajax'] : false;

        $img_source  = isset( $instance['img_source'] ) ? esc_attr( $instance['img_source'] ) : 'instagram';
        $gallery_ids = isset( $instance['gallery'] ) ? preg_replace( '/[^0-9,]/', '', $instance['gallery'] ) : '';

        $ids = array_filter( array_map( 'absint', array_map( 'trim', explode( ',', $gallery_ids ) ) ) );

        $images_link   = esc_attr( $instance['images_link'] ?? '' );
        $likes_custom_count   = esc_attr( $instance['likes_custom_count'] ?? '' );
        $comments_custom_count = esc_attr( $instance['comments_custom_count'] ?? '' );

        $show_likes    = ( ! isset( $instance['show_likes'] ) ) ? false : (bool) $instance['show_likes'];
        $show_comments = ( ! isset( $instance['show_comments'] ) ) ? false : (bool) $instance['show_comments'];
        ?>

        <p>
            <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title', 'xstore-core' ); ?>:
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'img_source' ); ?>"><?php esc_html_e( 'Image Source', 'xstore-core' ); ?>:</label>
            <select id="<?php echo $this->get_field_id( 'img_source' ); ?>"
                    name="<?php echo $this->get_field_name( 'img_source' ); ?>" class="widefat et-img-source">
                <option value="instagram" <?php selected( 'instagram', $img_source ); ?>>Instagram</option>
                <option value="gallery"   <?php selected( 'gallery',   $img_source ); ?>>Image gallery</option>
            </select>
        </p>

        <?php
        $wrap_id   = $this->get_field_id( 'gallery_wrap' );
        $input_id  = $this->get_field_id( 'gallery' );
        $input_name= $this->get_field_name( 'gallery' );
        $is_gallery= ( $img_source === 'gallery' );
        ?>
        <div id="<?php echo esc_attr( $wrap_id ); ?>_container"
             class="et-gallery-container"
             style="margin:8px 0; <?php echo $is_gallery ? '' : 'display:none;'; ?>">

            <div class="et-gallery-thumbs" id="<?php echo esc_attr( $wrap_id ); ?>"
                ...
                <?php if ( $ids ) : ?>
                    <?php foreach ( $ids as $id ) :
                        ...
                        $thumb = $thumb ? $thumb[0] : '';
                        ?>
                        <div class="et-gallery-item" data-id="<?php echo esc_attr( $id ); ?>"
                             style="position:relative;width:90px;height:90px;border:1px solid #ccd0d4;background:#fff;padding:2px;box-sizing:border-box">
                            <span class="et-remove"
                                  style="position:absolute;top:-7px;right:-7px;background:#d63638;color:#fff;width:22px;height:22px;border-radius:50%;line-height:22px;text-align:center;font-weight:bold;cursor:pointer">×</span>
                            <img src="<?php echo esc_url( $thumb ); ?>" alt="" style="width:100%;height:100%;object-fit:cover;display:block">
                        </div>
                    <?php endforeach; ?>
                <?php else : ?>
                    <span class="et-empty" style="color:#757575"><?php esc_html_e( 'No images selected', 'xstore-core' ); ?></span>
                <?php endif; ?>
            </div>
            ...
            <p style="margin-top:8px;">
                ...
                <button type="button" class="button et-select-gallery"
                        data-wrap="#<?php echo esc_attr( $wrap_id ); ?>"
                        data-input="#<?php echo esc_attr( $input_id ); ?>">
                    <?php esc_html_e( 'Select images', 'xstore-core' ); ?>
                </button>
                <button type="button" class="button et-clear-gallery"
                        data-wrap="#<?php echo esc_attr( $wrap_id ); ?>"
                        data-input="#<?php echo esc_attr( $input_id ); ?>"
                    <?php if ( empty( $ids ) ) echo 'style="display:none"'; ?>>
                    <?php esc_html_e( 'Clear', 'xstore-core' ); ?>
                </button>
            </p>

            <p>
                <label for="<?php echo $this->get_field_id( 'images_link' ); ?>">
                    <?php esc_html_e( 'Images link', 'xstore-core' ); ?>:
                    <input class="widefat"
                           id="<?php echo $this->get_field_id( 'images_link' ); ?>"
                           name="<?php echo $this->get_field_name( 'images_link' ); ?>"
                           type="text"
                           value="<?php echo $images_link; ?>"/>
                </label>
            </p>

            <p class="likes_custom_count-block" style="display: <?php echo ($show_likes) ? 'block': 'none';?>;">
                <label for="<?php echo $this->get_field_id( 'likes_custom_count' ); ?>">
                    <?php esc_html_e( 'Likes count', 'xstore-core' ); ?>:
                    <input class="widefat"
                           id="<?php echo $this->get_field_id( 'likes_custom_count' ); ?>"
                           name="<?php echo $this->get_field_name( 'likes_custom_count' ); ?>"
                           type="text"
                           placeholder="10-100"
                           value="<?php echo $likes_custom_count; ?>"/>
                </label>
            </p>

            <p class="comments_custom_count-block" style="display: <?php echo ($show_comments) ? 'block': 'none';?>;">
                <label for="<?php echo $this->get_field_id( 'comments_custom_count' ); ?>">
                    <?php esc_html_e( 'Comments count', 'xstore-core' ); ?>:
                    <input class="widefat"
                           id="<?php echo $this->get_field_id( 'comments_custom_count' ); ?>"
                           name="<?php echo $this->get_field_name( 'comments_custom_count' ); ?>"
                           type="text"
                           placeholder="5-20"
                           value="<?php echo $comments_custom_count; ?>"/>
                </label>
            </p>
        </div>

        <p>
            <label for="<?php echo $this->get_field_id( 'user' ); ?>"><?php esc_html_e( 'Choose Instagram account', 'xstore-core' ); ?>:</label>
            <select id="<?php echo $this->get_field_id( 'user' ); ?>"
                ...
                if ( is_array( $api_data ) && count( $api_data ) ) {
                    foreach ( $api_data as $key => $value ) {
                        ...
                        ?>
                        <option value="<?php echo esc_attr($key) ?>" <?php selected( $key, $user ); ?>><?php echo esc_html( $value_username ); ?></option>
                        <?php
                    }
                }
                ?>
        ...
        <p>
            <a class="add-instagram-account" href="<?php echo esc_url( admin_url( 'admin.php?page=et-panel-social' ) ); ?>" target="_blank">
                <?php esc_html_e( 'Add Instagram account?', 'xstore-core' ); ?>
            </a>
        </p>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'tag_type' ); ?>"><?php esc_html_e( 'Sort by', 'xstore-core' ); ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'size' ); ?>"><?php esc_html_e( 'Image size', 'xstore-core' ); ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'target' ); ?>"><?php esc_html_e( 'Open links in', 'xstore-core' ); ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'columns' ); ?>"><?php esc_html_e( 'Columns', 'xstore-core' ); ?>:</label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php esc_html_e( 'Link text', 'xstore-core' ); ?>:
                <input class="widefat" id="<?php echo $this->get_field_id( 'link' ); ?>"
                    name="<?php echo $this->get_field_name( 'link' ); ?>" type="text"
                    value="<?php echo $link; ?>"/></label>
        </p>
        ...
        <p>
            <input type="checkbox"
                id="<?php echo $this->get_field_id( 'show_likes' ); ?>"
                name="<?php echo $this->get_field_name( 'show_likes' ); ?>"
                value="1" <?php checked( true, $show_likes, true ); ?>>
            <label for="<?php echo $this->get_field_id( 'show_likes' ); ?>">
                <?php esc_html_e( 'Show likes count', 'xstore-core' ); ?>
            </label>
        </p>
        ...
        <p>
            <input type="checkbox"
                ...
                value="1" <?php checked( true, $show_comments, true ); ?>>
            <label for="<?php echo $this->get_field_id( 'show_comments' ); ?>">
                <?php esc_html_e( 'Show comments count', 'xstore-core' ); ?>
            </label>
        </p>

        <?php parent::widget_input_checkbox(
            esc_html__( 'Use ajax preload for this widget', 'xstore-core' ),
            $this->get_field_id( 'ajax' ),
            $this->get_field_name( 'ajax' ),
            checked( $ajax, true, false ),
            1
        ); ?>
    ...
    function update( $new_instance, $old_instance ) {
        ...
        $instance['img_source']  = in_array( $new_instance['img_source'] ?? 'instagram', array('instagram','gallery'), true ) ? $new_instance['img_source'] : 'instagram';
        $csv = preg_replace( '/[^0-9,]/', '', $new_instance['gallery'] ?? '' );
        $ids = array_unique( array_filter( array_map( 'absint', array_map( 'trim', explode( ',', $csv ) ) ) ) );
        $instance['gallery'] = $ids ? implode(',', $ids) : '';

        $instance['images_link']    = sanitize_text_field( $new_instance['images_link'] ?? '' );
        $instance['likes_custom_count']    = sanitize_text_field( $new_instance['likes_custom_count'] ?? '' );
        $instance['comments_custom_count']  = sanitize_text_field( $new_instance['comments_custom_count'] ?? '' );

        $instance['show_likes']    = ! empty( $new_instance['show_likes'] );
        $instance['show_comments'] = ! empty( $new_instance['show_comments'] );
    ...
    public function print_admin_scripts() {
        ...
        <script>
        (function($){
            if (window.etInstagramWidgetInit) {
                return;
            }
            ...
            function toggleGallery($select){
                var val = $select.val();
                var $content = $select.closest('.widget-content');
                var $box = $content.find('.et-gallery-container');
                var $userRow = $content.find('select[name$="[user]"]').closest('p');
                var $tagRow = $content.find('select[name$="[tag_type]"]').closest('p');
                var $addRow = $content.find('.add-instagram-account').closest('p');
                ...
                if (val === 'gallery') {
                    ...
                    $box.hide();
                    $userRow.attr('style', null);
                    $tagRow.attr('style', null);
                    $addRow.attr('style', null);
                }
            }

            window.etInstagramWidgetInit = function(context){
                var $context = context ? $(context) : $(document);
                $context.find('.et-img-source').each(function(){
                    toggleGallery($(this));
                });
            };

            $(document).off('change.etInstagram', '.et-img-source')
                .on('change.etInstagram', '.et-img-source', function(){
                    toggleGallery($(this));
                });

            $(document).off('click.etInstagram', '.et-select-gallery')
                .on('click.etInstagram', '.et-select-gallery', function(e){
                    e.preventDefault();
                    if (typeof wp === 'undefined' || !wp.media) { return; }

                    var $btn = $(this);
                    var $wrap = $($btn.data('wrap'));
                    var $input = $($btn.data('input'));
                    var frame = wp.media({
                        title:  '<?php echo esc_js( __( 'Select images', 'xstore-core' ) ); ?>',
                        button: { text: '<?php echo esc_js( __( 'Use these', 'xstore-core' ) ); ?>' },
                        library:{ type: 'image' },
                        multiple: true
                    });
                    ...
                    frame.on('select', function(){
                        ...
                        var ids = parseCSV($input.val());
                        $wrap.find('.et-empty').remove();
                        ...
                        sel.map(function(att){
                            ...
                            if (a && a.id && ids.indexOf(a.id) === -1) {
                                ...
                                $wrap.append(
                                    '<div class="et-gallery-item" data-id="'+a.id+'" style="position:relative;width:90px;height:90px;border:1px solid #ccd0d4;background:#fff;padding:2px;box-sizing:border-box">'+
                                        '<span class="et-remove" style="position:absolute;top:-7px;right:-7px;background:#d63638;color:#fff;width:22px;height:22px;border-radius:50%;line-height:22px;text-align:center;font-weight:bold;cursor:pointer">×</span>'+
                        ...
                        $btn.closest('.et-gallery-container').find('.et-clear-gallery').show();
                    });

                    frame.open();
                });

            $(document).off('click.etInstagram', '.et-gallery-thumbs .et-remove')
                .on('click.etInstagram', '.et-gallery-thumbs .et-remove', function(e){
                    e.preventDefault();
                    var $item = $(this).closest('.et-gallery-item');
                    var id = parseInt($item.data('id'), 10);
                    var $wrap = $item.closest('.et-gallery-thumbs');
                    var $input = $($wrap.data('input'));
                    $item.remove();

                    var ids = parseCSV($input.val()).filter(function(n){ return n !== id; });
                    if (!ids.length) {
                        $wrap.append('<span class="et-empty" style="color:#757575"><?php echo esc_js( __( 'No images selected', 'xstore-core' ) ); ?></span>');
                        $wrap.closest('.et-gallery-container').find('.et-clear-gallery').hide();
                    }
                    $input.val(ids.join(',')).trigger('change');
                });

            $(document).off('click.etInstagram', '.et-clear-gallery')
                .on('click.etInstagram', '.et-clear-gallery', function(e){
                    e.preventDefault();
                    var $btn = $(this);
                    var $wrap = $($btn.data('wrap'));
                    var $input = $($btn.data('input'));
                    $wrap.empty().append('<span class="et-empty" style="color:#757575"><?php echo esc_js( __( 'No images selected', 'xstore-core' ) ); ?></span>');
                    $input.val('').trigger('change');
                    $btn.hide();
                });

            $(document).off('change.etInstagram', 'input[name$="[show_likes]"]')
                .on('change.etInstagram', 'input[name$="[show_likes]"]', function(){
                    var $content = $(this).closest('.widget-content');
                    $content.find('.likes_custom_count-block').toggle($(this).is(':checked'));
                });

            $(document).off('change.etInstagram', 'input[name$="[show_comments]"]')
                .on('change.etInstagram', 'input[name$="[show_comments]"]', function(){
                    var $content = $(this).closest('.widget-content');
                    $content.find('.comments_custom_count-block').toggle($(this).is(':checked'));
                });

            $(function(){
                window.etInstagramWidgetInit();
            });

            $(document).on('widget-added widget-updated', function(e, widget){
                window.etInstagramWidgetInit(widget);
            });
        })(jQuery);
        </script>
        <?php
            ...
            foreach ( $callback['data'] as $key => $value ) {
                if ( $value['media_type'] === 'CAROUSEL_ALBUM' || $value['media_type'] === 'VIDEO' ) {
                    ...
                    if (
                        ...
                        $album_count = $this->get_album_count($value);
                        if ($album_count){
                            $callback['data'][ $key ]['album_count'] = $album_count;
                        }
                    }
                }
            }
    ...
    public function get_album_count($album){
        $responce = wp_remote_get($album['permalink']);

        if (wp_remote_retrieve_response_code($responce) == 200){
            $responce = wp_remote_retrieve_body($responce);
            if ($responce){
                $script = $this->get_between_strings($responce);
                if ($script){
                    $script = json_decode($script, true);
                    if (is_array($script) && isset($script['image']) && count($script['image'])){
                        return count($script['image']);
                    }
                }
            }
        }
    ...
    public function get_between_strings($str){
        $matches = array();
        preg_match("/<script[^>]*>(.*?)<\\/script>/si", $str, $matches);
        return $matches[1];
    }

    public function get_meta_count($url){
	    ...
	    preg_match('/(\d+)\s+likes,\s+(\d+)\s+comments/', $descriptionContent, $matches);

	    $result = [
		    'likes' => isset($matches[1]) ? (int)$matches[1] : 0,
		    'comments' => isset($matches[2]) ? (int)$matches[2] : 0,
	    ];

        return $result;
    }

	public function is_hide_by_count($hide_empty_meta, $qty){
		if ($hide_empty_meta){
			if ($qty<=0){
				return false;
			}
