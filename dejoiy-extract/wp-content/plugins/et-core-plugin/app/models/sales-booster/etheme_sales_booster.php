...
//                        'preview_url' => 'https://xstore.8theme.com/elementor2/sport/my-account/',
                        ...
                        'theme_mod' => true,
                        'theme_mod_url' => apply_filters('etheme_theme_options_permalink', admin_url( '/customize.php?autofocus[section]=xstore-wishlist' ), 'xstore-waitlist'),
                    ),
...
//                        'theme_mod_url' => admin_url( 'edit.php?post_type=etheme_linked_var' ), // uncomment to redirect to settings
                ...
                $sales_booster_features = apply_filters('etheme_sales_booster_list', $sales_booster_features);

                foreach ( $sales_booster_features as $slug => $sale_booster_feature ) {
                    if ($_sale_booster_feature && $_sale_booster_feature != $slug)
                        continue;

                    $is_banner = isset($sale_booster_feature['source_type']) && $sale_booster_feature['source_type'] == 'banner';
                    ...
                    if ( !isset($sale_booster_feature['image_url']) )
                        $sale_booster_feature['image_url'] = $this->dir_url . '/images/'.$slug.'.jpg';

                    $new_is_plugin_active = false;
                    if ( !$is_banner )
                ...
                ?>
                 ...
                 data-filter="<?php echo trim( esc_attr( implode( ' ', $filters ) ) ); ?>">
                    <div class="xstore-panel-grid-item-content">
                        ...
                        <?php if (!isset($sale_booster_feature['hide_info'])) : ?>
                    ...
                    <?php endif; ?>
                                ...
                                <?php if ( $is_banner) : ?>
                                    <a
                                        href="<?php echo esc_url( apply_filters('etheme_documentation_url', $details_url) ); ?>"
                                        target="_blank"
                                        ...
                                        <span><?php esc_html_e('Install', 'xstore-core'); ?></span>
                                    </a>
                                <?php else: ?>
                                    <span
