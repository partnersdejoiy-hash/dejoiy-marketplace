...
class ET_product_templates {
    ...
    public function reset_shortcodes() {
        remove_filter( 'vc_shortcode_set_template_vc_icon', array( $this, 'addVcIconShortcodesTemplates' ) );
        remove_filter( 'vc_shortcode_set_template_vc_button2', array( $this, 'addVcButton2ShortcodesTemplates' ) );
        remove_filter( 'vc_shortcode_set_template_vc_single_image', array(
            $this,
            'addVcSingleImageShortcodesTemplates',
        ) );
        remove_filter( 'vc_shortcode_set_template_vc_custom_heading', array(
            $this,
            'addVcCustomHeadingShortcodesTemplates',
        ) );
        remove_filter( 'vc_shortcode_set_template_vc_btn', array( $this, 'addVcBtnShortcodesTemplates' ) );

        remove_filter( 'vc_gitem_template_attribute_post_image_background_image_css', array(
            $this,
            'vc_gitem_template_attribute_post_image_background_image_css'
        ), 10, 2 );
    }
	...
	public function addShortcodesCustomCss( $id = '' ) {
		...
		if ( preg_match( '/^\d+$/', $id ) ) {
			...
			$parser = function_exists( 'wpbakery' ) ? wpbakery() : visual_composer();
			$shortcodes_custom_css = $parser->parseShortcodesCustomCss( $predefined_template['template'] );
		}
