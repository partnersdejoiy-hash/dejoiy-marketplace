...
?>
  ...
  <div class="reviews_area reviews_add_area reviews_area_live">
		...
		<div class="write_review">
			<div class="lft review_photo"><img src="<?php echo esc_url($wp_user_avatar); ?>" alt="Review"/></div>
			<div class="rgt review_text">
			  <form method="post" name="wcfmmp_store_review_form" id="wcfmmp_store_review_form">
					<div class="rating_area <?php echo esc_attr($rating_block_class); ?>">
						<?php foreach( $wcfm_review_categories as $wcfm_review_cat_key => $wcfm_review_category ) { ?>
							<div class="rating_box rating-stars">
								...
								<span><span class="rating_text"><?php if( $is_start_with_full_rating ) { echo '5'; } else { echo '0'; } ?></span>.0 <?php esc_html_e( $wcfm_review_category['category'], 'wc-multivendor-marketplace' ); ?></span>
								<input type="hidden" class="rating_value" name="wcfm_store_review_category[<?php echo esc_attr($wcfm_review_cat_key); ?>]" value="<?php if( $is_start_with_full_rating ) { echo '5'; } else { echo '0'; } ?>" />
							</div>
						<?php } ?>
					</div>
					<div class="add_review add_review_box">
					  <input type="hidden" name="wcfm_review_store_id" value="<?php echo esc_attr($store_user->get_id()); ?>" />
					  <input type="hidden" name="wcfm_review_author_id" value="<?php echo esc_attr($author_id); ?>" />
