...
function etheme_elementor_force_load_global_assets() {
    return apply_filters('etheme_elementor_force_load_global_assets', true, etheme_elementor_is_editor_or_preview_mode());
}

add_filter('etheme_elementor_force_localize_global_assets', function($script_key, $type = false) {
    if ( $type && etheme_elementor_force_load_global_assets() ) {
        switch ($type) {
            case 'general':
                $script_key = 'etheme_general_all_js';
                break;
            case 'woocommerce':
                $script_key = 'etheme_woocommerce_all_scripts';
                break;
            case 'elementor':
                $script_key = 'etheme_elementor_all_widgets_js';
                break;
            default;
        }
    }
...
add_filter('etheme_should_enqueue_global_style', function ($value, $script = false) {
    return etheme_elementor_force_load_global_assets();
}, 10, 2);

add_filter('etheme_should_enqueue_global_script', function ($value, $script = false) {
    return etheme_elementor_force_load_global_assets();
}, 10, 2);
function etheme_elementor_is_editor_or_preview_mode($check_edit = true, $check_preview = true) {
...
add_action('wp', function () {
    $is_preview = etheme_elementor_is_editor_or_preview_mode(false);
    $is_edit = etheme_elementor_is_editor_or_preview_mode(true, false);
    ...
    if ( defined('ELEMENTOR_PRO_VERSION') ) {
        ...
        $header_el_type = !empty($conditions_manager->get_documents_for_location('header'));
        if ( $header_el_type ) {
            add_filter('pre_option_etheme_disable_customizer_header_builder', function() {
                return true;
            }, 99);
            set_query_var( 'etheme_customizer_header_builder', false);
        }
...
foreach ($disable_options_edit_mode as $disable_option_edit_mode) {
    add_filter('theme_mod_'.$disable_option_edit_mode, function ($origin_value) {
        if ( is_customize_preview() || etheme_elementor_is_editor_or_preview_mode() ) {
            return false;
        }
        return $origin_value;
    }, 10);
...
add_filter('etheme_should_enqueue_local_script', function ($value, $script_key = false) {
    $edit_preview_mode = etheme_elementor_is_editor_or_preview_mode();
...
add_filter('etheme_should_enqueue_script', function ($value) {
    $edit_preview_mode = etheme_elementor_force_load_global_assets(); // etheme_elementor_is_editor_or_preview_mode();
    if ( $edit_preview_mode ) {
        return false;
    }
    return $value;
...
add_filter('etheme_should_register_style', function ($value, $script_data) {
    if ( isset($script_data['elementor-preview']) ) {
        $edit_preview_mode = etheme_elementor_force_load_global_assets(); // etheme_elementor_is_editor_or_preview_mode();
        if ( $edit_preview_mode && !$script_data['elementor-preview'] ) {
            return false;
        }
        elseif ( !$edit_preview_mode && $script_data['elementor-preview'] ) {
            return false;
        }
    }
    return $value;
...
add_filter('etheme_should_register_script', function ($value, $script_data) {
    if ( isset($script_data['elementor-preview']) ) {
        $edit_preview_mode = etheme_elementor_force_load_global_assets(); // etheme_elementor_is_editor_or_preview_mode();
        if ( $edit_preview_mode && !$script_data['elementor-preview'] ) {
            return false;
...
add_filter('etheme_should_enqueue_style', function ($value) {
    $edit_preview_mode = etheme_elementor_force_load_global_assets(); // etheme_elementor_is_editor_or_preview_mode();
    if ( $edit_preview_mode ) {
        return false;
    }
...
add_filter('etheme_should_enqueue_global_styles', function ($value) {
    $edit_preview_mode = etheme_elementor_force_load_global_assets(); // etheme_elementor_is_editor_or_preview_mode();
    if ( $edit_preview_mode ) {
        return true;
    }
...
add_filter('etheme_should_enqueue_global_scripts', function ($value) {
    $edit_preview_mode = etheme_elementor_force_load_global_assets(); // etheme_elementor_is_editor_or_preview_mode();
    if ( $edit_preview_mode ) {
        return true;
    }
...
add_action( 'elementor/frontend/after_enqueue_scripts', function () {
    ...
    $edit_preview_mode = etheme_elementor_is_editor_or_preview_mode(); // etheme_elementor_is_editor_or_preview_mode();
    if ( $edit_preview_mode ) {
        // disable all animations parts and load only one file instead of 70items
        $has_global_animations_css = defined('ELEMENTOR_PATH') && defined('ELEMENTOR_ASSETS_URL') ? file_exists(ELEMENTOR_PATH . 'assets/lib/animations/animations.min.css') : false;
        if ($has_global_animations_css) {
            $css_styles = \Elementor\Plugin::$instance->assets_loader->get_assets();
            $css_styles = $css_styles['styles'];
            foreach ($css_styles as $css_style_key => $css_style_data) {
                if (false !== strpos($css_style_key, 'e-animation-')) {
                    wp_dequeue_style($css_style_key);
                }
            }
            wp_enqueue_style('e-animations-global', ELEMENTOR_ASSETS_URL.'lib/animations/animations.min.css');
        }
    }
}, 50 );

add_filter('etheme_should_enqueue_icons_style', function ($value) {
    $edit_preview_mode = etheme_elementor_is_editor_or_preview_mode();
    if ( $edit_preview_mode ) {
        return false;
    }
...
add_action( 'elementor/frontend/before_register_scripts', function() {
    ...
    foreach (etheme_config_js_files() as $script){
        ...
        if ( !apply_filters('etheme_should_register_script', true, $script) ) continue;

        wp_register_script(
            $script['name'],
            get_template_directory_uri() . $script['file'],
            (isset($script['deps']) ? $script['deps'] : array('jquery', apply_filters('etheme_elementor_force_localize_global_assets', 'etheme',  'general'))),
...
add_action( 'elementor/frontend/before_register_styles', function() {
    ...
    foreach (etheme_config_css_files() as $script){
        if ( !apply_filters('etheme_should_register_style', true, $script) ) continue;
...
add_filter('etheme_product_filters_taxonomies', function ($elements) {
    if ( etheme_get_option( 'enable_brands', 1 ) ) {
        $elements['brand'] = sprintf(esc_html__('%s Brand', 'xstore'), apply_filters('etheme_theme_label', 'XStore'));
    }
    return $elements;
});
...
function etheme_product_grid_list_product_and_hover_elements_filter($elements) {
    ...
    if ( class_exists( 'WeDevs_Dokan' ) || class_exists( 'Dokan_Pro' ) ) {
        $seller_position = array_search('rating', array_keys($elements));
        if ( $seller_position > 1 ) {
            $elements = array_slice( $elements, 0, $seller_position, true ) +
                array( 'dokan_seller' => esc_html__( 'Show Seller (dokan)', 'xstore' ) ) +
                array_slice( $elements, $seller_position, count( $elements ) - $seller_position, true );
        }
        else {
            $elements['dokan_seller'] = esc_html__( 'Show Seller (dokan)', 'xstore' );
        }
        add_filter('etheme_product_grid_list_product_dokan_seller_styles', '__return_true');
    }
    ...
    if ( $enabled_brands ) {
        ...
        add_filter('etheme_product_grid_list_product_brands_styles', '__return_true');
    }
...
//        set_query_var('et_is-swatches', $enabled_swatch);
//    }
...
function etheme_product_grid_list_product_and_hover_elements_style ($control) {
    if ( apply_filters('etheme_product_grid_list_product_brands_styles', false) ) {
        $control->start_controls_section(
            'section_brands_style',
            [
                'label' => sprintf(esc_html__('%s Brands', 'xstore'), apply_filters('etheme_theme_label', 'XStore')),
    ...
    if ( apply_filters('etheme_product_grid_list_product_dokan_seller_styles', false) ) {
        $control->start_controls_section(
            'section_dokan_seller_style',
            [
                'label' => __('Seller (dokan)', 'xstore'),
                'tab' => \Elementor\Controls_Manager::TAB_STYLE,
                'condition' => [
                    'product_dokan_seller!' => ''
                ],
            ]
        );
        ...
        $control->add_group_control(
            \Elementor\Group_Control_Typography::get_type(),
            [
                'name' => 'dokan_seller_typography',
                'selector' => '{{WRAPPER}} .product_seller',
            ]
        );

        $control->start_controls_tabs('tabs_dokan_seller_colors');

        $control->start_controls_tab('tabs_dokan_seller_color_normal',
            [
                'label' => esc_html__('Normal', 'xstore')
            ]
        );
        ...
        $control->add_control(
            ...
            [
                ...
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .product_seller, {{WRAPPER}} .product_seller a' => 'color: {{VALUE}};',
                ],
            ]
        );
        ...
        $control->start_controls_tab('tabs_dokan_seller_color_hover',
            [
                'label' => esc_html__('Hover', 'xstore')
            ]
        );
        ...
        $control->add_control(
            'dokan_seller_color_hover',
            [
                'label' => __('Color', 'xstore'),
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .product_seller a:hover' => 'color: {{VALUE}};',
                ],
            ]
        );
        ...
        $control->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .product_seller' => 'margin-bottom: {{SIZE}}{{UNIT}}; display: block;',
                ],
            ]
        );
...
add_filter('etheme_product_grid_list_product_taxonomies', function ($taxonomies) {
    if ( etheme_get_option( 'enable_brands', 1 ) ) {
        $taxonomies['brand'] = sprintf(esc_html__('%s Brands', 'xstore'), apply_filters('etheme_theme_label', 'XStore'));
    }
    return $taxonomies;
});
...
add_filter('etheme_posts_post_element_image', function ($thumbnail, $post_id, $settings) {
    if ( get_post_format($post_id) == 'video' ) {
        ob_start();
            etheme_the_post_field( 'video', $post_id);
        $video_thumbnail = ob_get_clean();
        if ( !!$video_thumbnail )
...
add_filter('etheme_posts_post_image_link', function ($origin_value, $post_id, $settings) {
    if ( get_post_format($post_id) == 'video' ) {
        return false;
    }
    return $origin_value;
...
add_filter('etheme_product_meta_elements', function ($elements) {
    ...
    if ( get_theme_mod('enable_brands', true) )
        $elements['et_brand'] = sprintf(esc_html__('%s Brand', 'xstore'), apply_filters('etheme_theme_label', 'XStore'));
    return $elements;
});
...
add_action( 'elementor/element/common/_section_style/before_section_start', function( $element, $args ) {
    ...
    $element->start_controls_section(
        ...
        array(
            'label'     => sprintf(__( '%s Ajaxify', 'xstore' ), apply_filters('etheme_theme_label', 'XSTORE')),
    ...
    $element->add_control(
        ...
        [
            ...
            'description' => !get_theme_mod('global_ajaxify', 0) ? esc_html__('The Ajaxify feature is currently disabled globally. To enable this setting, please activate the \'Enable Ajaxify on website\' option in Theme Options.', 'xstore') : false,
...
function etheme_ajaxify_elementor_widgets($widget_content, $that){
    if (defined('DOING_ETHEME_AJAXIFY') || isset($_GET['et_ajax']) || !apply_filters('etheme_ajaxify_elementor_widget', true, $that) || etheme_elementor_is_editor_or_preview_mode(true, false) ){
...
add_action('wp', function () {
    if (!get_theme_mod('global_ajaxify', 0) || is_404() || is_search()) {
...
//        if (get_query_var('et_is-checkout-page-elementor-shortcode', false) || false !== strpos($content, 'etheme-elementor-checkout-widgets-contain')) {
            ...
            if ( ! get_query_var( 'et_is-loggedin', false) && function_exists( 'WC' ) && ! WC()->checkout()->is_registration_enabled() && WC()->checkout()->is_registration_required() ) :
...
// ADDED: Compatibility with WooCommerce Germanized plugin of Products grid/list/carousel widgets
if ( class_exists('WooCommerce_Germanized') ) {
    ...
    foreach ( wc_gzd_get_product_loop_shopmarks() as $shopmark ) {
        ...
        $should_execute = true;
        $location = $shopmark->get_location();
        $type     = $shopmark->get_type();
        $wc_gzd_filter = "woocommerce_gzd_shopmark_{$location}_{$type}_";
        $new_filter = false;

        switch($filter) {
            case 'woocommerce_after_shop_loop_item_title':
                $new_filter = 'after_etheme_product_grid_list_product_element_price';
                break;
            case 'woocommerce_before_shop_loop_item_title':
                $new_filter = 'before_etheme_product_grid_list_product_element_title';
                break;
            case 'woocommerce_shop_loop_item_title':
                $new_filter = 'after_etheme_product_grid_list_product_element_title';
                break;
            case 'woocommerce_after_shop_loop_item':
                $new_filter = 'etheme_product_grid_list_after_product_item_content';
                break;
            default:
                $should_execute = false;
                break;
        }
        if ( $new_filter && $should_execute ) {
            add_filter($wc_gzd_filter."filter", function($old_filter) use ($new_filter) {
                return $new_filter;
            });
            $shopmark->execute();
        }
...
add_action('wp_update_nav_menu', 'etheme_clear_menu_list_cache');
add_action('delete_nav_menu', 'etheme_clear_menu_list_cache');
add_action('wp_create_nav_menu', 'etheme_clear_menu_list_cache');

function etheme_clear_menu_list_cache() {
    delete_transient('etheme_get_available_menus_list');
}

add_action( 'save_post_elementor_library', 'xstore_clear_saved_content_transients' );
add_action( 'save_post_etheme_mega_menus', 'xstore_clear_saved_content_transients' );
add_action( 'save_post_etheme_slides', 'xstore_clear_saved_content_transients' );
add_action( 'delete_post', 'xstore_clear_saved_content_transients' );
add_action( 'wp_trash_post', 'xstore_clear_saved_content_transients' );
add_action( 'untrashed_post', 'xstore_clear_saved_content_transients' );

function xstore_clear_saved_content_transients( $post_id ) {

    $post_types_to_clear = ['elementor_library', 'etheme_mega_menus', 'etheme_slides'];

    if ( $post_id && !in_array( get_post_type( $post_id ), $post_types_to_clear ) ) {
        return;
    }

    $post_type = get_post_type( $post_id );
    if ( $post_type == 'elementor_library' )
        $terms_for_transients = ['section', 'widget'];
    else {
        $terms_for_transients = [];
        switch ($post_type) {
            case 'etheme_mega_menus':
                $terms_for_transients[] = 'mega_menu';
                break;
            case 'etheme_slides':
                $terms_for_transients[] = 'slide';
                break;
        }
    }

    foreach ( $terms_for_transients as $term ) {
        $term = sanitize_key($term);;
        delete_transient("etheme_elementor_saved_contents_{$term}_{$post_type}_0");
        delete_transient("etheme_elementor_saved_contents_{$term}_{$post_type}_1");
    }
}