    ...
    public function processing() {
        ...
        if (! empty($_POST['search']['value'])) {
            ...
            if (! empty($_POST['filter_date_form']) && ! empty($_POST['filter_date_to'])) {
                ...
                $time_filter = " AND DATE( commission.order_date ) BETWEEN %s AND %s";
                $sql .= $time_filter;
        ...
        $wcfm_orders_json = '{
														...
														"recordsTotal": ' . (float) $total_items . ',
														"recordsFiltered": ' . (float) $total_items . ',
														"data": ';
        ...
        if (!empty($the_order_summary)) {
            ...
            foreach ($the_order_summary as $order) {
                ...
                $amounts = [
                    ...
                    $order->product_shipping_amount,
                    $order->product_shipping_tax_amount,
                    $order->product_tax_amount
                ];
                $gross_sales = array_sum(array_map('floatval', $amounts));
