...
if ( function_exists( 'dejoiy_library_nexus_prepare_shelf' ) ) {
	$prep_nexus = ( function_exists( 'dejoiy_library_is_nexus_flow_request' ) && dejoiy_library_is_nexus_flow_request() )
...
if ( function_exists( 'dejoiy_library_get_cart_shelf_data' ) ) {
	$dlu_shelf    = dejoiy_library_get_cart_shelf_data();
	$dlu_count    = (int) $dlu_shelf['count'];
	$dlu_checkout = function_exists( 'dejoiy_library_get_checkout_url' ) ? dejoiy_library_get_checkout_url() : $dlu_cart;
} elseif ( function_exists( 'WC' ) && isset( WC()->cart ) && WC()->cart ) {
	foreach ( WC()->cart->get_cart() as $item ) {
		if ( function_exists( 'dejoiy_library_cart_item_is_book' ) && dejoiy_library_cart_item_is_book( $item ) ) {
			$dlu_count += (int) $item['quantity'];
		}
	}
}
?>
	...
	<div class="dlu-hdr-in">
		<div class="dlu-hdr-bar">
			...
			<div class="dlu-hdr-bar-actions">
				<a href="<?php echo esc_url( $dlu_cart ); ?>" class="dlu-cart-btn dlu-cart-btn--bar" aria-label="Nexus cart" data-count="<?php echo esc_attr( (string) $dlu_count ); ?>">
					...
					<span class="dlu-cart-badge" <?php echo $dlu_count < 1 ? 'hidden' : ''; ?>><?php echo esc_html( (string) $dlu_count ); ?></span>
				</a>
		...
		<div class="dlu-hdr-tools">
			...
			<button type="button" class="dlu-icon-btn dlu-icon-btn--hide-sm" id="dlu-wishlist-toggle" aria-label="Nexus wishlist" title="Nexus wishlist" aria-expanded="false" aria-controls="dlu-wishlist-panel">
				...
				<span class="dlu-wishlist-badge" id="dlu-wishlist-badge" hidden>0</span>
			</button>
			<button type="button" class="dlu-cart-btn dlu-cart-btn--tools" id="dlu-cart-toggle" aria-label="Open Nexus cart" data-count="<?php echo esc_attr( (string) $dlu_count ); ?>">
				<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
				<span class="dlu-cart-badge" <?php echo $dlu_count < 1 ? 'hidden' : ''; ?>><?php echo esc_html( (string) $dlu_count ); ?></span>
			</button>
...
<nav id="dlu-hdr-drawer" class="dlu-hdr-drawer" hidden aria-label="Mobile menu">
	...
	<a href="<?php echo esc_url( $dlu_cart ); ?>">Nexus Cart</a>
...
<aside class="dlu-cart-panel" id="dlu-cart-panel" hidden aria-label="Nexus cart">
	...
	<div class="dlu-cart-panel-foot">
		<?php if ( $dlu_count > 0 ) : ?>
			...
			<a href="<?php echo esc_url( $dlu_cart ); ?>" class="dlu-btn-secondary"><?php esc_html_e( 'View full shelf', 'dejoiy' ); ?></a>
		<?php else : ?>
			<a href="<?php echo esc_url( $dlu_cart ); ?>" class="dlu-btn-primary"><?php esc_html_e( 'View collection shelf', 'dejoiy' ); ?></a>
		<?php endif; ?>
	</div>
</aside>
