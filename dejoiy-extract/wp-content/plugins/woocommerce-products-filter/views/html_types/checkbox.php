...
if ( ! function_exists( 'woof_draw_checkbox_childs' ) ) {
	...
	function woof_draw_checkbox_childs( $taxonomy_info, $tax_slug, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos ) {
		...
		<?php if ( ! empty( $childs ) and is_array( $childs ) ) : ?>
			<ul class="woof_childs_list woof_childs_list_<?php echo esc_attr( $term_id ); ?>" 
			<?php
			if ( $hide_childs == 1 ) :
				?>
			...
			foreach ( $childs as $term ) :
				...
				?>
					...
					if ( woof()->settings['dispay_in_row'][ $tax_slug ] and empty( $term['childs'] ) ) :
						?>
						style="display: inline-block;"<?php endif; ?>>
						<input type="checkbox" <?php if ( ! $count and ! in_array( $term['slug'], $current_request ) and $show_count ) : ?>
								...
								/>
						<label <?php do_action( 'woof_print_label_attributes', $term ); ?>   class="woof_checkbox_label 
						<?php
						if ( in_array( $term['slug'], $current_request ) ) :
							?>
							woof_checkbox_label_selected<?php endif; ?>" for="<?php echo esc_attr( 'woof_' . $term['term_id'] . '_' . $inique_id ); ?>">
						<?php
...
<ul class="woof_list woof_list_checkbox">
	...
	<?php if ( ! empty( $terms ) and is_array( $terms ) ) : ?>
		...
		foreach ( $terms as $term ) :
			...
			<li class="woof_term_<?php echo esc_attr( $term['term_id'] ); ?>
			<?php
			if ( $hide_next_term_li ) :
				?>
				woof_hidden_term<?php endif; ?>" 
				<?php
				if ( $this->settings['dispay_in_row'][ $tax_slug ] and empty( $term['childs'] ) ) :
					?>
				style="display: inline-block;"<?php endif; ?>>
				<input type="checkbox" <?php if ( ! $count and ! in_array( $term['slug'], $current_request ) and $show_count ) : ?>
						disabled=""<?php endif; ?> 
