...
if(!function_exists('etheme_theme_styles')) {
    function etheme_theme_styles() {
        $is_admin = is_admin();
        if ( !$is_admin && (apply_filters('etheme_enable_load_styles_on_ajax', false) || !(isset( $_GET['et_ajax'] ) && $_GET['et_ajax'])) ) {
        	$theme = wp_get_theme();
	        ...
	        switch (etheme_get_option('fa_icons_library', 'disable') ) {
		        ...
		        case '5.15.3':
                    ...
                    break;
		        case '6.4.0':
			        wp_enqueue_style("etheme-fa",get_template_directory_uri().'/css/fontawesome/6.4.0/all.min.css', array(), $theme->version);
			        break;
                default;
            }
            ...
            foreach (etheme_config_css_files() as $script){
                ...
                if ( apply_filters('etheme_should_register_style', true, $script) )
            ...
            // load typekit fonts.
            $typekit_id = get_theme_mod( 'etheme_typekit_id', '' );

            if ( $typekit_id ) {
                $project_ids = explode( ',', $typekit_id );

                foreach ( $project_ids as $id ) {
                    wp_enqueue_style( 'etheme-typekit-' . $id, 'https://use.typekit.net/' . esc_attr( $id ) . '.css', array(), $theme->version );
                }
            }

            if ( apply_filters('etheme_is_fse_theme', false) ) {
                etheme_enqueue_style( "gutenberg-style" );
            }
            ...
            if ( apply_filters('etheme_should_enqueue_global_style', false, 'general-all-style') ) {
                etheme_enqueue_style( 'general-all-style');
            }

            if ( defined( 'ELEMENTOR_VERSION' ) ) {
                etheme_enqueue_style("elementor-style");
                if (apply_filters('etheme_should_enqueue_global_style', false, 'elementor-all-widgets-style')) {
                    etheme_enqueue_style('elementor-all-widgets-style');
                }
            }
	        ...
	        if ( class_exists('WCMp') || class_exists('MVX') || defined('MULTIVENDORX_PLUGIN_VERSION') ) {
            ...
            if ( isset($_GET['preview']) || get_query_var('et_is_customize_preview', false) ) {
                ...
                if ( get_query_var( 'et_is-woocommerce', false ) ) {
                    ...
                    if (apply_filters('etheme_should_enqueue_global_style', false, 'woocommerce-all-style')) {
                        etheme_enqueue_style('woocommerce-all-style');
                    }
...
//					        }
                        ...
                        if ( get_query_var('et_sb_infinite_scroll', false) || get_query_var('et_etspt', false) ) {
                    ...
                    elseif ( get_query_var( 'et_is-cart', false ) || get_query_var( 'et_is-checkout', false ) ) {
                        ...
                        etheme_enqueue_style('quantity-types-style');
...
//                }
		        if ( get_query_var('et_widgets-show-more', false) ) {
			        wp_add_inline_style( 'xstore-inline-css',
				        '.sidebar .sidebar-widget:not(.etheme_swatches_filter.type-st-image-swatch):not(.etheme_swatches_filter.type-st-color-swatch):not(.etheme_swatches_filter.type-st-label-swatch):not(.null-instagram-feed) ul:not(.children):not(.sub-menu) > li:nth-child('.get_query_var('et_widgets-show-more-after', 3).')
							~ li:not(.et_widget-open):not(.et_widget-show-more):not(.current-cat):not(.current-item):not(.selected),
							 .sidebar-widget ul.menu > li:nth-child('.get_query_var('et_widgets-show-more-after', 3).')
							~ li:not(.et_widget-open):not(.et_widget-show-more){
								   display: none;
        ...
        if ( !$is_admin ) {
            if ( apply_filters('etheme_should_enqueue_icons_style', true) ) {
...
function etheme_theme_header_styles() {
    if ( !get_query_var('etheme_customizer_header_builder', true) ) return;
...
if ( !function_exists('etheme_config_css_files')) {
	function etheme_config_css_files() {
        global $etheme_styles_config;
        if ( !!!$etheme_styles_config )
            $etheme_styles_config = include get_template_directory() . '/framework/config-css.php';
		return $etheme_styles_config;
	}
}
...
if ( !function_exists('etheme_enqueue_style')) {
	function etheme_enqueue_style($filename, $force_enqueue = false, $include_globally = true) {
		...
		$postfix = '';
        $cssjs_ver = get_query_var('et_cssjs_ver', 'undefined');
        if ( $cssjs_ver === 'undefined' ) {
            $cssjs_ver = etheme_get_option( 'cssjs_ver', 0 );
            set_query_var('et_cssjs_ver', $cssjs_ver);
        }
		if ( !$cssjs_ver ) {
            $theme_version = get_query_var('et_theme_version', 'undefined');
            if ( $theme_version === 'undefined' ) {
                $theme = wp_get_theme();
                $theme_version = $theme->version;
                set_query_var('et_theme_version', $theme_version);
            }
            $postfix = '?ver='.$theme_version;
		}
...
//	    wp_add_inline_script( 'load_css', "loadCSS('".$loadCssfilename."');", 'after' );
		?>
        <link rel="stylesheet" href="<?php echo esc_attr(get_template_directory_uri()) . esc_attr( $config[$filename]['file'] ) . ETHEME_MIN_CSS; ?>.css<?php echo esc_attr( $postfix ); ?>" type="text/css" media="all" /> <?php // phpcs:ignore ?>
		<?php
		if ( $did_wp_head_action && $include_globally ) {
			$etheme_styles[] = $config[ $filename ]['name'];
		}
...
//	    $theme = wp_get_theme();
	    ...
	    foreach ($config as $filedata) {
	        if ( in_array($filedata['name'], array('fixed-footer', 'header-vertical', 'et-core-plugin-off')) ) continue;
		    if ( get_query_var( 'etheme_single_product_builder', false ) && in_array($filedata['name'], array('single-product', 'single-product-wide', 'single-product-right', 'single-product-booking')) ) {
		        continue;
            }
...
if(!function_exists('etheme_register_required_plugins')) {
	...
	function etheme_register_required_plugins() {
		...
		if( !$allow_full_access && ! etheme_is_activated() ) return;

		$activated_data = get_option( 'etheme_activated_data' );

		$key = isset($activated_data['api_key']) ? $activated_data['api_key'] : '';

		if( !$allow_full_access && (! $key || empty( $key )) ) return;
		...
		if (! $plugins || empty( $plugins ) || isset($_GET['et_clear_plugins_transient'])){
			...
			$token = '?token=' . $key;
//			$url = apply_filters('etheme_protocol_url', ETHEME_BASE_URL . 'import/xstore-demos/1/plugins/?plugins_dir=' . $plugins_dir . '&token=' .$token . '&code=' . $activated_data['purchase']);
            $url = apply_filters('etheme_protocol_url', ETHEME_BASE_URL . 'import/xstore-demos/1/plugins/?plugins_dir=' . $plugins_dir);
    ...
    function etheme_footer_demo($position){
        switch ($position) {
            case 'footer-copyrights':
                ?>
                    <span style="color: #fff!important;">© Created by <a href="#"><i class="fa fa-heart"></i> &nbsp;<strong style="color: #fff!important;">8theme</strong></a> - Power Elite ThemeForest Author.</span>
                <?php
            break;
        }
    }
