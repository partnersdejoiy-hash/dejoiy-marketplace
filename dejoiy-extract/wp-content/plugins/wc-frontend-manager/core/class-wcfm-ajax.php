...
class WCFM_Ajax {
    ...
    public function wcfm_ajax_controller() {
        ...
        if (isset($_POST['controller'])) {
            ...
            switch ($controller) {
                ...
                case 'wcfm-profile':
                    if (!current_user_can('manage_woocommerce') && !current_user_can('wcfm_vendor') && !current_user_can('seller') && !current_user_can('vendor') && !current_user_can('shop_staff') && !current_user_can('wcfm_affiliate') && !current_user_can('wcfm_delivery_boy')) {
                        wp_send_json_error(esc_html__('You don&#8217;t have permission to do this.', 'woocommerce'));
                        wp_die();
                    }
    ...
    function wcfm_add_taxonomy_new_term() {
        ...
        $taxonomy     = wc_clean($_POST['taxonomy']);
        ...
        if (taxonomy_exists($taxonomy)) {
            ...
            if (is_wp_error($result)) {
                ...
                $raw_message = [
                    'l10n'    => [
                        'text'         => 'A new %s <b>%s</b> added to the store by <b>%s</b>',
                        ...
                        'wrapper'    => [
                            ...
                            'args'         => [
                                ucfirst($taxonomy),
                                $new_term,
                                get_user_by('id', $author_id)->display_name
                            ]
                        ]
                    ]
                ];

                $WCFM->wcfm_notification->wcfm_send_direct_message($author_id, $message_to, $author_is_admin, $author_is_vendor, $wcfm_messages, 'new_taxonomy_term', true, $raw_message);
    ...
    public function delete_wcfm_product() {
        ...
        $product_id = isset($_POST['proid']) ? absint($_POST['proid']) : 0;
        $product    = wc_get_product($product_id);
        if (!$product) {
            wp_send_json_error(__('Invalid Product.', 'wc-frontend-manager'));
        }

        $resource_owner_id = get_post_field('post_author', $product_id);
        if (!wcfm_user_can_perform_request($resource_owner_id, 'product_delete')) {
            wp_send_json_error(esc_html__('You don&#8217;t have permission to do this.', 'woocommerce'));
        }
        ...
        $permanent_delete = apply_filters('wcfm_is_allow_product_delete', false);
        $result = $permanent_delete ? wp_delete_post($product_id, true) : wp_trash_post($product_id);

        if ($result) {
            wp_send_json_success('success');
        } else {
            wp_send_json_error(__('Delete failed.', 'wc-frontend-manager'));
        }
    }
    ...
    public function wcfm_modify_order_status() {
        ...
        $order_status = isset($_POST['order_status']) ? wc_clean($_POST['order_status']) : '';
        $new_status   = str_replace('wc-', '', $order_status);
        if (!wc_is_order_status($order_status)) {
            wp_send_json_error(__('Invalid Order Status', 'wc-frontend-manager'));
        }

        do_action('before_wcfm_order_status_update', $order_id, $order_status);

        if (defined('WCFM_REST_API_CALL') && wcfm_is_vendor()) {
            return '{"status": true, "message": "' . esc_html__('Order status updated.', 'wc-frontend-manager') . '"}';
            die;
        }
        ...
        $shop_name = $user_data ? $user_data->display_name : __('Unknown', 'wc-frontend-manager');
        if (wcfm_is_vendor()) {
            $shop_name =  wcfm_get_vendor_store(absint($user_id));
        }
        $wcfm_messages = sprintf(__('Order status updated to <b>%s</b> by <b>%s</b>', 'wc-frontend-manager'), wc_get_order_status_name($new_status), $shop_name);
        $is_customer_note = apply_filters('wcfm_is_allow_order_update_note_for_customer', '1');
        ...
        $wcfm_messages = sprintf(__('<b>%s</b> order status updated to <b>%s</b> by <b>%s</b>', 'wc-frontend-manager'), '#<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url($order_id) . '">' . $order->get_order_number() . '</a>', wc_get_order_status_name($new_status), $shop_name);
        ...
        do_action('woocommerce_order_edit_status', $order_id, $new_status);
        do_action('wcfm_order_status_updated', $order_id, $new_status);
    ...
    function wcfm_dismiss_inactive_addon_notice() {
        if (! check_ajax_referer('wcfm_ajax_nonce', 'wcfm_ajax_nonce', false)) {
            wp_die(esc_html__('Invalid nonce! Refresh your page and try again.', 'wc-frontend-manager'));
        }
    ...
    function wcfm_menu_toggler() {
        ...
        $toggle_state = wc_clean($_POST['toggle_state']);
    ...
    function wcfm_email_verification_code() {
        ...
        if ($user_email) {
            ...
            $email_verification_code = get_user_meta($user_id, '_wcfm_email_verification_code', true);
            if ($email_verification_code) {
                $verification_code = $email_verification_code;
            } else {
                $verification_code = rand(100000, 999999);
                update_user_meta($user_id, '_wcfm_email_verification_code', $verification_code);
            }
            ...
            echo '{"status": true, "message": "' . esc_html__('Email verification code send to your email.', 'wc-frontend-manager') . '"}';
        } else {
            echo '{"status": false, "message": "' . esc_html__('Email verification not working right now, please try after sometime.', 'wc-frontend-manager') . '"}';
        }
        die;
    }
    ...
    function wcfm_vendor_disable() {
        ...
        if (isset($_POST['memberid'])) {
            ...
            $raw_message = [
                'l10n'    => [
                    'text'         => '<b>%s</b> (Store: <b>%s</b>) has been disabled.',
                    'domain'    => 'wc-frontend-manager',
                    'wrapper'    => [
                        'function'     => 'sprintf',
                        'args'         => [
            ...
            $WCFM->wcfm_notification->wcfm_send_direct_message(-2, 0, 1, 0, $wcfm_messages, 'vendor-disable', true, $raw_message);
            ...
            $raw_message = [
                'l10n'    => [
                    'text'         => 'Your Store: <b>%s</b> has been disabled.',
                    'domain'    => 'wc-frontend-manager',
                    'wrapper'    => [
                        'function'     => 'sprintf',
                        'args'         => [
            ...
            $WCFM->wcfm_notification->wcfm_send_direct_message(-1, $member_id, 1, 0, $wcfm_messages, 'vendor-disable', true, $raw_message);

            do_action('wcfm_vendor_disable_after', $member_id);

            echo '{"status": true, "message": "' . esc_html__('Vendor successfully disabled.', 'wc-frontend-manager') . '"}';
            die;
        }
        echo '{"status": false, "message": "' . esc_html__('Vendor can not be disabled right now, please try after sometime.', 'wc-frontend-manager') . '"}';
        die;
    }
    ...
    function wcfm_vendor_enable() {
        ...
        if (isset($_POST['memberid'])) {
            ...
            $raw_message = [
                'l10n'    => [
                    'text'         => '<b>%s</b> (Store: <b>%s</b>) has been enabled.',
                    'domain'    => 'wc-frontend-manager',
                    'wrapper'    => [
                        'function'     => 'sprintf',
                        'args'         => [
            ...
            $WCFM->wcfm_notification->wcfm_send_direct_message(-2, 0, 1, 0, $wcfm_messages, 'vendor-enable', true, $raw_message);
            ...
            $raw_message = [
                'l10n'    => [
                    'text'         => 'Your Store: <b>%s</b> has been enabled.',
                    'domain'    => 'wc-frontend-manager',
                    'wrapper'    => [
                        'function'     => 'sprintf',
                        'args'         => [
            ...
            $WCFM->wcfm_notification->wcfm_send_direct_message(-1, $member_id, 1, 0, $wcfm_messages, 'vendor-enable', true, $raw_message);

            do_action('wcfm_vendor_enable_after', $member_id);

            echo '{"status": true, "message": "' . esc_html__('Vendor successfully enabled.', 'wc-frontend-manager') . '"}';
            die;
        }
        echo '{"status": false, "message": "' . esc_html__('Vendor can not be enabled right now, please try after sometime.', 'wc-frontend-manager') . '"}';
        die;
    }
    ...
    function wcfm_knowledgebase_view() {
        ...
        if (!current_user_can('manage_woocommerce') && !current_user_can('wcfm_vendor') && !current_user_can('seller') && !current_user_can('vendor') && !current_user_can('shop_staff')) {
            esc_html_e('You don&#8217;t have permission to do this.', 'woocommerce');
            wp_die();
        }
        ...
        if (isset($_POST['knowledgebaseid'])) {
            ...
            echo wp_kses_post($knowledgebase_post->post_title);
            echo '</h2></td></tr><tr><td>';
    ...
    function wcfm_login_popup_submit() {
        ...
        if (empty($wcfm_login_popup_form_data['wcfm_login_popup_username'])) {
            echo '{"status": false, "message": "' . esc_html__('Please insert username before submit.', 'wc-frontend-manager') . '"}';
            die;
        }
        ...
        if (is_email($wcfm_login_popup_form_data['wcfm_login_popup_username'])) {
            ...
            if (!email_exists($wcfm_login_popup_form_data['wcfm_login_popup_username'])) {
                echo '{"status": false, "message": "' . esc_html__('Please insert a valid username / e-mail address.', 'wc-frontend-manager') . '"}';
                die;
            }
            ...
            if ($current_user && is_a($current_user, 'WP_User')) {
                ...
                if (is_wp_error($user)) {
                    echo '{"status": false, "message": "' . esc_html__('Please try again!', 'wc-frontend-manager') . $user->get_error_message() . '"}';
                } else {
                    echo '{"status": true, "message": "' . esc_html__('Login successfully ...', 'wc-frontend-manager') . '"}';
                }
            } else {
                echo '{"status": false, "message": "' . esc_html__('Please try again!', 'wc-frontend-manager') . '"}';
            }
        } else {
    ...
    function wcfm_store_external_product_view_update() {
        ...
        if (isset($_POST['product_id']) && !empty($_POST['product_id'])) {
            ...
            if (!isset($_SESSION['wcfm_pages']) || !isset($_SESSION['wcfm_pages']['products']) || (isset($_SESSION['wcfm_pages']) && isset($_SESSION['wcfm_pages']['products']) && !in_array($product_id, $_SESSION['wcfm_pages']['products']))) {
                ...
                $ip_address   = wc_clean($_SERVER['REMOTE_ADDR']);
                ...
                $wcfm_detailed_analysis = $wpdb->prepare(
                    "INSERT into {$wpdb->prefix}wcfm_detailed_analysis 
                ...
                $wcfm_daily_analysis = $wpdb->prepare(
                    "INSERT into {$wpdb->prefix}wcfm_daily_analysis 
