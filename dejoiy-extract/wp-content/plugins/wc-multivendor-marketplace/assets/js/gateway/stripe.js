jQuery( function( $ ) {
	...
	var wcfmmp_stripe_split_pay_form = {
		...
		hasToken: function() {
			...
			return Number(wcfmmp_stripe_split_pay_params.no_of_vendor) === $( 'input.stripe-token' ).length;
		},
		...
		createSource: function() {
			...
			return stripe.createSource( stripe_card, extra_details ).then(wcfmmp_stripe_split_pay_form.sourceResponse);
		},
		
		createToken: async function() {
			for(var i = 0; i < wcfmmp_stripe_split_pay_params.no_of_vendor; i++) {
				await stripe.createToken(stripe_card).then( wcfmmp_stripe_split_pay_form.tokenResponse );
			}
		},
		...
		onSubmit: function( e ) {
			...
			// If a source and token is already in place, submit the form as usual.
			...
			if( jQuery('input[name=wcfmmp_stripe_customer_id]').length > 0 ) {
				...
				(async () => {
					if ( ! wcfmmp_stripe_split_pay_form.hasSource() ) {
						await wcfmmp_stripe_split_pay_form.createSource();
					}
	
					if ( !wcfmmp_stripe_split_pay_form.hasToken() ) {
						await wcfmmp_stripe_split_pay_form.createToken();
					}
