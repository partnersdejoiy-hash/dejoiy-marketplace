...
class Dynamic_Categories extends \Elementor\Widget_Base {
    ...
    private static $ajax_category_no_support = null;
    ...
    public function get_style_depends() {
        return array_merge([ 'e-swiper' ], apply_filters('etheme_elementor_widget_style_depends', ['etheme-elementor-dynamic-product-categories'] ));
    }
...
//		);
	...
	public function render()
    {
        ...
        $edit_preview_mode = Elementor::is_editor_or_preview_mode(true, true);
        ...
        $heading_link = false;
        $return_link = !!$edit_preview_mode;
        $query_args = array(
            'edit_mode' => $edit_preview_mode,
            'all_categories' => $edit_preview_mode
        );
            ...
            <div <?php $this->print_render_attribute_string( 'wrapper-inner' ); ?>>
                <div <?php $this->print_render_attribute_string( 'slides-wrapper' ); ?>>
                    ...
                    if( $settings['show_all'] ) {
                        ...
                        $category_args = [
                            'url' => self::$shop_url,
                            'count' => count($this->count_all_products($settings, $edit_preview_mode)),
                    ...
                    if( $settings['show_new'] ) {
                        echo '<div class="swiper-slide">';
                            $category_args = [
                                ...
                                'count' => count($this->count_new_products($settings, $edit_preview_mode)),
                    ...
                    if( $settings['show_sale'] ) {
                        echo '<div class="swiper-slide">';
                            $category_args = [
                                ...
                                'count' => $this->count_on_sale_products($settings, $edit_preview_mode),
                ...
                <?php
                    if ( $swiper_latest ) {
                        if (in_array($settings['navigation'], array('both', 'arrows')))
                            Elementor::get_slider_navigation($settings, $edit_preview_mode);
                    }
                    //                                    if ( 1 < count($categories) ) {
                    if ( in_array($settings['navigation'], array('both', 'dots')) ) {
                        Elementor::get_slider_pagination($this, $settings, $edit_preview_mode);
                    }
    ...
    public static function get_query($settings, $extra_params = array()) {
        ...
        if( $extra_params['edit_mode'] || $extra_params['all_categories'] || ($source == 'included') || !is_object(self::$queried_object) || self::is_archive_shop() ) {
            ...
            if ( apply_filters('etheme_dynamic_categories_top_level_only', true) )
                $query_args['parent'] = 0;;
    ...
    public function get_content_category($category, $settings) {
                    ...
                    switch ($key) {
                        case 'image':
                            ...
                            if ( $local_settings['image_size'] != 'custom') {
                                ...
                                $local_content[$key] = $this->get_category_thumbnail($category);
                            }
                            break;
                        case 'title':
                            ob_start();
                            ...
                            break;
                        case 'count':
                            $local_content[$key] = $this->get_category_count($category->count, $category);
                            break;
                    }
                }
        ...
        if ( isset($local_content['image'])) {
            echo '<div class="etheme-category-grid-image'. (!!$local_settings['img_rounded'] ? ' rounded' : '') .'">';
                echo '<a '.(!empty($url_class['class']) ? 'rel="nofollow noopener" ' : '').' href="' . esc_url( $url_class['url'] ) . '" class="'.$url_class['class'].'">';
    ...
    public function check_ajax_category_no_support() {
        if ( null !== self::$ajax_category_no_support )
            return self::$ajax_category_no_support;

        self::$ajax_category_no_support = !apply_filters('etheme_dynamic_categories_support_ajax_filters', true) || !get_theme_mod('ajax_categories', 1) || is_tax('dc_vendor_shop') || (function_exists('wcfm_is_store_page') && wcfm_is_store_page()) || ( function_exists('wcfmmp_is_stores_list_page') && wcfmmp_is_stores_list_page());
    ...
    public function generate_category_url_class($page_url, $category) {
        $category_class = '';
        if ( !$page_url || $this->check_ajax_category_no_support() ) {
        ...
        <div <?php wc_product_cat_class( $classes, null ); ?>>
            ...
            foreach (self::get_category_elements() as $key => $string_text) {
                ...
                switch ($key) {
                    case 'image':
                        // filter image size
                        $local_content[$key] = wc_placeholder_img($local_settings['image_size'] != 'custom' ? $local_settings['image_size'] : apply_filters( 'subcategory_archive_thumbnail_size', 'woocommerce_thumbnail' ) );
                        break;
                    case 'title':
                        ob_start();
                    ...
                    case 'count':
                        if ( isset($category_args['count']) ) {
                            $local_content[$key] = $this->get_category_count($category_args['count']);
                        }
                        else {
                            $local_content[$key] = '';
                        }
            ...
            if ( isset($local_content['image'])) {
                echo '<div class="etheme-category-grid-image etheme-category-grid-image-ghost'. (!!$local_settings['img_rounded'] ? ' rounded' : '') .'" data-type="'.$type.'">';
                    echo '<a rel="nofollow noopener" href="' . esc_url( $category_args['url'] ) . '">';
                        echo $local_content['image'];
    ...
    public function get_category_count($count, $category = false) {
        $count_html = '';
//        if ( $count > 0 ) {
            $count_html .= '<div class="etheme-category-grid-count woocommerce-loop-category__count">';
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            $count_html .= apply_filters('woocommerce_subcategory_count_html', ' <span class="count">' . sprintf(_n('%s product', '%s products', $count, 'xstore-core'), $count) . '</span>', $category);
            $count_html .= '</div>';
//        }
        return $count_html;
    }
        ...
        <<?php echo $html_tag; ?> class="etheme-category-grid-title woocommerce-loop-category__title">
            <?php
                if ( $category && !$custom_url )
                    ...
                    echo '<a aria-label="' . sprintf(esc_attr__('Visit product category %1$s', 'xstore-core'), esc_attr($title)) . '" class="'.($custom_class?$custom_class:'').'" '.($custom_class?' rel="nofollow noopener" ' : '').'href="' . ($custom_url ? esc_url($custom_url) : esc_url(self::$shop_url)) . '">';
                }
...
//	    $settings = $this->get_settings_for_display();
    ...
    public function count_all_products($settings, $edit_preview_mode)
    {
        if ( !$settings['category_count'] ) return [];
        $ids = !$edit_preview_mode ? get_transient('etheme_get_publish_products_ids', false) : false;
        if ($ids === false) {
            $args = array(
                'post_type' => 'product',
                'post_status' => 'publish',
    ...
    public function count_new_products($settings, $edit_preview_mode) {
        if ( !$settings['category_count'] ) return [];
        ...
        if ( ! empty( $select_date_new_products ) && $select_date_new_products != 'anytime' ) {
            ...
            $ids = !$edit_preview_mode ? get_transient('etheme_get_new_products_ids_'.$transient_key, false) : false;
            if ($ids === false) {
                $args = array(
                    'post_type' => 'product',
                    'post_status' => 'publish',
            ...
            $ids = $this->count_all_products($settings, $edit_preview_mode);
        }
        return $ids;
    }

    public function count_on_sale_products($settings, $edit_preview_mode) {
        if ( !$settings['category_count'] ) return [];
