			...
			if ( isset( woof()->settings['by_onsales']['view'] ) && woof()->settings['by_onsales']['view'] === 'switcher' ) {
				...
				<div class="switcher23-container">
					...
					<label for="<?php echo esc_attr( $unique_id ); ?>" class="switcher23-toggle">
						<div class="switcher23-title2"><?php echo esc_html( $woof_ext_onsales_label ) . ' ' . wp_kses_post( wp_unslash( $count_string ) ); ?></div>
						<span></span>                    
					</label>
				</div>
				<?php
				...
				?>
				<input type="checkbox" class="woof_checkbox_sales" id="woof_checkbox_sales" name="sales" value="0" <?php checked( 'salesonly', woof()->is_isset_in_request_data( 'onsales' ) ? 'salesonly' : '', true ); ?> />&nbsp;&nbsp;<label for="woof_checkbox_sales">
				<?php
					echo esc_html( $woof_ext_onsales_label );
