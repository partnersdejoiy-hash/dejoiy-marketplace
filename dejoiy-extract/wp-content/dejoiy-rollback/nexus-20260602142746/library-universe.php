...
function dejoiy_library_is_dashboard_request() {
	...
	// Storefront AJAX (cart, search, LMS) uses admin-ajax.php — must not skip library bootstrap.
	if ( wp_doing_ajax() && ! empty( $_REQUEST['action'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$ajax_action = (string) $_REQUEST['action']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( 0 === strpos( $ajax_action, 'dejoiy_library_' ) ) {
			return false;
		}
	}
...
function dejoiy_library_should_load_nexus_app() {
	...
	$uri = isset( $_SERVER['REQUEST_URI'] ) ? (string) wp_unslash( $_SERVER['REQUEST_URI'] ) : '';
	if ( '' !== $uri && preg_match( '#/(dejoiy-library)(/|\?|$)#', $uri ) ) {
		return true;
	}
	return false;
}
...
/**
 * Register Nexus search AJAX early (customize provides handlers).
 */
function dejoiy_library_boot_nexus_search_ajax() {
	if ( function_exists( 'dejoiy_library_is_dashboard_request' ) && dejoiy_library_is_dashboard_request() ) {
		return;
	}
	dejoiy_library_ensure_customize_loaded();
	$path = get_stylesheet_directory() . '/library-nexus-search.php';
	if ( is_readable( $path ) && ! function_exists( 'dejoiy_library_nexus_search_register_ajax' ) ) {
		require_once $path;
	}
	if ( function_exists( 'dejoiy_library_nexus_search_register_ajax' ) ) {
		dejoiy_library_nexus_search_register_ajax();
	}
}
add_action( 'init', 'dejoiy_library_boot_nexus_search_ajax', 20 );
...
function dejoiy_library_boot_woocommerce() {
	...
	$cart_store = $dir . '/library-nexus-cart-store.php';
	if ( is_readable( $cart_store ) && ! function_exists( 'dejoiy_library_nexus_cart_store_init' ) ) {
		require_once $cart_store;
	}
	if ( function_exists( 'dejoiy_library_nexus_cart_store_init' ) ) {
		dejoiy_library_nexus_cart_store_init();
	}
	...
	$channel_checkout = $dir . '/dejoiy-channel-checkout.php';
	if ( is_readable( $channel_checkout ) && ! function_exists( 'dejoiy_channel_render_wc_checkout_page_content' ) ) {
		require_once $channel_checkout;
	}

	$nexus_chrome = $dir . '/library-nexus-chrome.php';
	if ( is_readable( $nexus_chrome ) && ! function_exists( 'dejoiy_library_nexus_chrome_init' ) ) {
		require_once $nexus_chrome;
	}
	if ( function_exists( 'dejoiy_library_nexus_chrome_init' ) ) {
		dejoiy_library_nexus_chrome_init();
	}

	$shop_routing = $dir . '/dejoiy-shop-routing.php';
	if ( is_readable( $shop_routing ) && ! function_exists( 'dejoiy_shop_routing_init' ) ) {
		require_once $shop_routing;
	}
	if ( function_exists( 'dejoiy_shop_routing_init' ) ) {
		dejoiy_shop_routing_init();
	}

	$universal = $dir . '/dejoiy-universal.php';
	if ( is_readable( $universal ) && ! function_exists( 'dejoiy_universal_bootstrap' ) ) {
		require_once $universal;
	}
	if ( function_exists( 'dejoiy_universal_bootstrap' ) ) {
		dejoiy_universal_bootstrap();
	}

	$nexus_search = $dir . '/library-nexus-search.php';
	if ( is_readable( $nexus_search ) && ! function_exists( 'dejoiy_library_nexus_search_register_ajax' ) ) {
		require_once $nexus_search;
	}
...
function dejoiy_library_universe_enqueue() {
	$load = ( function_exists( 'dejoiy_library_is_landing' ) && dejoiy_library_is_landing() )
		...
		|| ( function_exists( 'dejoiy_library_is_nexus_chrome_screen' ) && dejoiy_library_is_nexus_chrome_screen() );
	if ( ! $load ) {
		return;
	}
	...
	if ( $is_shelf ) {
		...
		wp_localize_script(
			...
			array(
				...
				'cart_boot'    => function_exists( 'dejoiy_library_get_cart_shelf_data' ) ? dejoiy_library_get_cart_shelf_data() : array( 'count' => 0, 'items' => array() ),
			)
		);
		wp_enqueue_script(
			'dejoiy-library-cart',
...
function dejoiy_library_body_class( $classes ) {
	if ( function_exists( 'dejoiy_library_nexus_body_class' ) ) {
		return dejoiy_library_nexus_body_class( $classes );
	}
...
add_filter( 'body_class', 'dejoiy_library_body_class', 20 );
	...
	<div id="dlu" class="dlu-root">
		<section class="dlu-hero" id="dlu-hero" aria-label="Living Knowledge Galaxy">
			...
			<div class="dlu-hero-content">
				...
				<div class="dlu-search-wrap dlu-hero-search-wrap" id="dlu-hero-search-wrap">
					<form class="dlu-hero-search" id="dlu-hero-search" action="<?php echo esc_url( add_query_arg( DEJOIY_LIBRARY_FLOW, '1', dejoiy_library_get_landing_url() ) ); ?>" method="get" role="search">
						...
						<input type="search" name="dlu_q" id="dlu-hero-search-in" placeholder="Search Nexus books, topics, creators…" aria-label="Search DEJOIY Nexus" autocomplete="off" value="<?php echo isset( $_GET['dlu_q'] ) ? esc_attr( sanitize_text_field( wp_unslash( $_GET['dlu_q'] ) ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>" />
					...
					<div id="dlu-hero-search-live" class="dlu-search-live" hidden aria-live="polite"></div>
				</div>
			...
			<div class="dlu-sec-in">
				<div class="dlu-joi-layout">
					...
					<div class="dlu-joi-panel">
						...
						<p class="dlu-sec-desc">Search the Nexus catalog only — books, topics, and creator editions.</p>
						<div class="dlu-joi-box">
							<input type="search" id="dlu-joi-input" placeholder="Search business, design, marketing, a title…" autocomplete="off" enterkeyhint="search" aria-label="Search Nexus books" />
		...
		<section id="dlu-creator-editions" class="dlu-sec">
			<div class="dlu-sec-in">
				...
				<p class="dlu-sec-desc">Books you add in WooCommerce under <strong>DEJOIY Library</strong> — sold through Nexus cart and checkout, separate from the main marketplace.</p>
