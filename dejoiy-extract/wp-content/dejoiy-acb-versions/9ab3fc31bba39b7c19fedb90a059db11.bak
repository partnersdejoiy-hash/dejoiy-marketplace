...
function dejoiy_studio_maybe_set_cookie() {
	if ( is_page( 4399 ) || ( isset( $_GET['dejoiy_studio'] ) && '1' === $_GET['dejoiy_studio'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( ! headers_sent() ) {
...
function dejoiy_studio_is_flow() {
	if ( isset( $_GET['dejoiy_studio'] ) && '1' === $_GET['dejoiy_studio'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return true;
	}
...
/**
 * Template swap: single product + checkout.
 *
...
function dejoiy_studio_add_to_cart_url( $url ) {
	if ( dejoiy_studio_is_flow() ) {
		$url = add_query_arg( 'dejoiy_studio', '1', $url );
	}
	return $url;
}
...
add_filter( 'woocommerce_add_to_cart_redirect', 'dejoiy_studio_add_to_cart_redirect' );

/**
 * Enqueue studio assets on single + checkout.
 */
function dejoiy_studio_wc_enqueue() {
	if ( ! dejoiy_studio_use_single_template() && ! ( function_exists( 'is_checkout' ) && is_checkout() && dejoiy_studio_is_flow() ) ) {
		return;
	}
	...
	wp_enqueue_style(
		...
		array(),
		(string) ( file_exists( $dir . '/studio-single.css' ) ? filemtime( $dir . '/studio-single.css' ) : '10.0' )
	);
	wp_enqueue_style(
		'dejoiy-studio-hf',
		$uri . '/studio-hf.css',
		array(),
		(string) ( file_exists( $dir . '/studio-hf.css' ) ? filemtime( $dir . '/studio-hf.css' ) : '10.0' )
	);
}
add_action( 'wp_enqueue_scripts', 'dejoiy_studio_wc_enqueue', 1004 );

/**
 * AJAX search — studio products first, then all marketplace products + pages.
 */
