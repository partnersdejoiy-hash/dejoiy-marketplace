...
<p class="add_product_video hide-if-no-js">
	<a href="#" class="button"><?php esc_html_e( 'Add product gallery video', 'xstore-core' ); ?></a>
</p>
<p class="product_video_autoplay">
    ...
    >
    <label for="et_product_video_autoplay"><?php esc_html_e( 'Autoplay product gallery video on single product page. Video will be muted.', 'xstore-core' ); ?></label>
</p>
<p>
	<?php esc_html_e('Or you can use YouTube or Vimeo iframe code', 'xstore-core'); ?>
    <a class="expand_product_video_code" href="#" data-show-text="<?php esc_attr_e('Expand', 'xstore-core'); ?>"
       data-hide-text="<?php esc_attr_e('Mimimize', 'xstore-core'); ?>">
        <?php
        if ( !$product_video_code )
            esc_html_e('Expand', 'xstore-core');
        else
            esc_html_e('Mimimize', 'xstore-core');
            ?>
    </a>
</p>
<div class="product_iframe_video"<?php if ( !$product_video_code ) : ?> style="display: none"<?php endif; ?>>

	<textarea name="et_video_code" id="et_video_code" rows="7"><?php echo esc_attr( $product_video_code ); ?></textarea>

</div>

<h4><?php esc_html_e('More video settings:', 'xstore-core'); ?></h4>
<ul>
    <li>
        <?php echo sprintf(__('You may <a href="%s" target="_blank">change the positions</a> of your video in product gallery on single product page.', 'xstore-core'),
            (get_option( 'etheme_single_product_builder', false ) ? admin_url( '/customize.php?autofocus[section]=product_gallery' ) : apply_filters('etheme_theme_options_permalink', admin_url( '/customize.php?autofocus[section]=single-product-page-layout' ), 'single-product-page-layout'))
        ); ?>
    </li>
    <li>
        <?php echo sprintf(__('You may use <a href="%s" target="_blank">video instead of featured image</a> for product archive pages.', 'xstore-core'),
                apply_filters('etheme_theme_options_permalink', admin_url( '/customize.php?autofocus[section]=products-style' ), 'products-style')
        ); ?>
    </li>
</ul>
...
<script type="text/javascript">
    jQuery(document).ready(function($){
        ...
        let expaned = <?php if ($product_video_code) : ?>true<?php else: ?>false<?php endif; ?>;
        ...
        $('.expand_product_video_code').on('click', function (e) {
            e.preventDefault();
            if ( expaned ) {
                $('.product_iframe_video').slideUp()
                $(this).text($(this).data('show-text'));
            }
            else {
                $('.product_iframe_video').slideDown()
