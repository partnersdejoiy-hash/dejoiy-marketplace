...
(function ($) {
    ...
    $(function () {
	    ...
	    let formdata = {};
	    woof_parse_str($(this).serialize(), formdata)
	...
	function woof_parse_str(str, array) {
...
function woof_tab_click(item, hash) {
    ...
    tabs.querySelectorAll('li').forEach((li, i) => {
        if (i === index) {
            li.classList.add('selected-tab');
            li.classList.add('tab-current');
        } else {
    ...
    tabs.parentNode.parentNode.querySelectorAll('.content-wrap > section').forEach((section, i) => {
        section.classList.remove('content-current');
        if (hash === '#' + section.id) {
            section.classList.add('content-current');
            section.querySelector('.woof-tabs > .content-wrap > section').style.display = 'block';
        }
    });
}
