...
class Product_Status_Filter extends WC_Widget {
	function __construct() {
		...
		$this->widget_description = esc_html__( 'Widget to filtering products by stock status', 'xstore-core' );
		...
		$this->settings           = array(
			...
			'request_type' => array(
				...
				'options' => array(
					'old' => esc_html__( 'Advanced', 'xstore-core' ),
					'new' => esc_html__( 'Default', 'xstore-core' ),
				),
			),
			'count' => array(
				'type'  => 'checkbox',
		...
		$this->is_tax_condition_set = false;
		$this->is_tax_condition = false;
		$this->ids_on_sale = false;
		parent::__construct();
	...
	function widget($args, $instance) {
		...
		if (is_admin() || ! is_shop() && ! is_product_taxonomy() && ! is_product_category() && ! is_product_tag() && ! is_tax('brand') ){
			return;
		}
...
//		}
        ...
        <ul>
            <li class="<?php echo $in_stock_class; ?>">
                <a rel="nofollow noopener" href="<?php echo $in_stock; ?>" class="etheme-ajax-filter etheme-stock-filter">
					<?php echo esc_html__( 'In stock', 'xstore-core' ); ?>
					<?php if ($count) {
						$in_stock_count = $this->get_status_count('instock');
						echo apply_filters( 'etheme_product_status_filter_widget_count', '<span class="count">(' . $in_stock_count . ')</span>', $in_stock_count );
					} ?>
                </a>
            </li>
            <li class="<?php echo $out_of_stock_class; ?>">
                <a rel="nofollow noopener" href="<?php echo $out_of_stock; ?>" class="etheme-ajax-filter etheme-stock-filter">
					<?php echo esc_html__( 'Out of stock', 'xstore-core' ); ?>
					<?php if ($count) {
						$out_of_stock_count = $this->get_status_count();
						echo apply_filters( 'etheme_product_status_filter_widget_count', '<span class="count">(' . $out_of_stock_count . ')</span>', $out_of_stock_count );
					} ?>
                </a>
            </li>
            <li class="<?php echo $class; ?>">
                <a rel="nofollow noopener" href="<?php echo $sale_link; ?>" class="etheme-ajax-filter etheme-sale-filter">
					<?php echo esc_html__( 'On sale', 'xstore-core' ) ?>
					<?php //echo ($count) ? '(' .count(wc_get_product_ids_on_sale()). ')' : ''?>
					<?php if ($count) {
						$sale_stock_count = $this->get_status_count('', '_sale_price', '!=');
						echo apply_filters( 'etheme_product_status_filter_widget_count', '<span class="count">(' . $sale_stock_count . ')</span>', $sale_stock_count );
					} ?>
                </a>
            </li>
        </ul>
	...
	public function get_status_count($type = 'outofstock', $k = '_stock_status', $q = '='){
		...
		$min_price          = isset( $_GET['min_price'] ) ? floatval(wc_clean( wp_unslash( $_GET['min_price'] ) ) ) : 0; // WPCS: input var ok, CSRF ok.
		$max_price          = isset( $_GET['max_price'] ) ? floatval(wc_clean( wp_unslash( $_GET['max_price'] ) ) ): 0; // WPCS: input var ok, CSRF ok.
		...
		if ($this->request_type == 'new'){
			...
			if ($k == '_sale_price'){
				$this->ids_on_sale = $this->ids_on_sale ? $this->ids_on_sale : wc_get_product_ids_on_sale();
				if ( count($this->ids_on_sale) ) {
//		        AND p.post_type IN ('product', 'product_variation')
					$outofstock = "SELECT ID FROM $wpdb->posts as p
                WHERE p.post_status = 'publish'
                AND p.post_type IN ('product')
                AND p.ID IN (" . implode( ',', $this->ids_on_sale ) . ")
            ";
				}
				else {
					return count($this->ids_on_sale);
				}
			} else {
				$outofstock = "SELECT ID FROM $wpdb->posts as p 
			...
			// $price_ids 		= $this->get_price_ids($wpdb, $min_price, $max_price); // price filter
			// $rating_ids 	= $this->get_rating_ids($wpdb, $rating_filter); // rating_filter
			// $brand_ids 		= $this->get_brand_ids($wpdb, $filter_brand); // filter_brand
			// $attributes_ids = $this->get_attributes_ids($wpdb $_chosen_attributes); // chosen_attributes
			// $taxonomy_ids   = $this->get_taxonomy_ids($wpdb); // $taxonomy
			// $cat_filter_ids = $this->get_product_cat_filter_ids($wpdb);// cat filter
			...
			//     $this->get_product_cat_filter_ids($wpdb)// cat filter
			// );
			...
			$attributes_ids = $this->get_attributes_ids($wpdb, $_chosen_attributes); // chosen_attributes
			$taxonomy_ids = $this->get_taxonomy_ids($wpdb); // $taxonomy
			$product_cat_ids = $this->get_product_cat_filter_ids($wpdb); // cat filter
			...
			if ($isd) {
				foreach ($filters as $filter) {
					$isd = array_intersect($isd, $filter);
				}
			}
        ...
        ";
			...
			if ( ! empty( $_chosen_attributes ) ) {
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					...
					if ($data['query_type'] == 'or'){
						$t_ids_imploded = implode( ',', $t_ids );
						if ( ! $t_ids_imploded ) {
							$t_ids = 0;
						} else {
							$t_ids = $t_ids_imploded;
						}
			...
			// $taxonomy
			$this->is_tax_condition_set = true;
			if (($this->is_tax_condition_set && $this->is_tax_condition) || is_product_category() || is_product_tag() || is_product_taxonomy() || is_tax('brand')){
				$this->is_tax_condition = true;
...
//        ";
	...
	private function get_price_ids($wpdb, $min_price, $max_price) {
		...
		if ($min_price || $max_price) {
			$price_query = "
		        ...
		        AND p.post_status = 'publish'
		        AND p.post_type = 'product'
		        AND pm.meta_value != ''  -- Ensure the price is not empty
		    ";

			if ($min_price) {
				$price_query .= " AND pm.meta_value >= $min_price";
			}
			if ($max_price) {
				$price_query .= " AND pm.meta_value <= $max_price";
			}

			$price_ids = array_keys($wpdb->get_results($price_query, OBJECT_K));
		}
	...
	private function get_product_cat_filter_ids($wpdb){
		$ids = array();
			...
			if (count($terms)) {
				foreach ($terms as $term) {
					...
					if ($term) {
						...
						$request = " SELECT $wpdb->term_relationships.object_id  FROM $wpdb->term_relationships WHERE term_taxonomy_id  IN (" . $category_id . ")";
						$ids = array_merge($ids, array_keys($wpdb->get_results( $request, OBJECT_K  )));
					}
				}
			}
		}
...
//            $this->ids_on_sale = $this->ids_on_sale ? $this->ids_on_sale : wc_get_product_ids_on_sale();
			if ( !$this->ids_on_sale )
				$this->ids_on_sale = wc_get_product_ids_on_sale();
			...
			if ( $variable_products_detach ) {
				if ( $variable_products_no_parent && function_exists('etheme_product_variable_with_children_excluded')) {
					$ids = array_diff($this->ids_on_sale, etheme_product_variable_with_children_excluded());
				}
			}
		}
		return $ids;
...
//		    $this->ids_on_sale = $this->ids_on_sale ? $this->ids_on_sale : wc_get_product_ids_on_sale();
			if ( !$this->ids_on_sale )
				$this->ids_on_sale = wc_get_product_ids_on_sale();
			if (count($this->ids_on_sale)){
				$ids = array_merge( $ids, $this->ids_on_sale );
	...
	public function show_in_stock_products( $query ) {
		if ( isset( $_GET['stock_status'] ) && $_GET['stock_status'] ) {
			$meta_query = array();
			if ( in_array($_GET['stock_status'], array('in_stock', 'out_of_stock')) ) {
				$meta_query = array(
					'relation' => 'AND',
					array(
						'key'     => '_stock_status',
						'value'   => str_replace(array('in_stock', 'out_of_stock'), array('instock', 'outofstock'), $_GET['stock_status']),
						'compare' => '=',
					),
				);
			}
